...
sum([Turing.logpdf(Turing.MvNormal(ℒ.Diagonal(ones(size(state_deviations_FV,1)))), state_deviations_FV[:,t]) for t in 1:size(data1, 2)])

sum([Turing.logpdf(Turing.MvNormal(zeros(size(data1)[1]),Matrix(0.01*I, size(data1)[1], size(data1)[1])), state_deviations_FV[:,t]) for t in 1:size(data1, 2)])



Plots.plot(data_sim[:,1:end]')
Plots.plot!(dataFV[:,1:end]')
...
Turing.@model function loglikelihood_scaling_function(m, data, observables,Ω)
    ...
    state_deviations = data[:,1:end] - state[observables_index,1:end-1]
...
#Plot true and estimated latents to see how well we backed them out
    ...
    ϵ_mean = tmp[1][:, 2]
    ϵ_std = tmp[1][:, 3]
    Plots.plot(ϵ_mean[1:end], ribbon=1.96 * ϵ_std[1:end], label="Posterior mean", title = "Second-Order Joint: Estimated Latents")
