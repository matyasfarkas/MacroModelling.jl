...
md"Next we want to make sure we are in the right directory. We can use `pwd()` to print the current directory and `cd()` to change it:"

# ╔═╡ 29c0b1e6-e2af-4e8b-8084-4bb845f2a5be
pwd()

# ╔═╡ 75730efc-f21b-4589-af0f-67e535def462
md"The package includes a function to translate model equations and parameters from a dynare mod file into the format for MacroModelling.jl:"

# ╔═╡ ad95a2a1-dc58-454b-9fec-16d51bd06bc8
import_model("Gali_2015_chapter_3_nonlinear.mod")

# ╔═╡ 45a6e367-b70f-47fa-b055-c3b4140260f6
md"Now that we translated the mod file let's compare the two:"

# ╔═╡ 4c0baa30-0011-4aa9-84d6-143b2561b94b
print(open(f->read(f, String), "Gali_2015_chapter_3_nonlinear.mod"))

# ╔═╡ adbef8c4-8e5f-40cf-bb5a-8757157d9f94
md"This is a fairly complex example making extensive use of dynares features such as macros, variable labelling, and steady state model."

# ╔═╡ a41f5788-d10c-4814-b0be-3a11d3315b1c
print(open(f->read(f, String), "Gali_2015_chapter_3_nonlinear.jl"))

# ╔═╡ 813cfae8-d66c-4f33-89b6-b55c40c144ce
md"The translation takes into account the macro settings of the mod files. 
Notable differences between the two files:
- no variable, parameter declarations thanks to slightly different naming convention (all variables followed by timing in square brackets)
- no steady state model (MacroModelling tries to solve it automatically)

Let's try to run the model then:
"

# ╔═╡ 2ea63cd2-55c7-45fa-9eb4-41f6d1e56e01
md"It does not work out of the box and we get an error that he cannot solve the system of equations determining the non stochastic steady state (NSSS). Furthermore, there is a hint that we can either set some variables in steady state as parameters to pin down the NSSS or check the model for nonstationary variables and eliminate them.

We need to do some changes to the model:"

# ╔═╡ e9a8f6e8-ca34-4790-bb8c-51ca5982de44
...
# ╔═╡ b2d51e7e-c8a4-4d71-964c-7ec55b6b5002
...
# ╔═╡ af5f819a-d0ce-4898-8e5f-cdf93acc541e
md"After the necessary changes we can work with the model.
For example we can get the NSSS:"

# ╔═╡ e9f88fdb-5787-4a54-b258-464961a477f8
get_SS(Gali_2015_chapter_3_nonlinear_fix) |> show_pluto

# ╔═╡ 689dd1e8-7f67-408a-b97c-2290bf3bfe74
md"Or you can plot IRFs:"

# ╔═╡ bc2cfb99-9f61-47b6-b514-b0fd5f8378c5
import StatsPlots

# ╔═╡ 4e205d2c-c3a8-4fdc-bc2b-9f2943881af2
plot_irf(Gali_2015_chapter_3_nonlinear_fix)[1]

# ╔═╡ 9545af35-d2f7-412e-95b1-59dd153eeabb
md"The plots show the level of the left y axis and the % deviation on the right y axis. The black horizontal line represents the NSSS."

# ╔═╡ f06a56da-7ee0-41ed-812e-4d994aab3b71
md"Some users might want to use MacroModelling only to get the IRFs and use another software to produce plots. This can easily done by calculating the IRFs as raw numbers and exporting them to a suitable format. Let's start by calculating the IRFs:"

# ╔═╡ a326af7a-babe-4e08-9ad2-470119b4a82c
irfs = get_irf(Gali_2015_chapter_3_nonlinear_fix);

# ╔═╡ d79c630c-cf22-4264-ae2d-06d38fe146e1
irfs |> show_pluto

# ╔═╡ cfed9638-46b0-4a68-9ef3-cad16dae988e
md"We get a 3-dimensional array containing with the axis being the model variables, shocks, and simulation periods. Note that the output is in absolute deviations from NSSS. Let's get them in absolute values. The help function will guide you to the right option:"

# ╔═╡ 7f9f96b3-5bfa-4656-af90-774e5fea1d0a
irf_lvls = get_irf(Gali_2015_chapter_3_nonlinear_fix, levels = true);

# ╔═╡ 3f605dce-f6ce-4c70-8ea4-ea9aaa9c7a94
irfs |> show_pluto

# ╔═╡ 5fc0944c-c423-4aee-9da7-f00576790fa5
md"Let's export these IRFs as a .mat file so we can use them in MATLAB. First we need to load the MAT package:"

# ╔═╡ 10d54ac8-4590-442f-bda1-6c2c3518dadd
md"and then we can export the array as a mat file:"

# ╔═╡ f761da87-9f8e-42dc-a2f6-bc8d8137c54e
matwrite("irfs.mat", Dict("irfs" => collect(irfs)))

# ╔═╡ 2d83d93c-cb13-4576-840b-c9cd35662b21
md"Another use case can be that we want to use MacroModelling to solve the NSSS model and then continue using dynare. In this case we can export the model backto dynare:"

# ╔═╡ d2c5421f-07f0-4f0d-8c05-ff04f5ff836e
write_mod_file(Gali_2015_chapter_3_nonlinear_fix)

# ╔═╡ 5c2bb200-5f43-400f-8b01-ef7f5f5bad3d
print(open(f->read(f, String), "Gali_2015_chapter_3_nonlinear_fix.mod"))

# ╔═╡ 93c49837-c2a4-4ec9-9d60-e7307c7db7c3
md"And with this mod file we can go back to dynare and run our analysis there if we want."

# ╔═╡ 41099ee5-0502-45a1-ac0f-e7b69058c02e
md"## Some Julia basics
Creating vectors and arrays
"

# ╔═╡ 356037da-d694-49b0-8a55-98f1eb74b149
V = [0, 1, 2]

# ╔═╡ 458a9404-e2b2-4a78-a971-05074fcbf5a2
M = [1 2 3
	 3 4 5]

# ╔═╡ 71119790-9655-4f6d-b09c-07b77c2a1ba7
M̂ = [1 2 3 ;3 4 5]

# ╔═╡ 02150232-d8b1-4222-8272-c67313a29f73
V2 = rand(3)

# ╔═╡ 19a4f837-7161-4efc-9a20-115e1f213390
M2 = rand(3,3)

# ╔═╡ 77a572f7-1ba6-4011-92bf-4dc744ceedeb


# ╔═╡ e378cbab-d432-4752-9a78-e1bed3ea801c
Vsparse = spzeros(3)

# ╔═╡ 552a2730-64a9-45d6-9e30-8046f5ce17ab
Msparse = spzeros(3,3)

# ╔═╡ 727ef317-5edc-4574-8c5f-8e348ed8469f
Vkeyed = KeyedArray(rand(2), Variable = [:K, :Y]);

# ╔═╡ cf6fcad2-a8fc-4be5-a3f8-9f086b111a6e
Vkeyed|> show_pluto

# ╔═╡ 53522503-51b6-4135-8fa1-5be39774d821
Mkeyed = KeyedArray(rand(2,2), Variable = [:K, :Y], Shock = [:ϵ, :ε])

# ╔═╡ dbd7b5f3-4e61-4500-a282-1d622085b7fd
Mkeyed |> show_pluto

# ╔═╡ 2130de2f-8d05-41b0-b047-ea54583f34fe
md"Accessing and modifying vectors and matrices"

# ╔═╡ 8c90a929-c44f-4a84-93b5-40d95cb7e935
V[1]

# ╔═╡ 5002c27b-3f2c-491d-9af1-051e1d2bf9a9
V[1:2]

# ╔═╡ ae55d689-661a-4dbc-be35-396448c5282b
V[2] = -9

# ╔═╡ 0c3a62e2-16cd-4019-984f-859843f0bff7
M[1,2]

# ╔═╡ 305dd45e-fed2-4a28-898d-cc1c587c1519
M[2,2:3]

# ╔═╡ 0f39782d-d322-429b-a60d-7a62b3586994
M[2,2:3] .= 14

# ╔═╡ df537bc3-9558-48f7-8736-817d6b5bb879
Msparse[1,2] = 10

# ╔═╡ 492df33a-b5fa-4b2d-adb7-d4d09878ca94
Msparse

# ╔═╡ f296609e-47e0-42b0-8fe8-c01a39ee2a41
findnz(Msparse)

# ╔═╡ 707c51fa-02d5-4804-963e-12c715c83191
Vkeyed[1]

# ╔═╡ 7ff30dcf-b497-4b9d-bdf5-86671b2db813
Vkeyed[1] = -12

# ╔═╡ 373486e9-e2e5-48e6-8cd9-bacc35141db4
# Vkeyed(:K) = 1 # doesnt work

# ╔═╡ 747689c2-e76d-424e-8e56-b2e2ea6689af
axiskeys(Vkeyed)

# ╔═╡ 15bb0759-f18b-4a73-b812-2f0b0be90c6a
Mkeyed(:K,:ϵ)

# ╔═╡ 4a25403b-2086-4036-9f66-3cf3d7ab2e25
Mkeyed[1,2]

# ╔═╡ ec0b2827-543c-48ea-bf87-99e79fee68c1
axiskeys(Mkeyed)

# ╔═╡ a430acd6-8763-4ecb-a230-2f0001965e4d
md""" 
...
# ╔═╡ 4f9cf45b-1845-41fb-8fcb-3a20f5ecb061
@model RBC begin
	K[0] = (1 - δ) * K[-1] + I[0]
	Y[0] = Z[0] * K[-1]^α
	Y[0] = C[0] + I[0]
...
# ╔═╡ 0436bccc-083e-45a4-bd3b-2ea42587a8aa
@model RBC1 begin
	K[0] = (1-δ) * K[-1] + I[0]
	Y[0] = Z[0] * K[-1]^(α)
	Y[0] = C[0] + I[0]
	1/(C[0]^γ) = β * 1/(C[1]^γ) * (α * Y[1]/K[0] + (1-δ))
	Z[0] = (1-ρ) + ρ* Z[-1] + σ *EPS[x]
end

# ╔═╡ 8b1e5494-76ce-4bd2-8cd7-d1fad1af589b
@parameters RBC1 verbose=true begin
	α = 1/3
	β = 0.99
	δ = 0.025
	γ = 1
...
plot_irf(RBC, parameters = [:α => x, :ρ => y],periods = 100)[1]
...
md"Once written down we can generate output like the non stochastic steady state, IRF plots, or the theoretical covariance."

# ╔═╡ 0fbaa428-a5f2-4aa0-9aa1-d91505f7819c
get_SS(RBC) |> show_pluto

# ╔═╡ d11a6224-8d9b-4144-aea6-94475c1e0f0f
plot_irf(RBC)[1]

# ╔═╡ 2123a14b-e200-4369-9318-4e68d772eabd
get_covariance(RBC) |> show_pluto

# ╔═╡ 3ada1d44-c996-48f9-8af8-692f2dffc0dd
md"Now lets try to calibrate the steady state such that the capital to yearly GDP ratio is 1.66"

# ╔═╡ 0f23e18f-ec52-42bf-87b4-7d82aba90b64
...
@parameters RBC_calib verbose = true symbolic = true begin
	α = 1/3
	β = 0.99
	k_y = 1.66
	δ | k_y = K[ss] / (4 * Y[ss])
...
md"Let's compare the NSSS and it's derivatives:"
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "a666705491dc5776a2ce21d2c1b127fcf9d404bd"
...
deps = ["ChainRulesCore", "Krylov", "LinearOperators"]
git-tree-sha1 = "25ae10b4942b9405a0ca63524270ba8b6da8694f"
...
[[deps.MacroModelling]]
deps = ["AxisKeys", "BlockTriangularForm", "ChainRulesCore", "ComponentArrays", "DataStructures", "DocStringExtensions", "DynarePreprocessor_jll", "ForwardDiff", "ImplicitDifferentiation", "IterativeSolvers", "JSON", "LaTeXStrings", "LinearAlgebra", "LinearMaps", "MacroTools", "PrecompileTools", "Random", "RecursiveFactorization", "Reexport", "Requires", "RuntimeGeneratedFunctions", "SparseArrays", "SpecialFunctions", "SpeedMapping", "Subscripts", "SymPy", "Symbolics"]
git-tree-sha1 = "30bd7364bed45014e64f629d5721cc746d7381bd"
