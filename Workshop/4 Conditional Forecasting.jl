...
# ╔═╡ d694dd0e-e74a-11ed-1bbf-db4c871ecd66
using MacroModelling

# ╔═╡ 6a935592-6634-4030-a323-9cc7fcd0e6c2
using Statistics, CSV, DataFrames, StatsPlots

# ╔═╡ 8b6547ca-4790-4663-b8fb-fee474d30760
show_pluto(x) = Text(sprint(show, "text/plain",x));

# ╔═╡ 0d41a6c7-63ff-4bdf-b8a3-ad333117f1e7
md"# Conditional forecasting"

# ╔═╡ 0ae27e8d-e4f3-42a5-bc60-093bc93f6904
md"What if the central bank does not hike interest rates for a year. What would be the additional impact on inflation?

We will use the Smets and Wouters 2003 model in it's nonlinear form."

# ╔═╡ 290b8c04-ddca-4d99-bf76-34807d8f2538
@model SW03 begin
...
# ╔═╡ 85cb90d0-0eda-4afa-a5d0-6c0e7263a778
...
# ╔═╡ 6ff85f0d-f0d8-4d5e-9f48-c96531e761b2
md"Let's start by having a look at some standard output:"

# ╔═╡ 4ffb4ef7-bf83-4dfa-8c96-ce2f18e07622
get_SS(SW03) |> show_pluto

# ╔═╡ f981f840-cc39-4773-9abf-895f324f5d2a
get_SS(SW03)([:R,:R_obs,:pi_obs,:Y_obs,:C_obs,:I_obs,:Rʸ_bps,:πʸ_bps])|> show_pluto

# ╔═╡ 17e16885-c478-49ea-a75c-0e1eff6c722c
md"In the following steps we will get the models view of the state of the euro area given [actual data](http://www.thorekockerols.eu/data/EA_SW_data_growth.csv). Therefore, we first need to load the data and bring it in a suitable format."

# ╔═╡ e0a25172-7c72-40dd-a061-96da3b01ae6b
dt = CSV.read("/Users/thorekockerols/GitHub/MacroModellingWorkshop/EA_SW_data_growth.csv", DataFrame)

# ╔═╡ 88fc5ac5-6244-4551-a367-652959a7e810
nms = names(dt)[[2:8...,11]]

# ╔═╡ 8439b7be-856b-4ec7-be28-747471c0116e
md"Let's have a look at the series:"

# ╔═╡ aeb2a4b1-12d6-4198-900e-e523c9346ede
plot(layout = (3, 3));

# ╔═╡ f9009a93-2743-4a44-90e5-0e645bcba26f
for (i,v) in enumerate([2:8...])
	plot!(subplot = i ,collect(dt[:,v]), labels = nms[i])
end

# ╔═╡ 9a5dbc67-e0f0-4fbb-911b-0da8ac9ee94f
plot!(subplot = 8,collect(dt[:,9]), labels = nms[8])

# ╔═╡ b950265a-a7d2-4155-9069-6de8c7d01335
md"The SW03 model is stationary therefore we substract the mean of the variables in growth rates."

# ╔═╡ 478b33d5-eadd-41e7-b21a-bc3307c2a683
...
# ╔═╡ e001ab78-6bae-447b-8707-8511b7310e05
...
# ╔═╡ 0534fb83-51b5-41bf-a341-519524217a20
subdata = data(sort([:R_obs,:Y_obs,:C_obs,:I_obs,:pi_obs,:W_obs,:L_obs]),:)

# ╔═╡ 1bdbf13a-34ce-428b-8d0c-65722aa2e495
md"This subset of the demeaned data as a `KeyedArray` is what we need to filter for the model states. We are interested only in the last state: 2022Q3 so that for 2022Q4 onwards we can do our scenario analysis."

# ╔═╡ 5eb366d0-3371-42f5-8335-1b8cce6ba83c
final_state = get_estimated_variables(SW03, subdata)(:,size(subdata,2))

# ╔═╡ 1eaff866-0721-472a-9c7a-b760a2b65512
md"We can also plot the filtered states of the model:"

# ╔═╡ 78e9eac4-394c-4d8d-8835-7c722db3b6cd
plot_model_estimates(SW03, subdata)[end-1]

# ╔═╡ 3bf718e3-c9c1-4f12-a206-4743614d27aa
md"And we can look at the uncoditional forecast from 2022Q3 onwards:"

# ╔═╡ 2c16818c-0a83-43da-b159-9c072ffa0252
plot_irf(SW03, initial_state = collect(final_state), shocks = :none, variables = [:Y_obs,:R_obs,:pi, :Rʸ_bps,:πʸ_bps])[1]

# ╔═╡ 983e9c0d-8f96-42e6-85ed-5002d8fc0c0d
md"The initial question was about what if the central bank keeps the rate constant during the period 2022Q4-2023Q4. Therefore we wil use conditional forecasting. The condition being that rates are constant for the next 4 quarters:"

# ╔═╡ f72617c7-1652-4493-bb4f-9f9e685debcf
conditions = KeyedArray(fill(final_state(:R_obs),1,4),Variables = [:R_obs], Periods = collect(1:4))

# ╔═╡ 037c6d03-087c-4f51-9812-f5fd9da67c0e
md"Furthermore, we want to know what the corresponding monetary policy shock would be which achieves this path of the interest rate. Therefore, we know that all other shocks should be 0 for those periods:"

# ╔═╡ 0e1c100e-e319-4505-a670-6ede9767f54f
shocks = KeyedArray(zeros(8,4),Variables = setdiff(SW03.exo,[:eta_R]), Periods = collect(1:4))

# ╔═╡ 35997e93-cd05-4e1a-ad49-1802fc0cb19f
md"Given these conditions on the endogenous variables and the shocks we can retrieve the condition forecast:"

# ╔═╡ 154acca6-3b95-4c3a-8a1e-010558ac1d5c
plot_conditional_forecast(SW03, conditions, shocks = shocks, initial_state = collect(final_state), variables = [:Y_obs,:R_obs,:pi, :Rʸ_bps,:πʸ_bps])[1]

# ╔═╡ 9fd45333-c29f-466d-a1ea-7d094003544d
md"The other part of the question was to quantify the additional impact from these monetary policy shocks on inflation. So far we understood the path for the economy but not the additional impact. In order to do so we have to get the shocks implied by the path:"

# ╔═╡ 0609fec4-89e6-4e70-aaec-63c80c0db547
implied_paths = get_conditional_forecast(SW03, conditions, shocks = shocks, initial_state = collect(final_state), variables = [:Y_obs,:R_obs,:pi])

# ╔═╡ d6ed0c76-7962-46f5-bbf3-22298794c842
md"We can use this shocks series to plot IRFs:"

# ╔═╡ 560b8615-9900-4ebc-8b36-9600529cc455
plot_irf(SW03, shocks = implied_paths([:eta_R₍ₓ₎],:), variables = [:Y_obs, :Rʸ_bps,:πʸ_bps,:W,:C,:I])[1]

# ╔═╡ b8457dcc-b687-4460-8803-9a0449da45b6
md"We see that the tigtening shocks imply an additional inflation impact at the peak of 40bps."
...
manifest_format = "2.0"
project_hash = "83fabfbc4588b901ffa55339388b89a119525123"
