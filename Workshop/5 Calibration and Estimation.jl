...
# ╔═╡ 4c53cfb4-e8ef-11ed-3a16-b38987e0a6fa
using MacroModelling

# ╔═╡ d3238b48-e613-4acc-b6a5-fe70245ba639
using Optim, LineSearches

# ╔═╡ ae42bf46-10a0-4cc7-ba09-c46eaecde1f0
using StatsPlots, Statistics, CSV, DataFrames

# ╔═╡ 2a5ae13f-d005-4c36-8124-c4726d57b1fc
show_pluto(x) = Text(sprint(show, "text/plain",x));

# ╔═╡ f1c6e686-146c-463d-9b92-492e0b869dde
md"# Calibration and estimation of models
...
# ╔═╡ 055542e1-4d1a-4719-99e0-0de73d1f3468
@model FS2000 begin
...
# ╔═╡ bb1bfcb6-78c1-41c7-bef0-0610269665a1
...
# ╔═╡ 813226c3-02d5-4f19-a72d-f2c7232156bc
get_SS(FS2000)|>show_pluto

# ╔═╡ a2bf396d-1054-4313-9d1c-a0df6a0e4a1c
md"Let's calibrate the SS such that the capital to GDP ratio is 10.4 and the short term interest rate is 7% in annualised terms by using the parameters δ and β."

# ╔═╡ eaf5c822-fac5-498a-a6b7-3905d7cc37fb
...
# ╔═╡ 83e7a45d-23ff-4528-842b-5fbcd5de527c
...
# ╔═╡ 316bbeb0-440f-4eaa-a400-e520c03654e2
md"Having done the necessary changes let's look at the standard output again:"

# ╔═╡ 1c179ddd-9b04-4651-8169-578164962e4b
get_SS(FS2000_calib)|>show_pluto

# ╔═╡ 41614b51-b64e-4297-abca-557bc1ecb211
get_std(FS2000_calib)|>show_pluto

# ╔═╡ b287832f-697f-43ba-830f-b5ec2a3ee085
md"Next, let's calibrate the standard deviation of the annualised interest rate to be 3.4 and the standard deviation of the output gap be 2. We are going to make this an optimisation problem with 2 targets and 3 unknowns (parameters of the model). We will use the parameters α, γ, and ρ to reach our targets. First we need a function mapping the parameters to the standard deviations:"

# ╔═╡ e6869253-bb8d-494a-ab3b-de9f9535f8e6
get_statistics(FS2000_calib,[0.4036,0.6811,0.8468],parameters = [:α, :ψ, :ρ], standard_deviation = [:R_ann,:y_gap])[1]

# ╔═╡ d4076140-e9a9-4f1c-9a1b-f51edc566cd6
md"Now we can use this to optimise over:"

# ╔═╡ 2ccb8367-6d40-4323-9568-d32a5731306d
sum(abs2, get_statistics(FS2000_calib,
	[0.4036, 0.6811, 0.8468],
	parameters = [:α, :ψ, :ρ], 
	standard_deviation = [:R_ann, :y_gap])[1] - [3.4, 2])

# ╔═╡ 0fde5e65-b545-47b1-87be-1e6adcca5955
results = optimize(x -> sum(abs2, get_statistics(FS2000_calib,x,
	parameters = [:α, :ψ, :ρ], 
	standard_deviation = [:R_ann,:y_gap])[1] - [3.4,2]),
	[0, 0, 0],
	[0.5, 2, 1],
	[0.4036, 0.6811, 0.8468],
	Fminbox(LBFGS(linesearch = BackTracking(order = 3))), 
	autodiff = :forward)

# ╔═╡ 870cf631-9be4-451c-bc2a-57e61aec1f38
md"Let us check the result of the optimisation. First the optimal parameter values and then the implied standard deviations."

# ╔═╡ f0555d16-a107-4f61-97ad-fb953b29c0dd
results.minimizer

# ╔═╡ 2c022633-2404-463f-83d4-c82115e2cd03
get_statistics(FS2000_calib,
	results.minimizer,
	parameters = [:α, :ψ, :ρ], 
	standard_deviation = [:R_ann,:y_gap])[1]

# ╔═╡ 7dc3fe7e-8bb9-409a-80c5-1f56bca365ec
md"We see that these caibration targets would imply higher α and ρ but substantially lower ψ."

# ╔═╡ 3355e8ad-eb41-4fe9-921b-34c6565a24e7
md"## Estimation"

# ╔═╡ 8346a3aa-11a4-4f0d-ae90-c415517ce822
md"For the estimation we will first load the data accompanying the Schorfheide (2000) paper."

# ╔═╡ b76bcb2f-7162-4ed9-97e8-a5b65fc0a8bc
dat = CSV.read("/Users/thorekockerols/GitHub/MacroModellingWorkshop/FS2000_data.csv", DataFrame)

# ╔═╡ a781fdb3-2e3b-4a56-a415-7ce975569826
data = KeyedArray(Array(dat)',Variable = [:dy_obs,:π_obs],Time = 1:size(dat)[1])

# ╔═╡ 336f09b3-1f17-4e5d-bc27-360c289ca76b
md"Let's plot the two series:"

# ╔═╡ 9300ef51-7dfe-4aad-b810-6ee4e53fa519
plot(collect(data(:dy_obs)),labels = "dy_obs")

# ╔═╡ 9835f066-c1c0-411e-b1ea-28417f52ef48
plot(collect(data(:π_obs)),labels = "π_obs")

# ╔═╡ 1f8123e6-bbf4-4ff1-a6fe-b4d5ece2f551
md"Next we are going to combine the prior and kalman filter loglikelihood in one function returning the posterior:"

# ╔═╡ d0ae6d7b-ebac-42d3-a7df-ebd787a8f559
import Turing

# ╔═╡ 56922c39-22cb-4cf9-af97-e1eaa93ad367
md"We need to define the observabes and then let's check the kalman ilter loglikelihood with the current parameters."

# ╔═╡ 110d9fda-971d-4012-87cc-f99ad20af9e7
observables = [:dy_obs, :π_obs]

# ╔═╡ 4b37ec9a-c637-45f3-a935-fd44cc44317a
calculate_kalman_filter_loglikelihood(FS2000_calib, data, observables; parameters = FS2000_calib.parameter_values)

# ╔═╡ 60ea442c-e8b7-458a-8e5c-37bca32a4ec5
md"Next we define model to estimate including the priors for the parameters."

# ╔═╡ 88169ef8-9629-400c-a55d-0e47862d7b23
...
# ╔═╡ 1ea63a0e-d937-4709-98a6-02597ad6ec0a
loglikelihood = loglikelihood_function(data, FS2000_calib, observables)

# ╔═╡ 723ddae0-6e73-4f9b-9495-72e325c99146
import Turing: NUTS, sample, logpdf

# ╔═╡ 4163686c-79a6-414b-b8d6-e6ce1b467c4c
samps = sample(loglikelihood, NUTS(), 100, progress = true)

# ╔═╡ a782b237-1909-433b-9dd0-fd0d116ccd53
samps |>show_pluto

# ╔═╡ 3d8d52d0-ff00-4d83-9051-386fe48180d7
md"FYI: a very nice [visualization of different samplers](https://chi-feng.github.io/mcmc-demo/app.html#EfficientNUTS,banana)"

# ╔═╡ 36154b71-ae1a-41ef-95c6-44fda267909c
md"What is also possible is to include system priors on the standard deviation of some variables:"

# ╔═╡ 5477659b-c89e-4141-b4a9-ba9cb9c8a7ba
...
# ╔═╡ caf4b8a8-657b-4b3f-b762-83b6f9d9e894
...
manifest_format = "2.0"
project_hash = "55a267a6d9f1c359b59437c211286e382ed3ee21"
...
git-tree-sha1 = "5f17d23c034537b58a57e5aa25226e10486aae04"
...
git-tree-sha1 = "675662dc2bb464823a5c3a767168c0bf39953f89"
...
git-tree-sha1 = "4efb3f5f382947dc85ae8fa5990d66d6e2b6ff7c"
