...
# ╔═╡ 3af2cd01-72cc-46cf-88eb-9d6bf6070b70
using MacroModelling

# ╔═╡ d3817060-eaa5-11ed-2b3a-b5bc267079c2
show_pluto(x) = Text(sprint(show, "text/plain",x));

# ╔═╡ 72646d12-83da-46ad-9376-28d652f1cde1
md"
# Introduction
## Why MacroModelling.jl is useful
Let's start by giving you a very quick demonstration of what you can do with this package.

Let's take an RBC model with capital and labor, and the only nonlinear parts are the Cobb-Douglas production function, and CRRA utility:
"

# ╔═╡ 6f4103c6-8d2e-4976-997a-70491d7169dc
...
# ╔═╡ 0f506fe0-44b4-4e3c-b2bb-fb817f2eafe0
md"Note:
- greek symbols can be written as such. Julia supports all [unicode characters] (https://docs.julialang.org/en/v1/manual/unicode-input/): \sigma + <TAB> (after a brief pause)
- timing is explicit even when t=0: [0], [-1], [2], ...
- shocks are followed by [x]
- no declaration needed for variables, parameters, shocks

Next we declare the parameters and we want to match:
- government expenditure of GDP: 	20.38%
- investment share of GDP: 			25%
- capital to quarterly GDP ratio: 	10.4
- hours worked: 					1/3
by setting the parameters for government expenditure, capital depreciation rate, discount factor, and Frisch elasticity accordingly."

# ╔═╡ 79119a5f-0c14-4434-9f05-40862c344651
...
# ╔═╡ 36b7e6be-4e9e-48f3-8218-94d9a59e88bd
md"
Note:
- automatically sets up the system of equations including calibration equations and solves for the non stochastic steady state
- define parameters as functions of other parameters
- define parameters as a variable with a corresponding calibration equation to be satisfied

With this we have an object on which we can do our analysis. Let's start by getting the non stochastic steady state:
"

# ╔═╡ 14469fac-a473-429f-8b8d-af27ef48ec3f
get_SS(RBC_baseline) |> show_pluto

# ╔═╡ e6d769f8-6457-4003-92d9-5b80af7b23f0
md"
Note:
- derivatives of the non stochastic steady state wrt the parameters

Interpretation: increasing the investment to GDP ratio while adhering to all other calibration targets implies a higher Frsich elasticity.

Useful for calibration purposes and understanding the model.
"

# ╔═╡ 90b8014c-c64a-4b5f-9be8-fe561d2731d0
import StatsPlots

# ╔═╡ 76db46a1-69c6-46d7-8ab9-ac426f3d507f
plot_irf(RBC_baseline)[1]

# ╔═╡ 44d59647-cd67-4264-b815-1773161575e9
md"
Note:
- solves the model with first order perturbation and shows IRFs

We get two panels with max 9 subplots each showing the response of the endogenous variables to a positive 1 standard deviation shock of the two shocks in the model.

Left y-axis hsows the level, right y-axis shows the percent deviation, black horizontal line is the non stochastic steady state.

Next let's change the parameters and solve the model with a second order perturbation.
"

# ╔═╡ 160486ca-bb56-4371-98f3-9cbc47b561f5
plot_irf(RBC_baseline, parameters = :labor_share => 2/3, algorithm = :second_order)[1]

# ╔═╡ d86b76e7-cd94-4563-b34c-000faab37479
md"
Note:
- updates parameters and solves for new non stochastic steady state and second order perturbation in the background
- level axis changed reflecting the changed (stochastic) steady state

Plot shows the exact IRF, we can also get the generalised IRF:
"

# ╔═╡ 0ed11a34-562b-40ca-95e8-ac2acc0fc2e7
plot_irf(RBC_baseline, generalised_irf = true, algorithm = :second_order)[1]

# ╔═╡ 26dde272-6883-45a1-8614-5031e4ba940a
md"Next let's look at the theoretical standard deviation (1st order perturbation) of the endogenous variables."

# ╔═╡ 77382216-dfc9-4041-aeb5-c7dba80d1523
std(RBC_baseline) |> show_pluto

# ╔═╡ d069a0ac-1917-4672-ac7d-177caa291aac
md"
Note:
- derivatives of the theoretical standard deviation wrt parameters

E.g. increasing the labor share decreases the standard deviation of the interest rate

We can do the same for the stochastic steady state
"

# ╔═╡ 9316182e-04bf-4358-ba3b-3172e2767a1f
get_SSS(RBC_baseline, parameters = :labor_share => 1/3)|>show_pluto

# ╔═╡ 3da28310-c5dd-4bd0-b909-fb84c5edba5f
md"
Note:
- stochastic steady state (second order) is higher for GDP
- SSS of GDP is increasing in shock volatility
"
...
manifest_format = "2.0"
project_hash = "6496a6226eb48dd56c511611becde40fec235ebf"
