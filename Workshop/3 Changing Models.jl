...
# ╔═╡ f6254abe-e515-11ed-1ffe-2d40eb5dc567
using MacroModelling

# ╔═╡ e19d31ce-ee32-4925-beeb-fbb8ca6f470c
show_pluto(x) = Text(sprint(show, "text/plain",x));

# ╔═╡ 760c4baf-1804-4293-9a9e-89136a1b46dd
md"
...
Let's start by loading the model and geting some standard output to familiarise ourselves with the model:
"

# ╔═╡ 44a27b66-1054-45d9-b749-dc128da4f4d5
@model SW03 begin
...
# ╔═╡ 456023c5-1049-4656-8f7d-f642843e4f10
...
# ╔═╡ cf551b26-dd1b-4769-8eb9-6d7974a2e873
get_SS(SW03)|>show_pluto

# ╔═╡ 8b0cb87b-1c14-4082-b628-85ccd8054511
md"here there are no derivatives wrt parameters shown because it would be too many (54*39 = 2106) but if we want to know some specific derivatives we can still get them. Checkt help (?) to find out more about the relevant arguments of the function:"

# ╔═╡ dffeca73-6ef4-4754-8209-0df5db7b8a50
get_SS(SW03, parameter_derivatives = :alpha)|>show_pluto

# ╔═╡ 01288ad0-4408-4b4d-ba63-c846c1f1c4e9
import StatsPlots

# ╔═╡ 61c61a18-a130-4966-b8be-5f9f548fb6fb
plot_irf(SW03)[1]

# ╔═╡ c10183b7-3094-4408-9593-4f793459ba08
md"Thats a lot of shocks and variables. Let's pick only a few. Check the help (?) to find out how to restrict the plots to certain shocks and variables:"

# ╔═╡ 5e94501e-f3e0-4a55-9e8c-0608db9eabe9
plot_irf(SW03, shocks = :eta_R, variables = (:C,:r_k,:K,:I,:R,:W, :L, :Y, :pi))[1]

# ╔═╡ 868521a1-b469-4d76-bc45-47e125bc7ff1
md"Let's simulate the model"

# ╔═╡ b1b7f57e-5b8f-45f5-a6f0-9d9366efd1bb
plot_simulations(SW03, shocks = :eta_R, variables = (:C,:r_k,:K,:I,:R,:W, :L, :Y, :pi))[1]

# ╔═╡ 56f4747c-4e5d-4b82-af10-c2e01e4f5bfe
md"And finally let's get the theoretical standard deviations:"

# ╔═╡ 10656abb-16d7-42f8-86bd-b21abb5efb83
std(SW03) |> show_pluto

# ╔═╡ 3db31191-9286-496f-834a-51785141348a
md"## Implement a time-varying perceived inflation objective 
Currently the inflation objective is a standard AR(1) process and we want to include a response to lagged inflation: $...+\rho_\pi \cdot  \log \left(\frac{\pi_{[-1]}}{\pi_{[-5]}} \right)$
...
# ╔═╡ f3168810-2395-4acf-bb04-cd221962019d
...
# ╔═╡ 39c8dfc8-953b-482d-8e5c-b49577ce3ed7
...
# ╔═╡ 8f13a662-daa6-40e3-ad97-20cfca009beb
md"Generate some output to understand what these changes do. For example the theoretical model standard deviations will tell us about the influence of this change."

# ╔═╡ 7bb7e901-351e-44a2-822f-a1e4c458d44c
std(SW03_obj) |> show_pluto

# ╔═╡ 1b60e9d4-225d-4a0e-b337-657fd1d08ea7
md"
...
# ╔═╡ 00fa05cf-da14-4151-ace3-71f03c70a839
@model SW03_TR begin
   -q[0] + beta * ((1 - tau) * q[1] + epsilon_b[1] * (r_k[1] * z[1] - psi^-1 * r_k[ss] * (-1 + exp(psi * (-1 + z[1])))) * (C[1] - h * C[0])^(-sigma_c))
...
# ╔═╡ 29e52e71-c0bc-4719-98b8-642b89c05b8c
...
# ╔═╡ 97baae07-5c5f-4acd-a5f5-690381d20a4a
md"Let's check what this does to the model behaviour:"

# ╔═╡ 04050234-fcf6-4cc9-928e-fd1eec88e174
get_SS(SW03_TR)|>show_pluto

# ╔═╡ 8b4a8df9-74c3-415b-8638-ef733383649d
std(SW03_TR)|>show_pluto

# ╔═╡ 14426b8d-9a25-4799-a491-76b99d408b15
md"Variance decmoposition:"

# ╔═╡ 2bfa5e5c-0582-4db9-ad14-04e0b03dab68
get_variance_decomposition(SW03_TR) |> show_pluto

# ╔═╡ 07fb611b-18a2-4f84-ada7-129402d1320a
std(SW03_TR, parameter_derivatives = [:rho_pi,:rho,:r_pi,:r_Y,:r_Delta_pi,:r_Delta_y])([:pi,:Y,:R],:) |> show_pluto

# ╔═╡ 8c4d2e0c-1b36-41c6-a2ad-bb351f88f1eb
md"What happens to the model standard deviations if we switch off the feedback from past inflation?"

# ╔═╡ bc723b04-6bd9-472d-9a05-e570324f0893
std(SW03_TR, parameter_derivatives = [:rho_pi,:rho,:r_pi,:r_Y,:r_Delta_pi,:r_Delta_y], parameters = :rho_pi => 0)([:pi,:Y,:R],:) |> show_pluto

# ╔═╡ 95a16c78-4247-431c-8921-592872471d91
plot_irf(SW03_TR, parameters = :rho_pi  => 0,shocks=[:eta_pi,:eta_R])[4]

# ╔═╡ a10cba11-0c37-42bc-9509-8acdc6fa1157
md"Version with feedback:"

# ╔═╡ b3f54be1-4a4a-4713-b102-dcf8dfd12817
plot_irf(SW03_obj, shocks=[:eta_pi,:eta_R])[4]

# ╔═╡ b845c33c-3ce9-4a41-a9e7-b184e87f4b99
md"## Implement stochastic volatility
Let's go back to the textbook Gali new keyensian model:"

# ╔═╡ 420bea7e-8741-426e-ada9-86c44fd19057
...
# ╔═╡ 34cdacfa-fab0-42c9-ae33-ad51cbb152f3
...
# ╔═╡ 2bbd144e-c9f7-4e02-a5fd-d64f37af7965
md"Let's look at some standard output first:"

# ╔═╡ 7e9029b6-cdab-4a1c-96ac-85a04f58b583
get_SS(Gali_2015_chapter_3_nonlinear)|>show_pluto

# ╔═╡ f6b118fa-d18f-4656-bfb7-a0e122917bf6
std(Gali_2015_chapter_3_nonlinear, parameter_derivatives = [:phi_pi, :phi_y])|>show_pluto

# ╔═╡ e660c075-96dc-4fb6-b737-a6411baf345d
md"Since we will be implementing stochastic volatility, we need to solve the model with higher order perturbation methods in order to see the effects of it. Given the nonlinear solution method we can also look at the stochastic steady state."

# ╔═╡ e60f3c0c-d215-4c71-b9a2-5acd7dc71fac
get_SSS(Gali_2015_chapter_3_nonlinear, parameter_derivatives = [:σ_a, :σ_z, :σ_nu])|>show_pluto

# ╔═╡ e83355a6-72d4-47bd-8ae8-3f96d9f26352
md"Note that in order to get the derivatives we have to define the parameters for which we want to get the derivatievs. Check the help (?) for details on how to do this.

Last but not least let's compare it to the NSSS."

# ╔═╡ 6bc1ecb9-ee7f-4400-a281-824b42283b76
get_SS(Gali_2015_chapter_3_nonlinear)|>show_pluto

# ╔═╡ 6a71e7d5-87c6-45a4-a8a4-4a92ef8c3072
md"Now let's make the monetary policy shock have stochastic volatility.
...
# ╔═╡ 3c6dad4e-e6de-4858-bcea-336e0c58c647
...
# ╔═╡ d3059160-9450-4899-baf0-15c4f6925627
...
# ╔═╡ f2c1e40a-7f64-4987-a9ef-2a66d01e6432
md"Let's compare impulse responses for first, second, and third order"

# ╔═╡ 41882d47-a931-41d2-938b-df56fcb88839
plot_irf(Gali_2015_sv)[1]

# ╔═╡ 9ccc2c50-b774-463f-bcb3-0fdc22bc92b8
plot_irf(Gali_2015_sv, algorithm = :second_order)[1]

# ╔═╡ a25109c2-b459-4e83-aa3d-243d2e4c133b
plot_irf(Gali_2015_sv, algorithm = :third_order)[1]

# ╔═╡ c729f4ca-1d83-4fc6-8da7-0a62960adffb
md"Note how the response changes for labor and marginal cost at higher order.
...
# ╔═╡ 59ccf158-4450-4dc7-afda-8a0fe00f526e
plot_irf(Gali_2015_sv, algorithm = :second_order, shocks = :σ_eps_nu)[1]

# ╔═╡ 3bf94b2a-61ee-4987-a461-eb2763a72f80
plot_irf(Gali_2015_sv, algorithm = :third_order, shocks = :σ_eps_nu)[1]

# ╔═╡ 27dfcfaf-20e3-4aa1-8b5e-a5dd7676cc23
md"Finally we can look at the solution mapping of past states to present variables for the different perturbation orders:"

# ╔═╡ a6f04878-47b2-4ea1-be0b-1ac28a4ba021
plot_solution(Gali_2015_sv, :A, algorithm = [:first_order,:second_order,:third_order])[2]
...
manifest_format = "2.0"
project_hash = "d03744c3efccad9bc3dd31078a66de1a3fb1ad44"
...
git-tree-sha1 = "ad0e7e7ecb27b6914e3057cec2b6fe5f675a3563"
