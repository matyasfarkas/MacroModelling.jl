# Smets-Wouters (2007) with Heer-Linde-Theobald nonlinear Kimball aggregation
# and smooth ZLB enforcement via softplus.
#
# Identical to Smets_Wouters_2007_HLT.jl except:
#   - Taylor rule split into shadow rate r_tilde (unconstrained) and
#     actual rate r = 1 + log(1 + exp(kappa_zlb*(r_tilde - 1))) / kappa_zlb
#   - This ensures r >= 1.0 (zero lower bound on the net nominal rate)
#   - The SEP solver evaluates the smooth softplus natively, so ZLB
#     enforcement is automatic without OBC decomposition or auxiliary shocks.
#   - New observable robs_tilde = 100*(r_tilde - 1) tracks the shadow rate.
#
# At steady state the actual rate r[ss] matches the non-ZLB HLT model exactly
# because the NSSS solver adjusts r_tilde[ss] to satisfy the softplus mapping.
# The shadow rate is slightly below: softplus bias approx 0.13% at kappa=100.

@model Smets_Wouters_2007_HLT_zlb begin
	...
	# HLT Kimball price block (nonlinear Phillips with quasi-kinked demand)
	pdot[0] = (1 - cprobp) * (Pratio[0] / dp[0]) ^ ((-cfc) * (1 + curvP) / (cfc - 1)) +
	          cprobp * pdot[-1] * (dp[-1] / dp[0] * pinf[-1] ^ cindp * cpie ^ (1 - cindp) / pinf[0]) ^ ((-cfc) * (1 + curvP) / (cfc - 1))

	# HLT Kimball wage block
	...
	# Kimball price reset condition (quasi-kinked demand)
	1 = (1 - cprobp) * (Pratio[0] / dp[0]) ^ ((-(1 + curvp * (1 - cfc))) / (cfc - 1)) +
	    cprobp * (dp[-1] / dp[0] * pinf[-1] ^ cindp * cpie ^ (1 - cindp) / pinf[0]) ^ ((-(1 + curvp * (1 - cfc))) / (cfc - 1))
	...
	# ─── Taylor rule with smooth ZLB via softplus ───
	# r_tilde is the unconstrained Taylor rule (shadow rate)
	r_tilde[0] = r_tilde[ss] ^ (1 - crr) * r_tilde[-1] ^ crr * (pinf[0] / cpie) ^ ((1 - crr) * crpi) * (y[0] / yflex[0]) ^ ((1 - crr) * cry) * (y[0] / yflex[0] / (y[-1] / yflex[-1])) ^ crdy * ms[0]

	# Softplus clamping: r = 1 + log(1 + exp(kappa_zlb*(r_tilde - 1))) / kappa_zlb
	# This ensures r >= 1.0 (ZLB at gross rate = 1, i.e., 0% net rate)
	# When r_tilde >> 1: r ≈ r_tilde (unconstrained)
	# When r_tilde << 1: r ≈ 1.0 (ZLB binds)
	r[0] = 1.0 + log(1.0 + exp(kappa_zlb * (r_tilde[0] - 1.0))) / kappa_zlb
	...
	# Shadow rate observable (can go negative)
	robs_tilde[0] = 100 * (r_tilde[0] - 1)
...
@parameters Smets_Wouters_2007_HLT_zlb begin
	...
	# Softplus smoothing parameter for ZLB
	# Higher = closer to hard max(1, r_tilde)
	# kappa_zlb=100: bias at SS is ~0.12% (r=1.0217 vs r_tilde=1.0205)
	# kappa_zlb=200: bias at SS is ~0.06%
	kappa_zlb = 100.0
