# HLT model variant: Capital utilization cost a(z) LINEARIZED
# Purpose: counterfactual to isolate contribution of exponential a(z_t)
# Replace exp(cZcap*(z-1)) with its first-order Taylor expansion: 1 + cZcap*(z-1)
# This makes afunc linear in z and afuncD constant (= rk[ss]).
# All other nonlinearities (quadratic S, Kimball, etc.) remain active.

@model Smets_Wouters_2007_HLT_obc_lina begin
	...
	# a(z) LINEARIZED: first-order Taylor of exp(cZcap*(z-1)) around z=1
	rk[0] = afuncD[0]
	...
	# a(z) linearized: first-order Taylor expansions of the exponential utilization
	# afunc ~ rk[ss]*(z-1), afuncD ~ rk[ss]*(1 + cZcap*(z-1))
	afunc[0] = rk[ss] * (zcap[0] - 1)
	afuncD[0] = rk[ss] * (1 + cZcap * (zcap[0] - 1))
	...
	# Flex-price block (also with a(z) linearized)
	...
	afuncflex[0] = rkflex[ss] * (zcapflex[0] - 1)
	afuncDflex[0] = rkflex[ss] * (1 + cZcap * (zcapflex[0] - 1))
...
@parameters Smets_Wouters_2007_HLT_obc_lina simplify = false begin
	R_bar = 0.0
