# HLT model variant: BOTH S(x) and a(z) LINEARIZED
# Purpose: counterfactual to measure the COMBINED effect of the two main
# investment-block nonlinearities. The residual gap (if any) comes from
# Tobin's q interaction, Kimball aggregation, and other non-investment sources.

@model Smets_Wouters_2007_HLT_obc_noS_lina begin
	...
	# a(z) linearized: first-order Taylor of exp(cZcap*(z-1))
	rk[0] = afuncD[0]
	...
	# a(z) linearized: first-order Taylor expansions
	afunc[0] = rk[ss] * (zcap[0] - 1)
	afuncD[0] = rk[ss] * (1 + cZcap * (zcap[0] - 1))

	# S linearized to zero (0*inve[0] satisfies MacroModelling's 2-variable requirement)
	Sfunc[0] = 0 * inve[0]
	SfuncD[0] = 0 * inve[0]
	...
	# Flex-price block (both S and a(z) linearized)
	...
	Sfuncflex[0] = 0 * inveflex[0]
	SfuncDflex[0] = 0 * inveflex[0]
...
@parameters Smets_Wouters_2007_HLT_obc_noS_lina simplify = false begin
	R_bar = 0.0
