@model Smets_Wouters_2007_HLT_obc_smooth begin
	...
	# Taylor rule with smoothed ZLB (using sqrt-based smooth max)
	# Standard Taylor rule: log(r) = (1-crr)*log(r[ss]) + crr*log(r[-1]) + ...
	# With ZLB: log(r) = max(R_bar, taylor_rule_expression)
	#
	# Smooth approximation: smoothmax(a,b) = (a + b + sqrt((a - b)^2 + zlb_eps)) / 2
	# This has negligible bias O(zlb_eps / |a-b|) when the constraint doesn't bind,
	# unlike log-sum-exp which introduces O(1/kappa) bias that corrupts the NSSS.
	#
	# We split into two equations:
	#   1) taylor_log = unconstrained Taylor rule (in log space)
	#   2) log(r) = smoothmax(R_bar, taylor_log)

	taylor_log[0] = (1 - crr) * log(r[ss]) + crr * log(r[-1]) +
	                (1 - crr) * crpi * (log(pinf[0]) - log(cpie)) +
	                (1 - crr) * cry * (log(y[0]) - log(yflex[0])) +
	                crdy * (log(y[0]) - log(yflex[0]) - (log(y[-1]) - log(yflex[-1]))) +
	                log(ms[0])

	log(r[0]) = (R_bar + taylor_log[0] + sqrt((R_bar - taylor_log[0]) ^ 2 + zlb_eps)) / 2
...
@parameters Smets_Wouters_2007_HLT_obc_smooth simplify = false begin
	...
	zlb_eps = 1e-8  # Smoothing parameter for sqrt-based smooth max (smaller = tighter approximation)
	...
	# Bounds to help NSSS solver avoid log(0) domain errors
	# Note: r bound is slightly below 1.0 to allow the smooth max to work
	# at the ZLB (sqrt-based smooth max can produce log(r) slightly < R_bar)
	r > 0.99
	pinf > 0.5
	y > 0.1
	yflex > 0.1
