# HLT model variant: Investment adjustment cost S(x) LINEARIZED (set to zero)
# Purpose: counterfactual to isolate contribution of quadratic S(I_t/I_{t-1})
# All other nonlinearities (exponential utilization, Kimball, etc.) remain active.
# Sfunc = 0 and SfuncD = 0 in both sticky-price and flex-price blocks.
# csadjcost is kept at calibrated value so SCALE1_eqs (shock scaling) is unchanged.

@model Smets_Wouters_2007_HLT_obc_noS begin
	...
	# S linearized to zero (S(gamma) = 0, S'(gamma) = 0 at steady state)
	# 0*inve[0] is a dummy to satisfy MacroModelling's 2-variable requirement
	Sfunc[0] = 0 * inve[0]
	SfuncD[0] = 0 * inve[0]
	...
	# Flex-price block (also with S = 0)
...
@parameters Smets_Wouters_2007_HLT_obc_noS simplify = false begin
	R_bar = 0.0
