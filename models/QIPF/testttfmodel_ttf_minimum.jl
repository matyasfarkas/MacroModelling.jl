...
@model qipf begin
	...
	I[0] = psi_theta * THETA[0] + (1 - psi_i) * (SS_I + psi_pi * (PI_C[0] - SS_PI_C) + psi_pid * (PI_D[0] - SS_PI_D)) + psi_i * I[-1] + SS_E_I
	...
	I[0] * (1 - SS_TAU_F) = PI_C[1] * I_ST[0] * Q[1] / Q[0] / PI_C_ST[1] + I[0] * GAMMA[0] * B_F[0] / (SS_Y_D + SS_Y_M_ST)

	B_F[0] = ( - B[0]) - SS_B_P + SS_B_M

	B[0] = Y_D[0] + (I[-1] * (1 - omega_f) / PI_D[0] + Q[0] * PI_C[0] * I_ST[-1] * omega_f / PI_D[0] / PI_C_ST[0] / Q[-1]) * B[-1] + B[-1] * (1 - omega_b) * (IB[-1] - I[-1]) / PI_D[0] + (I[-1] / PI_D[0] - Q[0] * PI_C[0] * I_ST[-1] / PI_D[0] / PI_C_ST[0] / Q[-1]) * (SS_B_P * (omega_p - omega_f) - SS_B_M * (1 - omega_f)) + I[-1] * SS_TAU_F * (1 - cfm_nonfa) / PI_D[0] * ((1 - omega_f) * B_F[-1] + SS_B_P * (1 - omega_p)) + Y_M_ST[0] * Q[0] * GAM_CD[0] / GAM_CM_ST[0] - C[0] * GAM_CD[0] - SS_G * GAM_GD[0]
