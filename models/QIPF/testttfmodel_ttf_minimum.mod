...
GAM_MD_TF          $  {\Gamma_{MD,TF}}               $  (long_name='178. Ratio of import to production prices w/ tariff (home)') 
GAM_MD_ST_TFST     $  {\Gamma_{MD,TF^{\ast}}^{\ast}} $  (long_name='179. Ratio of import to production prices w/ tariff (foreign)')
TF                 $  {\tau_{TF}}                    $  (long_name='180. Tariff rate (home)                               ')  
TF_ST              $  {\tau_{TF^{\ast}}}             $  (long_name='181. Tariff rate (foreign)                            ') 
;//------------------------------------------------------------------------------------------------------------------------)          
...
*/
EPS_TF             $  {\varepsilon_{\tau_{TF}}}      $  (long_name=' 25. Tariff shock for foreign goods (home)            ')
//EPS_TFST           $  {\varepsilon_{\tau_{TF}^{\ast}}} $  (long_name=' 26. Tariff shock for home goods (foreign)            ')
;//------------------------------------------------------------------------------------------------------------------------)
...
iota_e          = 0.25;        //  3. Weight of exchange rate in wage indexation          |  \iota_{e}                   |
...
omega_f         = 0.5;         // 29. Domestic ownership share of financiers             |  \omega_{F}                  |
omega_p         = 0.5;         // 30. Domestic ownership share of exog. financiers       |  \omega_{P}                  |
...
s_gy            = 0.15;         // 68. Share of government consumption in GDP (home)      |  s_{gy}                      |
...
tau_p           = theta_p;      // 78. Subsidy to production                              |  \tau_{p}                    |
tau_w           = theta_w;      // 79. Subsidy to labor                                   |  \tau_{w}                    |
...
varrho_m        = 0.95;         // 83. Persistence of FXI                                 |  \varrho_{m}                 |
varrho_p        = 0.95;         // 84. Persistence of portfolio shock                     |  \varrho_{p}                 |
varrho_g        = 0.967;        // 85. Persistence of government spending (home)          |  \varrho_{G}                 |
...
zeta            = 1/2;        // 89. Relative size of home economy                      |  \zeta                       |
zeta_ST         = 1-zeta;       // 90. Relative size of foreign economy                   |  \zeta^{\ast}                |
rho_tau_tf      = 0.9;          // 91. Persistance of tariff (home)                       |  \rho_{\tau_{tf}}            |
// ----------------------------------------------------------------------------------------------------------------------|
...
SS_Q                = 1;       //S25.
SS_TAU_C            = 0.15;    //S26.
SS_TAU_C_ST         = 0.15;    //S27.
SS_TAU_F            = 0;       //S28.
SS_TAU_N            = 0.15;    //S29.
SS_TAU_N_ST         = 0.15;    //S30.
...
LAM=(C_TIL-varkappa*C_TIL(-1)-SS_C_TIL*SS_NU)^(-1/sigma)/(1+SS_TAU_C); // Equation 1

[name='Euler equation (optimality condition with respect to domestic bond holdings)']
LAM=beta*(SS_VARSIGMA/SS_VARSIGMA)*(IB/PI_C(+1))*LAM(+1); // Equation 2
...
Z_7=SS_VARSIGMA*chi_0*W_C^((1+theta_w)/theta_w*(1+chi))*N^(1+chi)/z7_corr +beta*xi_w*(PI_W(+1)/PI_C(+1))^(-(1+theta_w)/theta_w*(1+chi))*Z_7(+1); // Equation 4

[name='Recursive law of motion for Z_8']
Z_8=SS_VARSIGMA*LAM*(1-SS_TAU_N)*SS_UPSILON_W*W_C^((1+theta_w)/theta_w)*N/z8_corr +beta*xi_w*(PI_W(+1)/PI_C(+1))^(-1/theta_w)*Z_8(+1); // Equation 5
...
C_TIL=C+eta_0*SS_G; // Equation 9
                
[name='Expression for real marginal cost in terms of domestic goods prices']
MC_D=1/(1-alpha)*W_C*GAM_CD*(N/k)^(alpha)*(1/SS_Z^(1-alpha)); // Equation 10
...
Z_1=(1+psi)*(1+theta_p)/(1+psi+psi*theta_p)*SS_VARSIGMA*LAM*Y_D*VARTHETA^((1+theta_p)/theta_p*(1+psi))*MC_D/GAM_CD+beta*xi_p*(PI_P(+1)/PI_D(+1))^(-(1+theta_p)/theta_p*(1+psi))*Z_1(+1); // Equation 12

[name='Recursive law of motion for Z_2']
Z_2=SS_VARSIGMA*LAM*SS_UPSILON*Y_D/GAM_CD*VARTHETA^((1+theta_p)/theta_p*(1+psi))+beta*xi_p*(PI_P(+1)/PI_D(+1))^(-(1+psi+psi*theta_p)/theta_p)*Z_2(+1); // Equation 13

[name='Recursive law of motion for Z_3']
Z_3=(psi*theta_p)/(1+psi+psi*theta_p)*SS_VARSIGMA*LAM*SS_UPSILON*Y_D/GAM_CD+beta*xi_p*PI_P(+1)/PI_D(+1)*Z_3(+1); // Equation 14

[name='Price indexation scheme, domestic sales']
PI_P=SS_PI_D^(1-iota)*PI_D(-1)^iota; // Equation 15

[name='Aggregate production function']
P_AMP_D*Y_D+P_AMP_M_ST*Y_M_ST=k^(alpha)*(SS_Z*N)^(1-alpha); // Equation 16
...
Y_D=(1-omega_c)*GAM_CD^((1+rho_c)/rho_c)*C+(1-omega_g)*GAM_GD^((1+rho_g)/rho_g)*SS_G; // Equation 25
...
Z_M_1=(1+psi_m)*(1+theta_p)/(1+psi_m+psi_m*theta_p)*SS_VARSIGMA*LAM*Y_M_ST*VARTHETA_M^((1+theta_p)/theta_p*(1+psi_m))*MC_D/GAM_CD+beta*xi_m*(PI_PM_ST(+1)/PI_M_ST(+1))^(-(1+theta_p)/theta_p*(1+psi_m))*Z_M_1(+1); // Equation 27

[name='Recursive law of motion for Z_M_2']
Z_M_2=SS_VARSIGMA*LAM*SS_UPSILON_M_ST*Y_M_ST/GAM_CM_ST*Q*VARTHETA_M^((1+theta_p)/theta_p*(1+psi_m))+beta*xi_m*(PI_PM_ST(+1)/PI_M_ST(+1))^(-(1+psi_m+psi_m*theta_p)/theta_p)*Z_M_2(+1); // Equation 28

[name='Recursive law of motion for Z_M_3']
Z_M_3=(psi_m*theta_p)/(1+psi_m+psi_m*theta_p)*SS_VARSIGMA*LAM*SS_UPSILON_M_ST*Y_M_ST/GAM_CM_ST*Q+beta*xi_m*PI_PM_ST(+1)/PI_M_ST(+1)*Z_M_3(+1); // Equation 29
...
Y_M_ST=zeta_ST/zeta*(omega_c_ST*(GAM_CM_ST)^((1+rho_c_ST)/rho_c_ST)*C_ST+omega_g_ST*(GAM_GM_ST)^((1+rho_g_ST)/rho_g_ST)*SS_G_ST); // Equation 35

[name='Aggregate to domestically produced price ratio']
GAM_CD=((1-omega_c)+omega_c*(GAM_MD_TF)^(-1/rho_c))^(-rho_c); // Equation 36

[name='Government to domestically produced price ratio']
GAM_GD=((1-omega_g)+omega_g*(GAM_MD_TF)^(-1/rho_g))^(-rho_g); // Equation 37

[name='Foreign aggregate to imported price ratio']
GAM_CM_ST=((1-omega_c_ST)*GAM_MD_ST_TFST^(1/rho_c_ST)+omega_c_ST)^(-rho_c_ST); // Equation 38

[name='Foreign government to imported price ratio']
GAM_GM_ST=((1-omega_g_ST)*GAM_MD_ST_TFST^(1/rho_g_ST)+omega_g_ST)^(-rho_g_ST); // Equation 39
...
M_G = omega_g*(GAM_GM)^((1+rho_g)/rho_g)*SS_G; // Equation 43

[name='Determination of home monetary policy']
%I=max(elb,psi_theta*THETA +(1-psi_i)*(SS_I+psi_pi*(PI_C-SS_PI_C)+psi_pid*(PI_D-SS_PI_D)+psi_x*(Y/Y_POT-1))+psi_i*I(-1)+SS_E_I); // Equation 44
I=psi_theta*THETA +(1-psi_i)*(SS_I+psi_pi*(PI_C-SS_PI_C)+psi_pid*(PI_D-SS_PI_D))+psi_i*I(-1)+SS_E_I; // Equation 44
%I=psi_theta*THETA +(1-psi_i)*(SS_I+psi_pi*(PI_C-SS_PI_C)+psi_pid*(PI_D-SS_PI_D)+psi_x*(Y/SS_Y-1))+psi_i*I(-1)+SS_E_I; // Equation 44
...
LAM_ST=(C_TIL_ST-varkappa_ST*C_TIL_ST(-1)-SS_C_TIL_ST*SS_NU_ST)^(-1/sigma)/(1+SS_TAU_C_ST); // Equation 45

[name='Foreign Euler equation (optimality condition with respect to domestic bond holdings)']
LAM_ST=beta_ST*(SS_VARSIGMA_ST/SS_VARSIGMA_ST)*I_ST/PI_C_ST(+1)*LAM_ST(+1); // Equation 46
...
Z_7_ST=SS_VARSIGMA_ST*chi_0_ST*W_C_ST^((1+theta_w)/theta_w*(1+chi))*N_ST^(1+chi)/z7_corr_ST +beta*xi_w_ST*(PI_W_ST(+1)/PI_C_ST(+1))^(-(1+theta_w)/theta_w*(1+chi))*Z_7_ST(+1); // Equation 48

[name='Foreign recursive law of motion for Z_8_ST']
Z_8_ST=SS_VARSIGMA_ST*LAM_ST*(1-SS_TAU_N_ST)*SS_UPSILON_W_ST*W_C_ST^((1+theta_w)/theta_w)*N_ST /z8_corr_ST +beta*xi_w_ST*(PI_W_ST(+1)/PI_C_ST(+1))^(-1/theta_w)*Z_8_ST(+1); // Equation 49
...
C_TIL_ST=C_ST+eta_0*SS_G_ST; // Equation 53

[name='Expression for foreign real marginal cost in terms of foreign good prices']
MC_D_ST=1/(1-alpha)*W_C_ST*GAM_CD_ST*(N_ST/k_ST)^(alpha)*(1/SS_Z_ST^(1-alpha)); // Equation 54
...
Z_1_ST=(1+psi_ST)*(1+theta_p)/(1+psi_ST+psi_ST*theta_p)*SS_VARSIGMA_ST*LAM_ST*Y_D_ST*VARTHETA_ST^((1+theta_p)/theta_p*(1+psi_ST))*MC_D_ST/GAM_CD_ST+beta_ST*xi_p_ST*(PI_P_ST(+1)/PI_D_ST(+1))^(-(1+theta_p)/theta_p*(1+psi_ST))*Z_1_ST(+1); // Equation 56

[name='Foreign recursive law of motion for Z_2_ST']
Z_2_ST=SS_VARSIGMA_ST*LAM_ST*SS_UPSILON_ST*Y_D_ST/GAM_CD_ST*VARTHETA_ST^((1+theta_p)/theta_p*(1+psi_ST))+beta_ST*xi_p_ST*(PI_P_ST(+1)/PI_D_ST(+1))^(-(1+psi_ST+psi_ST*theta_p)/theta_p)*Z_2_ST(+1); // Equation 57

[name='Foreign recursive law of motion for Z_3_ST']
Z_3_ST=(psi_ST*theta_p)/(1+psi_ST+psi_ST*theta_p)*SS_VARSIGMA_ST*LAM_ST*SS_UPSILON_ST*Y_D_ST/GAM_CD_ST+beta_ST*xi_p_ST*PI_P_ST(+1)/PI_D_ST(+1)*Z_3_ST(+1); // Equation 58
...
P_AMP_D_ST*Y_D_ST+P_AMP_M*Y_M=k_ST^(alpha)*(SS_Z_ST*N_ST)^(1-alpha); // Equation 60
...
Y_D_ST=(1-omega_c_ST)*GAM_CD_ST^((1+rho_c)/rho_c)*C_ST+(1-omega_g_ST)*GAM_GD_ST^((1+rho_g)/rho_g)*SS_G_ST; // Equation 69
...
Z_M_1_ST=(1+psi_m_ST)*(1+theta_p)/(1+psi_m_ST+psi_m_ST*theta_p)*SS_VARSIGMA_ST*LAM_ST*Y_M*VARTHETA_M_ST^((1+theta_p)/theta_p*(1+psi_m_ST))*MC_D_ST/GAM_CD_ST+beta_ST*xi_m_ST*(PI_PM(+1)/PI_M(+1))^(-(1+theta_p)/theta_p*(1+psi_m_ST))*Z_M_1_ST(+1); // Equation 71

[name='Foreign recursive law of motion for Z_M_2_ST']
Z_M_2_ST=SS_VARSIGMA_ST*LAM_ST*SS_UPSILON_M*Y_M/GAM_CM/Q*VARTHETA_M_ST^((1+theta_p)/theta_p*(1+psi_m_ST))+beta_ST*xi_m_ST*(PI_PM(+1)/PI_M(+1))^(-(1+psi_m_ST+psi_m_ST*theta_p)/theta_p)*Z_M_2_ST(+1); // Equation 72

[name='Foreign recursive law of motion for Z_M_3_ST']
Z_M_3_ST=(psi_m_ST*theta_p)/(1+psi_m_ST+psi_m_ST*theta_p)*SS_VARSIGMA_ST*LAM_ST*SS_UPSILON_M*Y_M/GAM_CM/Q+beta_ST*xi_m_ST*PI_PM(+1)/PI_M(+1)*Z_M_3_ST(+1); // Equation 73
...
Y_M=zeta/zeta_ST*(omega_c*(GAM_CM)^((1+rho_c)/rho_c)*C+omega_g*(GAM_GM)^((1+rho_g)/rho_g)*SS_G); // Equation 79

[name='Foreign aggregate to domestically produced price ratio']
GAM_CD_ST=((1-omega_c_ST)+omega_c_ST*(GAM_MD_ST_TFST)^(-1/rho_c_ST))^(-rho_c_ST); // Equation 80

[name='Foreign government to domestically produced price ratio']
GAM_GD_ST=((1-omega_g_ST)+omega_g_ST*(GAM_MD_ST_TFST)^(-1/rho_g_ST))^(-rho_g_ST); // Equation 81

[name='Foreign aggregate to imported price ratio']
GAM_CM=((1-omega_c)*GAM_MD_TF^(1/rho_c)+omega_c)^(-rho_c); // Equation 82

[name='Foreign government to imported price ratio']
GAM_GM=((1-omega_g)*GAM_MD_TF^(1/rho_g)+omega_g)^(-rho_g); // Equation 83
...
M_G_ST = omega_g_ST*(GAM_GM_ST)^((1+rho_g_ST)/rho_g_ST)*SS_G_ST; // Equation 87

[name='Determination of foreign monetary policy']
%I_ST=max(elb_ST,(1-psi_i_ST)*(SS_I_ST+psi_pi_ST*(PI_C_ST-SS_PI_C_ST)+psi_pid_ST*(PI_D_ST-SS_PI_D_ST)+psi_x_ST*(Y_ST/Y_ST_POT-1))+psi_i_ST*I_ST(-1)+SS_E_I_ST); // Equation 88
I_ST=(1-psi_i_ST)*(SS_I_ST+psi_pi_ST*(PI_C_ST-SS_PI_C_ST)+psi_pid_ST*(PI_D_ST-SS_PI_D_ST))+psi_i_ST*I_ST(-1)+SS_E_I_ST; // Equation 88
%I_ST=(1-psi_i_ST)*(SS_I_ST+psi_pi_ST*(PI_C_ST-SS_PI_C_ST)+psi_pid_ST*(PI_D_ST-SS_PI_D_ST)+psi_x_ST*(Y_ST/SS_Y_ST-1))+psi_i_ST*I_ST(-1)+SS_E_I_ST; // Equation 88

//-------------------------------------------------------------------------------------------------------------------------)
// Additional Equations
//-------------------------------------------------------------------------------------------------------------------------)
[name='UIP condition']
(1-SS_TAU_F)*I = I_ST*Q(+1)/Q*PI_C(+1)/PI_C_ST(+1) + GAMMA*I*B_F/(SS_Y_D+SS_Y_M_ST); // Equation 89

[name='Domestic bond market clearing']
B_F = -B-SS_B_P+SS_B_M; // Equation 90

[name='Net foreign assets (modified to account for cfm_nonfa)']
B = ((1-omega_f)*I(-1)/PI_D+omega_f*I_ST(-1)/PI_D*PI_C/PI_C_ST*Q/Q(-1))*B(-1) + (1-omega_b)*(IB(-1)-I(-1))/PI_D*B(-1) + (I(-1)/PI_D-I_ST(-1)/PI_D*PI_C/PI_C_ST*Q/Q(-1))*((omega_p-omega_f)*SS_B_P-(1-omega_f)*SS_B_M) + (1-cfm_nonfa)*SS_TAU_F*I(-1)/PI_D*((1-omega_f)*B_F(-1)+(1-omega_p)*SS_B_P) + Y_D + GAM_CD/GAM_CM_ST*Q*Y_M_ST - GAM_CD*C - GAM_GD*SS_G; // Equation 91
...
//U=SS_VARSIGMA*(log(C_TIL-varkappa*C_TIL(-1)-SS_C_TIL*NU) - chi_0*W_AMP_U*N^(1+chi)/(1+chi)) + beta*U(+1); // Equation 98

//[name='Average foreign lifetime utility for sigma = 1']
//U_ST=SS_VARSIGMA_ST*(log(C_TIL_ST-varkappa_ST*C_TIL_ST(-1)-SS_C_TIL_ST*SS_NU_ST) - chi_0_ST*W_AMP_U_ST*N_ST^(1+chi)/(1+chi)) + beta_ST*U_ST(+1); // Equation 99
...
LAM_ST_POT=(C_TIL_ST_POT-varkappa_ST*C_TIL_ST_POT(-1)-SS_C_TIL_ST*NU_ST)^(-1/sigma)/(1+SS_TAU_C_ST); // Equation 115
...
(1-SS_TAU_N_ST)*W_C_ST_POT=(1+theta_w)/(1+tau_w)*chi_0_ST*N_ST_POT^(chi)/LAM_ST_POT; // Equation 117
...
(1+tau_p)/(1+theta_p)=(1/(SS_Z_ST)^(1-alpha))*1/(1-alpha)*W_C_ST_POT*GAM_CD_ST_POT*(N_ST_POT/k_ST)^(alpha); // Equation 119

[name='FP foreign aggregate production function']
Y_D_ST_POT+Y_M_POT=k_ST^(alpha)*(SS_Z_ST*N_ST_POT)^(1-alpha); // Equation 120
...
TAU_C_ST-SS_TAU_C_ST=rho_tau_c_ST*(TAU_C_ST(-1)-SS_TAU_C_ST)+EPS_TAU_C; // Equation 151
...
//-------------------------------------------------------------------------------------------------------------------------)
// Tariffs: Equations, relative prices stochastic volatility 
//-------------------------------------------------------------------------------------------------------------------------)

[name='Ratio of import to production prices w/ tariff (home)']
GAM_MD_TF = GAM_MD*exp(TF); // Equation 178

[name='Ratio of import to production prices w/ tariff (foreign)']
GAM_MD_ST_TFST = GAM_MD_ST*exp(TF_ST); // Equation 179

[name='Tariff on foreign goods levied by home (home)']
//TF=0.9*TF(-1) +  exp(SV_EPS_TF(-1))*EPS_TF; // Equation 179
//TF=0.8*TF(-1) +  exp(SV_EPS_TF(-1))*EPS_TF; // Equation 179
//TF=0.99*TF(-1) + EPS_TFST; // Equation 180
//TF=TF_ST; // Equation 179
//TF=0;
TF=0.9*TF(-1) + EPS_TF; // Equation 179

[name='Tariff on home goods levied by foreign (foreign)']
//TF_ST=0.8*TF_ST(-1) +  exp(SV_EPS_TF(-1))*EPS_TF; // Equation 179
//TF_ST=0; // Equation 179
//TF_ST=TF; // Equation 179
//TF_ST=0.9*TF_ST(-1) + EPS_TFST; // Equation 181
//TF_ST=0.99*TF_ST(-1) + EPS_TFST; // Equation 181
TF_ST=TF;
...
Y_CAL_ST            = 0;                          //177. 
//-------------------------------------------------------------------------------------------------------------------------)
// Tariffs related variables
//-------------------------------------------------------------------------------------------------------------------------)
GAM_MD_TF           = SS_GAM_MD;                  //178.
GAM_MD_ST_TFST      = SS_GAM_MD_ST;               //179.
TF                  = 0;                          //180.
TF_ST               = 0;                          //181.
end;
...
shocks;
//var      EPS_B_P; stderr 0.3;
//var      EPS_TAU_C; stderr 0.05;
var      EPS_TF; stderr 0.1;
//var      EPS_TFST; stderr 0.25;
//periods  1;
//values   0.3;
end;

//stoch_simul(order=1,irf=40) C C_ST Y Y_ST Y_D Y_D_ST Y_M Y_M_ST Q PI_C PI_C_ST B;
stoch_simul(order=4,pruning,k_order_solver,irf=40,nomoments) C C_ST Y Y_ST Y_D Y_D_ST Y_M Y_M_ST Q PI_C PI_C_ST B;

//perfect_foresight_setup(Periods=100);
//perfect_foresight_solver(stack_solve_algo=0,maxit=25);

//Uncomment to generate LaTeX documentation
