@model Smets_Wouters_2007_HLT_obc_smooth begin
	...
	# Taylor rule with smoothed ZLB (using log-sum-exp approximation)
	# Standard Taylor rule: log(r) = (1-crr)*log(r[ss]) + crr*log(r[-1]) + ...
	# With ZLB: log(r) = max(R_bar, taylor_rule_expression)
	# Smoothed: log(r) = (1/κ) * log(exp(κ*R_bar) + exp(κ*taylor_rule_expression))
	# where κ = zlb_kappa (smoothing parameter, higher = closer to hard max)

	log(r[0]) = (1 / zlb_kappa) * log(
		exp(zlb_kappa * R_bar) +
		exp(zlb_kappa * ((1 - crr) * log(r[ss]) + crr * log(r[-1]) +
...
@parameters Smets_Wouters_2007_HLT_obc_smooth simplify = false begin
	...
	zlb_kappa = 50.0  # Smoothing parameter for ZLB (higher = closer to hard max)
