...
@model QMIPF_stoch begin
	...
	PI_W[0] = PI_W[-1] ^ (1 - nu) * SS_PI_C ^ (nu * (1 - iota_w)) * (PI_C[-1] ^ (1 - iota_e) * (SS_PI_C_ST * PI_C[-1] * Q[-1] / AUX_ENDO_LAG_98_1[-1] / PI_C_ST[-1]) ^ iota_e) ^ (nu * iota_w)
	...
	GAM_CM_ST[0] = (omega_c_ST + (1 - omega_c_ST) * GAM_MD_ST[0] ^ (1 / rho_c_ST)) ^ (-rho_c_ST)

	GAM_GM_ST[0] = (omega_g_ST + (1 - omega_g_ST) * GAM_MD_ST[0] ^ (1 / rho_g_ST)) ^ (-rho_g_ST)
	...
	I[0] = max(elb,(1 - psi_i) * (SS_I + psi_pi * (PI_C[0] - SS_PI_C) + psi_pid * (PI_D[0] - SS_PI_D) + psi_x * (Y[0] / Y_POT[0] - 1)) + psi_i * I[-1] + E_I[0]) #psi_theta * THETA[0] 
	...
	I_ST[0] = max(elb_ST,(1 - psi_i_ST) * (SS_I_ST + psi_pi_ST * (PI_C_ST[0] - SS_PI_C_ST) + psi_pid_ST * (PI_D_ST[0] - SS_PI_D_ST) + psi_x_ST * (Y_ST[0] / Y_ST_POT[0] - 1)) + psi_i_ST * I_ST[-1] + E_I_ST[0])
	...
	U_ST[0] = VARSIGMA_ST[0] * (log(C_TIL_ST[0] - varkappa_ST * C_TIL_ST[-1] - SS_C_TIL_ST * NU_ST[0]) - N_ST[0] ^ (1 + chi) * chi_0_ST * W_AMP_U_ST[0] / (1 + chi)) + beta_ST * U_ST[1]
	...
	(1 + tau_p) / (1 + theta_p) = W_C_POT[0] * 1 / Z[0] ^ (1 - alpha) / (1 - alpha) * GAM_CD_POT[0] * (N_POT[0] / k) ^ alpha

	Y_D_POT[0] + Y_M_ST_POT[0] = k ^ alpha * (Z[0] * N_POT[0]) ^ (1 - alpha)

	Y_D_POT[0] = C_POT[0] * (1 - omega_c) * GAM_CD_POT[0] ^ ((1 + rho_c) / rho_c) + G[0] * (1 - omega_g) * GAM_GD_POT[0] ^ ((1 + rho_g) / rho_g)

	Y_M_ST_POT[0] = zeta_ST / zeta * (omega_c_ST * GAM_CM_ST_POT[0] ^ ((1 + rho_c_ST) / rho_c_ST) * C_ST_POT[0] + G_ST[0] * omega_g_ST * GAM_GM_ST_POT[0] ^ ((1 + rho_g_ST) / rho_g_ST))

	GAM_CD_POT[0] = (1 - omega_c + omega_c * GAM_MD_POT[0] ^ (( - 1) / rho_c)) ^ (-rho_c)

	GAM_GD_POT[0] = (1 - omega_g + omega_g * GAM_MD_POT[0] ^ (( - 1) / rho_g)) ^ (-rho_g)

	GAM_CM_ST_POT[0] = (omega_c_ST + (1 - omega_c_ST) * GAM_MD_POT[0] ^ (( - 1) / rho_c_ST)) ^ (-rho_c_ST)

	GAM_GM_ST_POT[0] = (omega_g_ST + (1 - omega_g_ST) * GAM_MD_POT[0] ^ (( - 1) / rho_g_ST)) ^ (-rho_g_ST)

	PI_C_POT[0] = GAM_CD_POT[0] / GAM_CD_POT[-1] * PI_D_POT[0]

	M_C_POT[0] = C_POT[0] * omega_c * GAM_CM_POT[0] ^ ((1 + rho_c) / rho_c)

	M_G_POT[0] = G[0] * omega_g * GAM_GM_POT[0] ^ ((1 + rho_g) / rho_g)

	LAM_ST_POT[0] = (C_TIL_ST_POT[0] - varkappa_ST * C_TIL_ST_POT[-1] - SS_C_TIL_ST * NU_ST[0]) ^ (( - 1) / sigma) / (1 + TAU_C_ST[0])

	LAM_ST_POT[0] = beta_ST * VARSIGMA_ST[1] / VARSIGMA_ST[0] * I_ST_POT[0] / PI_C_ST_POT[1] * LAM_ST_POT[1]

	(1 - TAU_N_ST[0]) * W_C_ST_POT[0] = chi_0_ST * (1 + theta_w) / (1 + tau_w) * N_ST_POT[0] ^ chi / LAM_ST_POT[0]

	C_TIL_ST_POT[0] = eta_0 * G_ST[0] + C_ST_POT[0]

	(1 + tau_p) / (1 + theta_p) = W_C_ST_POT[0] * 1 / Z_ST[0] ^ (1 - alpha) / (1 - alpha) * GAM_CD_ST_POT[0] * (N_ST_POT[0] / k_ST) ^ alpha

	Y_D_ST_POT[0] + Y_M_POT[0] = k_ST ^ alpha * (Z_ST[0] * N_ST_POT[0]) ^ (1 - alpha)

	Y_D_ST_POT[0] = C_ST_POT[0] * (1 - omega_c_ST) * GAM_CD_ST_POT[0] ^ ((1 + rho_c) / rho_c) + G_ST[0] * (1 - omega_g_ST) * GAM_GD_ST_POT[0] ^ ((1 + rho_g) / rho_g)

	Y_M_POT[0] = zeta / zeta_ST * (C_POT[0] * omega_c * GAM_CM_POT[0] ^ ((1 + rho_c) / rho_c) + G[0] * omega_g * GAM_GM_POT[0] ^ ((1 + rho_g) / rho_g))

	GAM_CD_ST_POT[0] = (1 - omega_c_ST + omega_c_ST * GAM_MD_POT[0] ^ (1 / rho_c_ST)) ^ (-rho_c_ST)

	GAM_GD_ST_POT[0] = (1 - omega_g_ST + omega_g_ST * GAM_MD_POT[0] ^ (1 / rho_g_ST)) ^ (-rho_g_ST)

	GAM_CM_POT[0] = (omega_c + (1 - omega_c) * GAM_MD_POT[0] ^ (1 / rho_c)) ^ (-rho_c)

	GAM_GM_POT[0] = (omega_g + (1 - omega_g) * GAM_MD_POT[0] ^ (1 / rho_g)) ^ (-rho_g)

	PI_C_ST_POT[0] = GAM_CD_ST_POT[0] / GAM_CD_ST_POT[-1] * PI_D_ST_POT[0]

	M_C_ST_POT[0] = omega_c_ST * GAM_CM_ST_POT[0] ^ ((1 + rho_c_ST) / rho_c_ST) * C_ST_POT[0]

	M_G_ST_POT[0] = G_ST[0] * omega_g_ST * GAM_GM_ST_POT[0] ^ ((1 + rho_g_ST) / rho_g_ST)

	I_POT[0] * (1 - SS_TAU_F) = PI_C_POT[1] * I_ST_POT[0] * Q_POT[1] / Q_POT[0] / PI_C_ST_POT[1] + I_POT[0] * gamma_0 * var_e ^ gamma_1 * B_F_POT[0] / (SS_Y_D + SS_Y_M_ST)

	B_F_POT[0] = ( - B_POT[0]) - SS_B_P + SS_B_M

	B_POT[0] = Y_D_POT[0] + ((1 - omega_f) * I_POT[-1] / PI_D_POT[0] + Q_POT[0] * PI_C_POT[0] * omega_f * I_ST_POT[-1] / PI_D_POT[0] / PI_C_ST_POT[0] / Q_POT[-1]) * B_POT[-1] + (I_POT[-1] / PI_D_POT[0] - Q_POT[0] * PI_C_POT[0] * I_ST_POT[-1] / PI_D_POT[0] / PI_C_ST_POT[0] / Q_POT[-1]) * (SS_B_P * (omega_f - omega_p) - (1 - omega_f) * SS_B_M) + I_POT[-1] * (1 - cfm_nonfa) * SS_TAU_F / PI_D_POT[0] * ((1 - omega_f) * B_F_POT[-1] + (1 - omega_p) * SS_B_P) + Y_M_ST_POT[0] * Q_POT[0] * GAM_CD_POT[0] / GAM_CM_ST_POT[0] - C_POT[0] * GAM_CD_POT[0] - G[0] * GAM_GD_POT[0]

	# GAMMA_POT[0] = gamma_0 * var_e ^ gamma_1

	GAM_MD_POT[0] = GAM_CD_POT[0] * Q_POT[0] / GAM_CD_ST_POT[0]

	I_POT[0] = (1 - psi_i) * (SS_I + psi_pi * (PI_C_POT[0] - SS_PI_C) + psi_pid * (PI_D_POT[0] - SS_PI_D)) + psi_i * I_POT[-1]

	I_ST_POT[0] = (1 - psi_i_ST) * (SS_I_ST + psi_pi_ST * (PI_C_ST_POT[0] - SS_PI_C_ST) + psi_pid_ST * (PI_D_ST_POT[0] - SS_PI_D_ST)) + psi_i_ST * I_ST_POT[-1]

	Y_POT[0] = Y_D_POT[0] + Y_M_ST_POT[0]

	Y_ST_POT[0] = Y_D_ST_POT[0] + Y_M_POT[0]

	VARSIGMA[0] - SS_VARSIGMA = rho_varsigma * (VARSIGMA[-1] - SS_VARSIGMA) + sigma_EPS_VARSIGMA * EPS_VARSIGMA[x] + spill_varsigma * sigma_EPS_VARSIGMA_ST * EPS_VARSIGMA_ST[x]
	...
	(Z[0] - SS_Z) / SS_Z = rho_z * (Z[-1] - SS_Z) / SS_Z + sigma_EPS_Z * EPS_Z[x] + spill_z * sigma_EPS_Z_ST * EPS_Z_ST[x]
	...
	log((1 + tau_w) / UPSILON_W[0]) = rho_upsilon_w * log((1 + tau_w) / UPSILON_W[-1]) + sigma_EPS_UPSILON_W * EPS_UPSILON_W[x] + sigma_EPS_UPSILON_W_ST * spill_upsilon_w * EPS_UPSILON_W_ST[x]

	E_I[0] = rho_e_i * E_I[-1] + EPS_I[x] + spill_i * EPS_I_ST[x]

	VARSIGMA_ST[0] - SS_VARSIGMA_ST = sigma_EPS_VARSIGMA_ST *EPS_VARSIGMA_ST[x] + rho_varsigma_ST * (VARSIGMA_ST[-1] - SS_VARSIGMA_ST)

	NU_ST[0] - SS_NU_ST = rho_nu_ST * (NU_ST[-1] - SS_NU_ST) + EPS_NU_ST[x]

	TAU_C_ST[0] - SS_TAU_C_ST = rho_tau_c_ST * (TAU_C_ST[-1] - SS_TAU_C_ST) + EPS_TAU_C_ST[x]
	...
	(Z_ST[0] - SS_Z_ST) / SS_Z_ST =sigma_EPS_Z_ST * EPS_Z_ST[x] + rho_z_ST * (Z_ST[-1] - SS_Z_ST) / SS_Z_ST
	...
	log((1 + tau_w) / UPSILON_W_ST[0]) =sigma_EPS_UPSILON_W_ST * EPS_UPSILON_W_ST[x] + rho_upsilon_w_ST * log((1 + tau_w) / UPSILON_W_ST[-1])

	E_I_ST[0] = EPS_I_ST[x] + rho_e_i_ST * E_I_ST[-1]

	(B_P[0] - SS_B_P) / SS_Y = varrho_p * (B_P[-1] - SS_B_P) / SS_Y + sigma_EPS_B_P *  EPS_B_P[x]

	(B_M[0] - SS_B_M) / SS_Y = (B_P[0] - SS_B_P) * ppsim_bp / SS_Y + EPS_B_M[x] #- THETA[0] * ppsim_theta 

	TAU_F[0] - SS_TAU_F = ( - ppsif_b) * (B[0] - SS_B) / SS_Y + EPS_TAU_F[x]#max(0.000,( - ppsif_b) * (B[0] - SS_B) / SS_Y) + EPS_TAU_F[x]

	D4Q_CAL[0] = 100 * log(Q[0] / AUX_ENDO_LAG_98_3[-1])

	D4W_CAL[0] = 100 * log(W_C[0] / AUX_ENDO_LAG_125_3[-1])

	D4W_CAL_ST[0] = 100 * log(W_C_ST[0] / AUX_ENDO_LAG_127_3[-1])

	D4Y_CAL[0] = 100 * log(Y[0] / AUX_ENDO_LAG_133_3[-1])

	D4Y_CAL_ST[0] = 100 * log(Y_ST[0] / AUX_ENDO_LAG_145_3[-1])

	I_CAL[0] = 400 * (I[0] - SS_I)

	I_CAL_ST[0] = 400 * (I_ST[0] - SS_I_ST)

	PI_CAL[0] = (PI_D[0] - SS_PI_D) * 400

	PI_CAL_ST[0] = (PI_C_ST[0] - SS_PI_C_ST) * 400

	Q_CAL[0] = 100 * log(Q[0] / SS_Q)

	TB_CAL[0] = 100 * (Y_D[0] + Y_M_ST[0] * Q[0] * GAM_CD[0] / GAM_CM_ST[0] - C[0] * GAM_CD[0] - G[0] * GAM_GD[0]) / (Y_D[0] + Y_M_ST[0] * Q[0] * GAM_CD[0] / GAM_CM_ST[0]) - 100 + 100 * (SS_G + SS_C) / SS_Y

	UIP_CAL[0] = 100 * (I[0] * gamma_0 * var_e ^ gamma_1 * B_F[0] - SS_I * SS_GAMMA * SS_B_F) / (SS_Y_D + SS_Y_M_ST)

	W_CAL[0] = 100 * log(W_C[0] / SS_W_C)

	W_CAL_ST[0] = 100 * log(W_C_ST[0] / SS_W_C_ST)

	Y_CAL[0] = 100 * log(Y[0] / SS_Y)

	Y_CAL_ST[0] = 100 * log(Y_ST[0] / SS_Y_ST)

	AUX_ENDO_LAG_98_1[0] = Q[-1]

	AUX_ENDO_LAG_98_2[0] = AUX_ENDO_LAG_98_1[-1]

	AUX_ENDO_LAG_98_3[0] = AUX_ENDO_LAG_98_2[-1]

	AUX_ENDO_LAG_125_1[0] = W_C[-1]

	AUX_ENDO_LAG_125_2[0] = AUX_ENDO_LAG_125_1[-1]

	AUX_ENDO_LAG_125_3[0] = AUX_ENDO_LAG_125_2[-1]

	AUX_ENDO_LAG_127_1[0] = W_C_ST[-1]

	AUX_ENDO_LAG_127_2[0] = AUX_ENDO_LAG_127_1[-1]

	AUX_ENDO_LAG_127_3[0] = AUX_ENDO_LAG_127_2[-1]

	AUX_ENDO_LAG_133_1[0] = Y[-1]

	AUX_ENDO_LAG_133_2[0] = AUX_ENDO_LAG_133_1[-1]

	AUX_ENDO_LAG_133_3[0] = AUX_ENDO_LAG_133_2[-1]

	AUX_ENDO_LAG_145_1[0] = Y_ST[-1]

	AUX_ENDO_LAG_145_2[0] = AUX_ENDO_LAG_145_1[-1]

	AUX_ENDO_LAG_145_3[0] = AUX_ENDO_LAG_145_2[-1]

end


@parameters QMIPF_stoch begin
	sigma_EPS_VARSIGMA_ST  = 0.027621
	sigma_EPS_Z_ST = 0.035034
	sigma_EPS_VARSIGMA = 0.02971
	sigma_EPS_Z = 0.0010004
	sigma_EPS_B_P = 0.13022
	sigma_EPS_UPSILON_W = 10^-12
	sigma_EPS_UPSILON_W_ST = 10^-12
	...
	SS_N_ST = 0.676386252884814 #QMIPF_solve_SS(zeta,zeta_ST,s_gy,s_gy_ST,SS_Y,SS_C,chi,sigma,eta_0,k_n_ST,alpha,theta_p,chi_0_ST,SS_TAU_N_ST,SS_TAU_C_ST,varkappa_ST,SS_NU_ST,SS_N,theta_w,tau_p,tau_w)
	...
	# ===== STEADY STATE BOUNDS FOR NSSS CONVERGENCE =====
	# These bounds help the NSSS solver find economically meaningful solutions

	# Interest rates (must be above ZLB)
	I > 1.001          # Home interest rate above ZLB (elb = 1.0)
	I_ST > 1.001       # Foreign interest rate above ZLB (elb_ST = 1.0)

	# Positive quantities
	Y > 0.1            # Home output
	Y_ST > 0.1         # Foreign output
	C > 0.1            # Home consumption
	C_ST > 0.1         # Foreign consumption
	C_TIL > 0.1        # Home effective consumption
	C_TIL_ST > 0.1     # Foreign effective consumption

	# Positive prices/wages
	W_C > 0.1          # Home real wage
	W_C_ST > 0.1       # Foreign real wage
	MC_D > 0.1         # Home marginal cost
	MC_D_ST > 0.1      # Foreign marginal cost

	# Positive multipliers
	LAM > 0.01         # Home multiplier
	LAM_ST > 0.01      # Foreign multiplier

	# Positive labor
	N > 0.01           # Home labor
	N_ST > 0.01        # Foreign labor

	# Exchange rate (positive)
	Q > 0.1            # Real exchange rate
