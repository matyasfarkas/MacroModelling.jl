...
X = [-5:0.01:5];
[f,xi] = ksdensity(oo_.SmoothedShocks.em,X);
[f1,xi1] = ksdensity(oo_.SmoothedShocks.em./std(oo_.SmoothedShocks.em) - mean(oo_.SmoothedShocks.em./std(oo_.SmoothedShocks.em)),X);

figure
plot(X,normpdf(X)); hold on; plot(xi,f);  plot(xi1,f1);
legend({'Theoretical - Standard Normal' ;   'Empirical - Filtered shocks distribution';  'Empirical - Filtered shocks distribution - Standardized'});
title('Monetary policy shock distributions - SW07')
print('Monetary policy shock distributions - SW07','-dpng')
