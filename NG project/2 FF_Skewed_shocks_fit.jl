...
Turing.@model function loglikelihood_scaling_function_ff(m, data, observables, Ω) 
    ...
    #DF_out = zeros(eltype(skew_distribution), size(data, 2))
     ...
     skew_distributiont =  Turing.SkewNormal(0,1,DF_out)
     ...
     for t in 2:size(data, 2)
        #  DF  = DF_out[t-1]
        #  DF_out[t] ~  MacroModelling.Normal(0.99*DF,scale*1) 
        #  skew_distribution =  Turing.SkewNormal(0,1,DF_out[t])
         ϵ_skewdraw[t] ~ skew_distributiont
            #  ϵ_skewdraw[1] = rand(skew_distribution,1,1)
...
ff_estimated_parameters_indices = indexin([Symbol("DF_out") for a in 1:size(data,2)], ff_estimated_parameters )
...
FF_shocks = get_estimated_shocks(AS07, data, parameters = parameter_mean)
...
save("FF_TWSK_NAWM_AS07.jld") 




Turing.@model function loglikelihood_scaling_function_ff_TWSK_v0(m, data, observables, Ω) 
...
loglikelihood_scaling_fftwskv0 = loglikelihood_scaling_function_ff_TWSK_v0(AS07, data, observables,Ω) # Kalman
n_samples =50
samps_fftwskv0 = Turing.sample(loglikelihood_scaling_fftwskv0,Turing.NUTS(),n_samples, progress = true)

ff_estimated_parameters_twsk = Turing.describe(samps_fftwskv0)[1].nt.parameters
ff_estimated_means_twsk = Turing.describe(samps_fftwskv0)[1].nt.mean
ff_estimated_std_twsk = Turing.describe(samps_fftwskv0)[1].nt.std
