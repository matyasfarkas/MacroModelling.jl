...
#Build the RBC model
∞ = Inf
@variables α, β, ρ, δ, σ, ξ ,Ω_1
@variables t::Integer, k(..), z(..), c(..), q(..)

x = [k, z] # states
y = [c, q] # controls
p = [α, β, ρ, δ, σ, ξ, Ω_1] # parameters
...
Γ = [σ; 0;;  0; ξ;;] # matrix for the 2 shocks.  The [;;] notation just makes it a matrix rather than vector in julia
η = [0; -1;; -1;0;;] # η is n_x * n_ϵ matrix.  The [;;] notation just makes it a matrix rather than vector in julia
...
model_rbc2 = @make_and_include_perturbation_model("rbc_notebook_example2", H, (; t, y, x, p, steady_states, Γ, Ω, η, Q, overwrite_model_cache)) # Convenience macro.  Saves as ".function_cache/rbc_notebook_example.jl"

#Solve model at some fixed parameters
p_f = (ρ = 0.2, δ = 0.02, σ = 0.01, ξ =0.01, Ω_1 = 0.01) # Fixed parameters
p_d = (α = 0.5, β = 0.95) # Pseudo-true values
m = model_rbc2  # ensure notebook executed above
...
Random.seed!(54321) #Fix seed to reproduce data
noiseshocks2 = rand(shockdist,T)

noise = Matrix(noiseshocks' ) # the ϵ shocks are "noise" in DifferenceEquations for SciML compatibility 
noise= [noiseshocks'; noiseshocks2';;]
#Solve problem forward with Student-t noise
problem1 = LinearStateSpaceProblem(sol, x_iv, (0, T); noise)
problem1 = LinearStateSpaceProblem(sol, x_iv, (0, T); noise)
...
cache = SolverCache(model_rbc2, Val(1),  [:α, :β])
settings = PerturbationSolverSettings(; print_level = 0)
p_f = (ρ = 0.2, δ = 0.02, σ = 0.01,  ξ =0.01, Ω_1 = 0.01) # Fixed parameters
z = z_rbc # simulated in previous steps
turing_model = rbc_1_t_joint(z, model_rbc2, p_f, dof, cache, settings) # passing observables from before 
