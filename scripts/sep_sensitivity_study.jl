...
out_dir        = parse_kv_string(ARGS, "--out-dir",
    joinpath(REPO_ROOT, ".local_artifacts/sep_sensitivity"))
...
println(tex, "\\toprule")
println(tex, "tol & \$K\$ & conv. & periods & max Euler err. & \$\\log_{10}(\\mathrm{err.})\$ & RMSE vs ref.\\\\")
println(tex, "\\midrule")
for ci in 1:n_cells
    atol, K = cells[ci]
    is_ref = (ci == ref_ci)
...
println(tex, "\\begin{minipage}{0.92\\textwidth}")
println(tex, "\\footnotesize\\textit{Notes}: Runs use $(effective_n) posterior draws stratified by Mahalanobis distance from the pooled warm-started surrogate HMC chains (seeds 42, 5, 6, 7). ``Convergence'' is the fraction of draws where SEP completed all $(T_obs) periods. ``Max Euler err.'' is the maximum SEP residual (\$\\ell_\\infty\$ norm of the model's first-order-condition residuals) across all periods, averaged over draws. ``RMSE vs.\\ ref.'' is the root-mean-squared observation prediction error against the reference cell (acceptance tolerance \$= $(ref_cell.accept_tol)\$, \$K=$(ref_cell.K)\$), pooled over draws and $(T_obs) periods.")
println(tex, "\\end{minipage}")
