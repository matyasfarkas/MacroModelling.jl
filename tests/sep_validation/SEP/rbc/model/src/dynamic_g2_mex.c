...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
      ...
      if (T_order_on_input < 2)
        {
          ...
          T_mx = mxCreateDoubleMatrix(max(33, mxGetM(T_old_mx)), 1, mxREAL);
          memcpy(mxGetDoubles(T_mx), mxGetDoubles(T_old_mx), mxGetM(T_old_mx)*sizeof(double));
        }
      else if (mxGetM(T_mx) < 33)
        mexErrMsgTxt("T_mx should have at least 33 elements");
    }
  else
    {
      T_order_mx = mxCreateDoubleScalar(2);
      T_mx = mxCreateDoubleMatrix(33, 1, mxREAL);
      T_order_on_input = -1;
    }
  ...
  dynamic_g2(y, x, params, steady_state, T, mxGetDoubles(plhs[0]));
  if (nlhs == 3)
    {
      plhs[1] = T_order_mx;
      plhs[2] = T_mx;
