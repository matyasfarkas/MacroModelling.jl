...
void dynamic_1_resid(double *restrict y, const double *restrict x, const double *restrict params, const double *restrict steady_state, double *restrict T)
{
  y[12]=params[7]*y[5]+params[8]*x[0];
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (!(mxIsDouble(prhs[7]) && !mxIsComplex(prhs[7]) && !mxIsSparse(prhs[7]) && mxGetNumberOfElements(prhs[7]) >= 1))
    mexErrMsgTxt("T must be a real dense numeric array with at least 1 elements");
  plhs[1] = mxDuplicateArray(prhs[7]);
  double *restrict T = mxGetDoubles(plhs[1]);
  dynamic_1_resid(y, x, params, steady_state, T);
}
