...
function solve!(𝓂::ℳ;
    ...
    if (algorithm == :stochastic_extended_path) && (sep_needs_update || sep_force)
        ...
        conv_flag = result.flag
        final_err = result.err
        
        # Create placeholder state_update function
        # Future: use SEP tree for nonlinear policy
        function state_update(state::Vector{T}, shock::Vector{S}) where {T,S}
            # Placeholder - returns steady state for now
            # Full implementation would use SEP tree structure
            return 𝓂.solution.non_stochastic_steady_state
        end
        
        # Store solution
        𝓂.solution.perturbation.stochastic_extended_path = sep_solution(
            Y, layout, state_update,
            sep_opts.periods, sep_opts.order, sep_opts.nnodes,
            conv_flag, final_err, runtime
        )
        ...
        if !silent 
            println("SEP solution computed in ", round(runtime, digits=3), " seconds")
            println("Convergence: ", conv_flag == 0 ? "SUCCESS" : "FAILED (code: $conv_flag)")
