...
\small
\begin{itemize}
  \item {\bf Nonlinear DSGE likelihoods and filters:}
    \begin{itemize}
      \scriptsize
      \item Fernandez-Villaverde and Rubio-Ramirez (2007);
            Fernandez-Villaverde, Rubio-Ramirez, and Schorfheide (2016);
            Herbst and Schorfheide (2019).
      \item These papers make nonlinear likelihood-based estimation credible,
            but repeated nonlinear filtering remains costly in medium-scale models.
    \end{itemize}

  \item {\bf Occasionally binding constraints and piecewise-linear filtering:}
    \begin{itemize}
      \scriptsize
      \item Guerrieri and Iacoviello (2015);
            Cuba-Borda, Guerrieri, Iacoviello, and Zhong (2019);
            Aruoba, Cuba-Borda, Higa-Flores, Schorfheide, and Villalvazo (2021).
      \item These methods are stable and useful for OBCs, but keep the solution
            close to linear or piecewise-linear objects.
    \end{itemize}

  \item {\bf Neural network solvers and estimation:}
    \begin{itemize}
      \scriptsize
      \item Maliar, Maliar, and Winant (2021);
            Azinovic, Gaegauf, and Scheidegger (2022);
            Kase, Melosi, and Rottner (2025, MKR);
            Naubert (2025).
      \item Closest paper: MKR use neural networks to make nonlinear structural
            estimation feasible in heterogeneous-agent models.
    \end{itemize}

  \item {\bf This paper: improvements relative to MKR and related work}
    \begin{itemize}
      \scriptsize
      \item \textbf{SEP teacher:} the NN is trained and audited against direct global nonlinear solves.
      \item \textbf{Residual target:} learn \(g^{\sep}-g^{\rom}\), so the method collapses to ROM1 when linearization is enough.
      \item \textbf{Gated likelihood:} the correction is active only where support has been checked.
      \item \textbf{Economic output:} the residual identifies which DSGE mechanisms create the nonlinear gap.
    \end{itemize}
\end{itemize}
\sourcefoot{MKR denotes Kase, Melosi, and Rottner, ``Estimating Nonlinear Heterogeneous Agent Models with Neural Networks'' (2025).}
\end{frame}
...
\claim{For a real results slide, run one adverse demand shock through every solver on the same calibration.}
...
\node[font=\small\bfseries] at (7.0,13.2) {Policy-rate response};
...
\sourcefoot{Schematic template for an adverse demand/natural-rate shock in which the shadow policy rate falls and the ELB binds. This is not a policy-rate shock result. For evidence, the same shock vector and calibration must be run through each solver and the artifact path reported.}
\end{frame}
...
% -----------------------------------------------------------------------------
\begin{frame}{What Would a Full SEP--NN Run Take?}
...
\vspace{0.25em}
\small
If base and ZLB data jobs run in parallel, the end-to-end wall-clock estimate is
\textbf{5--8 days}, including training and matched HMC checks.

\smallnote{The upper tail is solver risk: difficult SEP points, retries, and initialization matter more than neural-network training time.}
\end{frame}
...
\textbf{Still open}
\begin{itemize}
  \item Full 18-parameter SEP--NN posterior run.
  \item Larger prior/posterior support for the corrected unit-shock dataset.
  \item Robust SEP initialization for the hardest binding points.
