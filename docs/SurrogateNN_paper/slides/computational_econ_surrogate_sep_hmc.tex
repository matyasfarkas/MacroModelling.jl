...
\claim{Use the linear solution unless the likelihood needs the nonlinear correction.}
...
\softline
Each period starts from ROM1. A neural net adds only the SEP--ROM1 residual,
and only where the training support says that correction is reliable.
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Why it matters}

\softline
The nonlinear solver is not called inside HMC. The sampler sees one smooth
likelihood, while the likelihood records whether each period used ROM1 or ROM1+NN.
\end{column}
\end{columns}

\smallnote{Frontier contribution: a differentiable likelihood whose solution order is observed period by period: linear where it is enough, nonlinear where it improves the likelihood, direct SEP as the teacher and benchmark.}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{The Intuition: Precompute the Hard Object}
\claim{A surrogate is useful only when we are clear about the hard function it replaces.}
...
\begin{center}
\normalsize
Here the hard function is the SEP transition map:
\[
...
\claim{The economics wants nonlinear dynamics; the posterior sampler wants speed.}
...
\vspace{0.55em}
\begin{center}
\normalsize The goal is not to replace linear methods everywhere. It is to know when linear is enough and when the nonlinear correction changes inference.
\end{center}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{Question}
\claim{Can we move the nonlinear solve outside the posterior loop without losing the likelihood?}
...
\begin{itemize}
  ...
  \item Period-level diagnostics that say when linearization is enough.
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{itemize}
  ...
  \item Slow, noisy, or non-differentiable likelihood evaluations.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\textbf{This paper} & \textbf{ROM1 by default, NN residual when supported} & \textbf{A smooth HMC likelihood with reported support and error} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{center}
\normalsize
The neural net is not the model. It is the correction to the linear solution, trained against SEP and used only where checked.
\end{center}
...
\claim{The nonlinear solve is too expensive to repeat at every HMC proposal.}
...
\]
\end{center}
The expensive object is recomputed at the point where the sampler needs thousands of fast, stable evaluations.
...
\claim{One model, one likelihood, three solution orders.}
...
\node[box, fill=PaleBlue] (rom) {\textbf{1. ROM1}\\ use when enough};
\node[box, fill=PaleGreen, below=of rom] (nn) {\textbf{2. ROM1 + NN}\\ use where nonlinear};
...
\smallnote{The switch is inside the likelihood evaluation. The posterior draw does not choose a different model; it reveals where first-order dynamics suffice and where nonlinear solution information enters.}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{Algorithm: Switching Solution-Order Likelihood}
\claim{The algorithm returns a likelihood value and a record of which solution order was used.}
...
\midrule
Offline 1 & Solve SEP at design triples \((\state,\eps,\theta)\). & SEP is the expensive teacher, not the online likelihood engine. \\
Offline 2 & Compute ROM1 at the same triples and train \(r_\phi\approx g^\sep-g^\rom\). & The NN learns only what the linear solution misses. \\
Online 1 & Given \(\theta\), recover or sample shocks and compute support score \(d_t\). & Filter-free shock treatment follows Childers et al.; inversion gives the medium-scale profile target. \\
Online 2 & Evaluate \(\widehat g_t=g^\rom_\theta+\omega(d_t)r_\phi\). & If \(\omega_t\simeq0\), this is linear; if \(\omega_t\simeq1\), the SEP residual is active. \\
Online 3 & Accumulate likelihood and use HMC/NUTS on the differentiable target. & HMC sees one smooth target; the output records where nonlinear information enters. \\
\bottomrule
\end{tabular}
\end{table}
...
\claim{The NN approximates a residual map, not preferences, constraints, or equilibrium conditions.}
...
\vspace{0.45em}
\begin{center}
\small If ROM1 is exact, the residual is zero. If nonlinear dynamics matter, the residual is precisely the object the NN has to learn.
\end{center}
...
\claim{The gate adds the NN correction only on checked support.}

\vspace{-0.15em}
\begin{center}
\resizebox{0.88\textwidth}{!}{%
\begin{tikzpicture}[
  node distance=0.48cm and 0.66cm,
...
\begin{itemize}
  \item ROM1 periods.
  \item SEP--ROM1 correction periods.
  \item Weak-support periods to benchmark.
\end{itemize}
\end{column}
\end{columns}
...
\vspace{-0.2em}
\begin{center}
\scriptsize Output: likelihood contribution plus period-level solution-order diagnostic.
\end{center}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{Step 0: Choose the Estimation Target}
\claim{First define the likelihood; then decide how to approximate its transition map.}
...
\begin{itemize}
  ...
  \item Which regions are checked against direct SEP.
\end{itemize}
\end{column}
\end{columns}

\smallnote{A surrogate that is accurate for one target need not be accurate for another. This is why the validation is tied to the likelihood, not only to one-step forecasts.}
...
\claim{Use SEP where accuracy matters most: to create the training target and benchmark cells.}
...
\begin{itemize}
  ...
  \item Produces the solved transition that ROM1 and the NN are compared to.
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\claim{Linearization is the control variate: cheap, stable, and often already right.}
...
\softline
ROM1 is fast and often accurate near the steady state. The NN should learn only
the forecast error of ROM1 relative to SEP.
...
\[
  ...
  \text{linear baseline}
  +
  \text{learned nonlinear correction}.
\]
\end{center}
...
\claim{Train the smallest object that carries the nonlinearity.}
...
\begin{itemize}
  \item Smaller target.
  \item Zero target where ROM1 is already right.
...
\claim{The gate can only be trusted where direct SEP checks say the residual is accurate.}
...
\begin{itemize}
  ...
  \item Finite likelihood and finite-gradient checks for HMC.
\end{itemize}
\vspace{0.25em}
\textbf{Rule}

\softline
Outside checked support, fall back toward ROM1 or benchmark the draw with direct SEP.
\end{column}
\end{columns}
...
\claim{The direct benchmark and the medium-scale target answer different questions.}
...
\[
  ...
  \right).
\]
\scriptsize This is the clean synthetic-data benchmark: does surrogate-HMC recover the same posterior as direct SEP-HMC?
\end{column}
\begin{column}{0.46\textwidth}
...
\]
\scriptsize Here shocks are recovered period by period. The gate reports when the likelihood uses the nonlinear correction rather than ROM1.
\end{column}
\end{columns}
...
\claim{Filter-free HMC moves parameters and shocks together through a differentiable recursion.}
...
\smallnote{For validation, a low surrogate RMSE is not enough. The posterior from surrogate-HMC must agree with the posterior from direct SEP-HMC within Monte Carlo error.}
...
\claim{The likelihood should leave an audit trail: where was it linear, where nonlinear, and where unsupported?}
...
\vspace{0.4em}
\begin{center}
\small The replication package should make these checks mechanical: every likelihood run should report support, gate weights, residual error, gradients, and HMC behavior.
\end{center}
...
\claim{Validation is meaningful only where the nonlinear model visibly differs from ROM1.}
...
\claim{Hold the inputs fixed, then ask what each solution order predicts.}
...
\begin{center}
\parbox{0.90\linewidth}{\centering\scriptsize The first question tests the map. The second tests the likelihood. A conference audience should see both.}
\end{center}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{Validation: What the Small Model Shows}
\claim{The small model asks whether the full estimation loop can recover known nonlinear parameters.}
...
\begin{itemize}
  \item The pipeline can recover parameters when the data are generated by the nonlinear model.
...
\claim{Smets--Wouters is the stress test: many states, many shocks, and several nonlinear mechanisms.}
...
\begin{itemize}
  ...
  \item Mechanisms are interpretable: investment, utilization, pricing, wages, constraints.
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{itemize}
  ...
  \item Strong need to know whether posterior draws stay inside checked support.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\claim{Near the posterior region, ROM1's largest SEP error comes from real-side curvature.}
...
\claim{Kimball curvature matters in a high-curvature neighborhood, but that is not the posterior region.}
...
\softline
The posterior-region result says where the likelihood needs nonlinear help in
this estimation problem. It is not a claim that Kimball curvature never matters.
\end{column}
\end{columns}
\end{frame}
...
\claim{The computational gain comes from paying the nonlinear price only once, then using it selectively.}
...
Gate/support rule & reporting rule & chooses order by date \\
...
\vspace{0.55em}
\begin{center}
\normalsize The online estimator is cheap because ROM1 carries the easy part.\\
The NN is valuable because it adds only the nonlinear part SEP teaches offline.
\end{center}
\end{frame}

\begin{frame}{Preliminary Evidence: What the Diagnostics Say}
\claim{The current evidence supports the pipeline, while the full posterior run is still being extended.}
...
\begin{itemize}
  ...
  \item The relevant question is whether the posterior lies on checked support.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\claim{A good nonlinear DSGE estimator should know when not to be nonlinear.}

\vspace{0.35em}
\begin{enumerate}
  \item The computational obstacle is clear: global nonlinear solves are too
        expensive for the inner likelihood loop.
  \item The solution is simple: ROM1 does the cheap part; the NN adds the
        SEP residual only where support has been checked.
  \item Direct SEP remains the teacher and benchmark, not the online estimator.
  \item The small nonlinear validation checks posterior agreement, not only map fit.
...
\smallnote{For this audience, the key object is not a black-box NN. It is a structural likelihood whose solution order is observed, gated, and benchmarked.}
\end{frame}
...
\vspace{0.45em}
\begin{center}
\small The shared idea is amortization. The distinction is the likelihood object: I train the SEP--ROM1 residual, keep ROM1 as the default online transition, and report posterior-support checks explicitly.
\end{center}
\end{frame}
