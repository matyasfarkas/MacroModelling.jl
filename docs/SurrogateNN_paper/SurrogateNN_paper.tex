...
\noindent
\footnotesize
This paper asks where first-order Smets--Wouters loses quantitatively important
nonlinear dynamics. In the model studied here, the answer is the investment
block. The investment adjustment cost has zero level and zero marginal cost at
trend growth, so the level resource cost is first-order invisible and the
state-dependent products involving marginal adjustment costs are compressed to
local linear coefficients. A stochastic-extended-path decomposition assigns 69
percent of the nonlinear-minus-linear solution gap to investment and capital,
and 0.2 percent to the price Phillips curve. Investment remains the largest
source of the correction in simulations with lower-bound episodes, although its
binding-episode share falls at the most extreme shock scale.

I estimate the nonlinear model on U.S. data with a neural network surrogate.
The network does not learn the full transition. It learns the residual between
the nonlinear stochastic-extended-path transition and the first-order transition.
A gate applies this correction only when the economy is far from steady state.
This design makes the approximation error measurable. In a reduced
four-parameter HLT bridge, all 10{,}000 supported direct-SEP grid cells solve;
the surrogate lowers prediction RMSE from 0.1436 to 0.00075 relative to direct
SEP, and log-posterior-surface RMSE falls from 88.07 to 0.053. Baseline
surrogate NUTS-HMC estimation completes in 2.4 hours with zero divergent
transitions.

The empirical evidence is model-specific. On 1959--2004 data, nonlinear
estimation is associated with higher risk-premium volatility and lower
wage-markup persistence. On 1959--2025 data, a high-likelihood warm-started
surrogate mode improves the posterior-mean profiled log objective by 35.92 nats
relative to the linear Kalman posterior mean, and the linear smoother requires
risk-premium shocks of $56\sigma$ during COVID. This comparison is conditional
on one country, one model specification, one posterior basin, the surrogate
approximation, the inversion objective, the gate design, and COVID
out-of-distribution risk. Restoring the ARMA(1,1) markup terms improves the
linear log likelihood by 34 nats on the baseline sample, almost the same scale
as the nonlinear gap. The posterior shifts should therefore be read as evidence
relative to this HLT benchmark, not as a specification-invariant rejection of
linear DSGE estimation. Calm-period forecasting remains a gate-design risk:
the original quiet-sample gate fails, while a q95 padded re-estimation lowers
the full-window aggregate RMSE ratio to 1.09.
\end{abstract}
...
Nonlinear DSGE estimation is often motivated by the effective lower bound
(ELB). The ELB creates a kink that first-order perturbation cannot represent,
and the U.S. ELB episodes in 2009--2015 and 2020--2022 are central episodes for
monetary policy. A large literature has therefore built nonlinear tools around
this constraint \citep{gust2012effects,boehl2022monetary,guerrieri2015occbin}.
In the Smets--Wouters specification studied here, the main measured
nonlinearity is instead in the investment block.

I do three things. First, I measure which equations create the
gap between the nonlinear solution and the first-order solution. Second, I
build a surrogate estimator that can take this nonlinear model to U.S. data.
Third, I report what changes in the posterior, while keeping the limits of
that comparison explicit.

The first result is deterministic. I solve the 18-parameter Smets--Wouters
model with occasionally binding constraints at 22,080 state--shock--parameter
combinations. I then compare each nonlinear stochastic-extended-path (SEP)
solution with the corresponding first-order perturbation. The investment and
capital block accounts for 69 percent of the total gap. The price Phillips
curve, where the Kimball aggregator enters and where much of the nonlinear DSGE
literature has focused \citep{gust2012effects,boehl2022monetary,fernandezvillaverde2021estimating},
accounts for 0.2 percent. In a separate shock-scale exercise, investment still
accounts for 88--92 percent of the unconditional gap when lower-bound episodes
occur, although its share during binding periods falls at the largest shock
scale (Figure~\ref{fig:decomposition_vs_scale}).

The reason is simple. Let \(x_t=I_t/I_{t-1}\) and write the level investment
adjustment cost as \(S(x_t)=\tfrac{\chi}{2}(x_t-1)^2\). At trend growth,
\(S(1)=S_x(1)=0\). The level resource cost is therefore invisible to a
first-order approximation. The marginal cost \(S_x(x_t)=\chi(x_t-1)\) is
locally visible, but first-order perturbation keeps only the local slope. It
does not keep the products between \(S_x\), Tobin's \(q\), the shadow price of
capital, and the stochastic discount factor. Proposition~\ref{prop:invisibility}
formalizes the level-cost statement: any function with
\(f(\bar{x})=f'(\bar{x})=0\) contributes zero in levels to a first-order method.

Three diagnostics support the ranking. The equation-block decomposition puts
investment at 69 percent and pricing at 0.2 percent (Table~\ref{tab:equation_block}).
A Gal\'i (2015) model with the same ELB and Kimball aggregation but no
investment block has near-zero nonlinearity at empirically relevant shock scales
(Figure~\ref{fig:gali_obc_vs_no_obc}). In counterfactual HLT specifications,
removing \(S(\cdot)\) increases the measured gap because the adjustment cost
both creates nonlinear marginal products and dampens investment volatility,
while linearizing the utilization cost \(a(z)\) changes the gap by less than
1.5 percent.

If the linear solution misses this investment-channel amplification, the
posterior may compensate through shocks and persistence parameters that load on
investment dynamics. On U.S. data from 1959Q1 to 2004Q4, nonlinear estimation
is associated with a doubling of risk-premium volatility \(\sigma_b\), a
compression of wage-markup persistence \(\rho_w\) from 0.37 to 0.08, and a
2.7-fold rise in Kimball curvature \(\varepsilon_p\)
(Table~\ref{tab:18param_switching_posterior}). These shifts match the
invisibility mechanism, but they are conditional on the HLT model, the priors,
and the surrogate objective.

The extended 1959Q1--2025Q1 sample is more informative and more fragile. The
linear smoother explains COVID with risk-premium shocks above \(56\sigma\),
price-markup shocks of \(32\sigma\), and monetary-policy shocks of
\(23\sigma\). A high-likelihood warm-started surrogate mode improves the
posterior-mean profiled log objective by 35.92 nats
(\(-1{,}781.68\) versus \(-1{,}817.59\)) and shifts the posterior toward lower
shock volatilities and higher investment-related persistence. This is not a
marginal-likelihood or Bayes-factor comparison. It is conditional on one
posterior basin, the surrogate approximation, the inversion objective, the gate
design, and COVID out-of-distribution risk.

There is also a specification caveat. The HLT baseline suppresses the
ARMA(1,1) price- and wage-markup shock terms used by
\citet{smets2007shocks}. This is the same block where
\citet{farkas2020bayesian} document a flat posterior target in the full
Smets--Wouters model. Restoring those MA terms on the baseline sample improves
the linear log likelihood by 34 nats (Appendix~\ref{app:hlt_vs_ft}), almost the
same scale as the 35.92-nat nonlinear objective gap in the extended sample. I
therefore read the nonlinear posterior shifts as evidence relative to the HLT
benchmark used to train the surrogate, not as a specification-invariant
measurement of linearization bias.

The estimator is designed around one idea: learn only what first order misses.
The neural network learns the residual
\(g^{\text{SEP}}-g^{\text{ROM1}}\), not the full nonlinear transition. A
regime-switching gate applies the correction only when the economy is far from
steady state and uses the exact Kalman filter in calm periods. I sample with
NUTS-HMC. A bootstrap particle filter, the standard nonlinear likelihood
alternative, collapses to a single effective particle in every period at all
particle counts tested; at \(N_p=5{,}000\), its log-likelihood magnitude is
roughly four orders worse than the Kalman value (Table~\ref{tab:pf_benchmark}).
The filter-free formulation avoids this problem by treating shocks as explicit
unknowns.

Two features make the surrogate auditable. First, the residual target makes the
approximation error directly measurable on validation data, and
Appendix~\ref{sec:formal_consistency} states conditions under which this error
has limited posterior impact. Second, the network is trained once on parameter
draws spanning prior support and then reused across samples. A reduced
four-parameter HLT bridge benchmarks the same residual surrogate against direct
SEP: all 10{,}000 supported cells solve, the surrogate lowers prediction RMSE
from 0.1436 to 0.00075, and a held-out posterior-grid comparison matches direct
SEP intervals for all four investment-block parameters
(Appendix~\ref{sec:validation_design}). The online technical appendix gives the
full computation record: scripts, output files, validation criteria, and the
limits of each diagnostic.

The main implication is narrow but useful. In the HLT model studied here, a
first-order estimate can attribute too much variation to exogenous shocks when
the missing investment channel is large. For researchers deciding which
nonlinearities to keep in estimated Smets--Wouters models, the deterministic
evidence favors prioritizing the investment block over the ELB and Kimball
pricing. The posterior comparison strengthens that conclusion within the
current model, but it does not establish global posterior dominance across
alternative specifications.

The out-of-sample evidence reinforces this caution. The switching surrogate
improves COVID-window conditional predictions but fails a quiet-sample
1995Q1--2007Q4 pilot under the original gate, where aggregate RMSE is 2.40 times
ROM1. A q95 gate diagnostic using the same posterior draws reduces the ratio to
1.04 and the hard-gate activation rate to 1.9 percent. Re-estimating the
surrogate chain under this q95 gate gives a 1.09 ratio: much better than the
original gate and better than ROM1 early in the quiet window, but still worse
over the full calm holdout. Calm-period gate design remains a risk, not a
resolved strength.

The paper draws on three literatures. For the nonlinear solver, I use the
stochastic extended path method \citep{fair1983solution,adjemian2025stochastic},
which handles occasionally binding constraints and scales to medium-size
models; \citet{richter2017nonlinear} surveys nonlinear solution methods with
occasionally binding constraints. For the surrogate, I build on multi-fidelity
methods \citep{peherstorfer2018survey} and model-discrepancy methods
\citep{kennedy2001bayesian}. The closest recent DSGE surrogate papers are
\citet{kase2025estimating} and \citet{naubert2025differentiable}; the difference
here is that the network approximates a transition residual rather than the
likelihood itself. For sampling, I extend the filter-free HMC approach of
\citet{childers2022filter} and \citet{farkas2020bayesian} to the nonlinear
setting, motivated by particle-filter degeneracy in medium-scale DSGE models
\citep{herbst2019tempered,cubaborda2019likelihood}. The formal consistency
arguments build on likelihood inference with computed nonlinear solutions in
\citet{fernandezvillaverde2006convergence}.
