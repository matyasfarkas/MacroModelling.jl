...
Harding--Lind\'e--Trabandt (HLT) specification and audited support studied here,
...
Harding--Lind\'e--Trabandt (HLT), where is the largest nonlinear error from
...
profiled inversion objective, the gate, and COVID out-of-distribution risk. A
corrected unit-shock dynamic MVP confirms that the gate, inversion filter, and
HMC stack run end to end on the 18-parameter real-data payload, but in that
narrow run the full gate+NN and linear+gate objectives are nearly
indistinguishable. I therefore do not interpret the empirical criterion gap as a
stand-alone estimate of the neural residual's contribution.
...
Structural parameters explain 11 percent of nonlinearity variation in separate diagnostic regressions. The strongest correlates are government spending persistence $\rho_g$ ($+0.18$), risk-premium volatility $\sigma_b$ ($+0.17$), and Kimball curvature $\varepsilon_p$ ($-0.11$)---higher curvature flattens demand and makes the model \emph{more} linear. Current-period shocks contribute negligibly ($R^2 = 0.0003$). Nonlinearity accumulates through persistent displacement, clustering in temporal bursts with autocorrelation 0.81 and mean duration 12.5 quarters; per-observable diagnostic $R^2$ values remain below 12 percent. A posterior-region local ablation gives the complementary check: across 40 representative posterior draws and shock scales 0.1, 0.25, and 0.5, all SEP paths converge; Kimball curvature is the largest local sensitivity in 5, 0, and 0 percent of draws, while investment-adjustment and utilization curvature are largest in the remaining 95, 100, and 100 percent.

Figure~\ref{fig:posterior_mean_curvature_surface} shows the same comparison as a density-scaled local surface around the maintained SW07--HLT baseline calibration. The axes are measured in local standard deviations, using documented prior/posterior scales where available and calibration-scale proxies for fixed parameters. In the plotted units, RMSE times 1,000, the $\pm 1$ s.d. grid has range 11.4 for the real-side surface and 1.4 for the Kimball surface; over the $\pm 2$ s.d. grid the corresponding ranges are 23.5 and 2.6. Thus, around the baseline and estimated posterior region, the investment-adjustment and utilization parameters move the SEP--ROM1 forecast error much more than price and wage Kimball curvature.

This ranking is not a global theorem. A fixed-calibration stress check at the \citet{harding2022missingdeflation} high-curvature neighborhood, mapping their reported $\varphi_p=1.20$, $\xi_p=0.667$, and $\varepsilon_p=77.3$ values plus the matching reported core and shock-process parameters into the maintained code, gives an investment/capital share of 76.5 percent and a price Phillips share of 2.0 percent in the block decomposition. But local surface plots centered at that high-Kimball point show that sufficiently large price and wage curvature can dominate the short-run ROM1 forecast-error surface: at $\pm 1$ s.d., the real-side and Kimball ranges are 37.0 and 54.7; at $\pm 2$ s.d., they are 53.9 and 210.6. The conclusion is therefore precise: nonlinear Phillips-curve effects can be large at selected high-curvature calibrations, but active estimation places the maintained HLT specification in a region where the largest local nonlinear residual is in the real investment block.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_hlt_curvature_surface_sw07_hlt_baseline_13x13_std2_shock01.png}
\caption{Baseline ROM1 forecast-error contours. The metric is \(1{,}000\) times the RMSE of one-step observable forecast errors, SEP minus ROM1, evaluated at the same SEP state, same shocks, and same parameter vector. Panels~A and~B plot filled contours over a \(\pm 2\) local-standard-deviation neighborhood around the maintained SW07--HLT baseline calibration; black contour lines are equal forecast-error levels, and both panels use the same color scale. Panel~C plots cross-sections through the center cell. Panel~D reports the RMSE range over each grid. The real-side surface combines investment-adjustment and capital-utilization parameters; the Kimball surface combines price and wage curvature parameters.}
\label{fig:posterior_mean_curvature_surface}
\end{figure}
...
The off-diagonal entries provide a chain-mean accounting exercise. At $\hat{\theta}_{\text{surr}}$, the linear Kalman likelihood falls by 35.04 nats relative to its own posterior mean ($-1{,}852.63$ versus $-1{,}817.59$): the linear model does not absorb the surrogate's parameters. At $\hat{\theta}_{\text{lin}}$, the surrogate objective is 12.85 nats below the linear posterior-mean likelihood---swapping criteria without re-tuning loses ground because each parameter vector is fitted to its own criterion. Symmetrically averaging the two path decompositions of the gap (model-change-then-parameters and parameters-then-model-change) assigns 29.05 nats (80.9 percent) to the criterion change and 6.86 nats (19.1 percent) to parameter relocation. This shows that most of the reported gap is tied to changing the profiled objective within the high-objective surrogate mode, but it does not by itself identify whether that change comes from the residual surrogate, the gate, the inversion filter, or their interaction.

The remaining design concern is whether the gain comes from the gate/inversion architecture itself rather than from the learned nonlinear residual. I therefore run a corrected unit-shock MVP that keeps the same real-data payload, gate, inversion filter, observation scaling, priors, seed, and NUTS settings, but sets the NN residual correction to zero. Table~\ref{tab:linear_gate_ablation} reports the matched HMC runs and fixed-theta cross-evaluations.

The result is sharper, and less favorable to an NN-residual interpretation, than the earlier fixed-parameter diagnostic. Both 18-parameter chains complete with zero post-warmup divergences, land in almost the same posterior region, and deliver almost identical objectives. At the full gate+NN posterior mean, switching off the NN residual changes the objective by only \(0.006\) nats; the same is true at the linear+gate posterior mean. I therefore read this corrected MVP as validation of the dynamic gate/inversion/HMC stack, not as evidence that the learned residual correction delivers a large additional real-data likelihood gain in this design.

\begin{table}[htbp]
\centering
\caption{Corrected unit-shock MVP: gate+NN versus linear+gate.}
\label{tab:linear_gate_ablation}
\begin{tabular}{lcc}
\toprule
& Full gate+NN & Linear+gate, no NN \\
\midrule
Post-warmup draws & 100 & 100 \\
Divergences & 0 & 0 \\
Mean acceptance & 0.840 & 0.812 \\
Posterior-mean objective & $-1{,}382.805$ & $-1{,}382.877$ \\
\midrule
$Q_{\text{full}}(\hat{\theta}_{\text{full}})$ & $-1{,}382.805307$ &  \\
$Q_{\text{gate}}(\hat{\theta}_{\text{full}})$ & $-1{,}382.799759$ &  \\
$Q_{\text{full}}(\hat{\theta}_{\text{gate}})$ &  & $-1{,}382.882119$ \\
$Q_{\text{gate}}(\hat{\theta}_{\text{gate}})$ &  & $-1{,}382.876532$ \\
\bottomrule
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: Corrected unit-shock MVP on the non-OBC HLT real-data payload. Both chains use 100 warmup draws, 100 post-warmup draws, the same soft gate, the same inversion filter, and seed 21. \(Q_{\text{full}}\) is the gate+NN profiled hybrid objective. \(Q_{\text{gate}}\) is the same objective with \texttt{--disable-nn-correction=true}. The artifact root is \texttt{.local\_artifacts/hlt\_18param\_validation\_unitshock\_mvp\_20260620/}.}
\end{table}
...
estimator. The current validation record is strongest for the deterministic
decomposition and the reduced HLT bridge; the corrected 18-parameter dynamic
MVP validates the execution stack but does not show a large incremental
real-data gain from the neural residual. The next step is clear: extend the
direct validation beyond the reported HLT bridge, run the corrected full-scale
unit-shock estimation, and test the investment result in other medium-scale
models with adjustment costs.
...
The quantitatively meaningful comparison for likelihood-based inference is the prediction RMSE of the surrogate (non-binding validation RRMSE $= 0.0012$) and the realized profiled-objective improvement (35.92 nats over the linear model on the extended sample). The Euler-error ranking does not contradict these results because the two metrics measure different objects: Euler errors quantify how well a trajectory satisfies the model's own equations, while the objective quantifies how well a method's predictions match observed data under the chosen inversion criterion. The 69-percent investment-dominance decomposition (Section~\ref{subsec:nonlinearity_sources}) is computed from SEP solutions---the most accurate method on the Euler-error metric---so the decomposition does not inherit the surrogate's larger Euler error. The full 265-period SEP-sensitivity rerun in Appendix~\ref{app:sep_sensitivity} supports tolerance stability within fixed quadrature-node settings; the $K=3$ cells sit just above the original RMSE screen against the $K=5$ reference and are reported as a solver-stability diagnostic rather than as a new benchmark choice.
...
A stricter dynamic HLT bridge now diagnoses the multi-period inversion layer. In that exercise I generate a short observation panel directly from SEP at a held-out truth, recover shocks with the same baseline-ROM1 inversion filter used by the empirical HMC code, and evaluate direct SEP and surrogate objectives under a common measurement-error likelihood. The fixed one-step NN residual is not enough for this task: in an eight-period, ten-anchor diagnostic all ten direct-SEP anchors are finite, but the centered surface RMSE is 1.872 for the one-step surrogate versus 1.395 for ROM1. I therefore add a path-calibrated residual correction. It fits a small ridge map from dynamic ROM1 states, recovered shocks, parameters, and period index to direct SEP minus ROM1 observation residuals on five calibration anchors, then evaluates five held-out anchors. Under the same observation floor as the fixed-NN diagnostic, the held-out residual RMSE is 0.0118, the held-out centered log-posterior-surface RMSE falls from 1.437 under ROM1 to 0.703 under the dynamic residual, the local MAP agrees with direct SEP, and all four 90 percent intervals overlap. This is evidence for the dynamic residual target on a reduced HLT bridge; it is still not a full 18-parameter direct-SEP HMC certificate.
...
Four-period HLT inversion-bridge smoke & Corrected generator-path diagnostic passes on three anchors: direct SEP is finite, local MAP and 90 percent intervals agree, and centered surface RMSE falls from 1.190 under ROM1 to 0.861 under the residual surrogate \\
Eight-period fixed-NN HLT inversion diagnostic & Direct SEP is finite at 10 of 10 anchors, but the fixed one-step NN fails the dynamic surface-shape criterion: centered surface RMSE is 1.872 for the surrogate versus 1.395 for ROM1 \\
Eight-period dynamic-calibrated HLT inversion bridge & Passes held-out dynamic bridge: five calibration anchors, five held-out anchors, held-out residual RMSE 0.0118, centered surface RMSE falls from 1.437 under ROM1 to 0.703 under the dynamic residual, local MAP agrees, and all four 90 percent intervals overlap direct SEP \\
Corrected unit-shock HLT HMC MVP & Full gate+NN and linear+gate 18-parameter real-data NUTS smokes both complete with 100 post-warmup draws and zero divergences; fixed-theta objective differences are below 0.006 nats, so this validates the dynamic gate/inversion/HMC stack but not a large incremental NN-residual gain \\
...
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: The package-level audit validates the ROM1-residual, inversion-filter, and matched-HMC layers in a smaller hard-ELB Gal\'i OBC exercise. The HLT bridge validates the ROM1-residual target against direct SEP on finite support in the medium-scale model, but it is not a full 18-parameter direct-SEP HMC certificate. The corrected dynamic HLT smoke uses artifact \texttt{.local\_artifacts/hlt\_reduced\_bridge\_validation/full\_dynamic\_hlt\_profile\_generator\_smoke\_fixed\_20260602/}. The fixed-NN eight-period diagnostic uses artifact \texttt{.local\_artifacts/hlt\_reduced\_bridge\_validation/full\_dynamic\_hlt\_profile\_generator\_moderate\_20260602/}. The dynamic-calibrated eight-period bridge uses artifact \texttt{.local\_artifacts/hlt\_reduced\_bridge\_validation/full\_dynamic\_hlt\_profile\_dynamic\_ridge\_floor01\_20260602/}. The corrected unit-shock HLT HMC MVP uses artifact \texttt{.local\_artifacts/hlt\_18param\_validation\_unitshock\_mvp\_20260620/}. The $\sigma_\nu$ probes are reported as identification diagnostics rather than approximation failures.}
\end{table}
...
\caption{FOM--ROM1 gap decomposition in the Gal\'i (2015) model, which has a lower bound but no investment block or Kimball demand curvature. Shares are percentages of the total squared transition-function gap at each shock scale. The plotted gap is much smaller than in the HLT model at every shock scale, consistent with the investment channel being the main difference between the models.}
...
The linear HMC baseline in Table~\ref{tab:18param_posterior} uses the Harding--Lind\'e--Trabandt (HLT) reparameterization of the \citet{smets2007shocks} model, which differs from the original specification in ways that affect posterior comparison with earlier work. This appendix documents these differences, compares the 18-parameter HMC posterior to the results in \citet{farkas2020bayesian}, and reports a sensitivity analysis restoring the moving-average (MA) markup structure.
