...
\noindent
\footnotesize
First-order perturbation misses the dominant deterministic nonlinearity in the Smets--Wouters model studied here. The investment adjustment cost vanishes with zero marginal cost at trend growth, so the level resource-cost term is first-order invisible while the state-dependent products involving marginal adjustment costs are compressed to local linear coefficients. A stochastic-extended-path decomposition of the nonlinear-minus-linear solution gap assigns 69 percent of the gap to the investment and capital block and 0.2 percent to the price Phillips curve. In simulations with binding lower-bound episodes, investment remains the largest source of the nonlinear correction, although its binding-episode share falls at the most extreme shock scale.

To estimate the nonlinear model on U.S. data, I develop a neural network surrogate that learns the residual between nonlinear and linear transitions, coupled with a regime-switching filter that activates the nonlinear correction only in periods of large displacement from steady state. The residual formulation makes approximation error observable on validation data, and a formal consistency result gives conditions under which this error has limited posterior impact. A reduced four-parameter HLT bridge benchmarks the same residual surrogate against direct SEP on finite support: the surrogate lowers prediction RMSE from 0.1427 to 0.00564 relative to direct SEP and lowers log-posterior-surface RMSE from 86.48 to 0.36. Surrogate-based Bayesian estimation via NUTS-HMC completes in 2.4 hours with zero divergent transitions in the baseline run.

The empirical results are model-specific rather than a generic proof of bias from linearization. On the 1959--2004 sample, nonlinear estimation is associated with higher risk-premium volatility and lower wage-markup persistence, consistent with the missing investment channel changing how shocks are allocated. On the 1959--2025 sample, a high-likelihood warm-started surrogate mode improves the posterior-mean profiled log objective by 35.92 nats relative to the linear Kalman posterior mean, and the linear smoother requires risk-premium shocks of $56\sigma$ during COVID. The same comparison is conditional on one country, one model specification, a single posterior basin, the surrogate approximation, the inversion objective, the gate design, and COVID out-of-distribution risk. A restored ARMA(1,1) markup specification improves the linear log likelihood by 34 nats on the baseline sample, comparable to the nonlinear gap, so the posterior shifts should be read as evidence relative to this HLT benchmark rather than as a specification-invariant rejection of linear DSGE estimation. A quiet-sample forecast pilot fails under the original gate, while a no-HMC gate recalibration diagnostic reduces the aggregate RMSE ratio from 2.40 to 1.04. Re-estimating the surrogate posterior under the q95 padded gate gives a 1.09 full-window ratio, with gains early in the quiet sample and losses later, so the calm-period gate-design risk is reduced but unresolved.
\end{abstract}
...
The ranking predicts that the investment block should dominate the FOM--ROM1 gap, the wage block should contribute modestly, and the pricing block and ELB should be small at empirically relevant shock scales. Section~\ref{sec:decomposition} finds patterns consistent with these predictions.
...
A common DSGE estimation benchmark linearizes \eqref{eq:equilibrium} around steady state, evaluates the likelihood with a Kalman filter, and samples \eqref{eq:posterior} with random-walk Metropolis--Hastings or related MCMC methods. I keep the nonlinear structure at every stage. I solve \eqref{eq:equilibrium} globally using the stochastic extended path method on a design set $\{\theta^{(i)}\}_{i=1}^{N_\theta}$, train a neural network $\hat{\Delta}(s_{t-1}, \eps_t, \theta; w)$ to approximate the nonlinear-minus-linear transition residual, and sample the augmented posterior $p(\theta, s_0, \eps_{1:T} \given y_{1:T}^{\text{obs}})$ with Hamiltonian Monte Carlo.
...
The Gal\'i (2015) New Keynesian model is used as a negative-control model because it has Calvo pricing, a Taylor rule, and an occasionally binding ZLB but no investment or capital block. The Taylor rule is
\[
R_t = \max\{1, (R_{ss})^{1-\rho_R}(\Pi_t/\Pi_{ss})^{(1-\rho_R)\phi_\pi}(Y_t/Y_{ss})^{(1-\rho_R)\phi_y}R_{t-1}^{\rho_R}e^{\varepsilon_t^R}\},
\]
where the $\max\{1,\cdot\}$ operator enforces the ZLB. Appendix~\ref{sec:validation_design} reports a two-parameter hard-ELB validation package for the ROM1-residual, inversion-filter, and matched-HMC layers on the maintained Gal\'i OBC implementation; the full three-parameter direct-SEP HMC comparison remains a scaling target rather than a reported result. Full equations are in Appendix~\ref{app:model}.

The 18-parameter analysis uses SW07-HLT: habit formation, capital accumulation, investment adjustment costs, variable utilization, Kimball demand, and the ZLB. The specification has 66 variables (26 states), 7 shocks, and 18 estimated parameters covering shock processes and the price/wage setting block $(\xi_p, \iota_p, \varepsilon_p, \xi_w)$. See Appendix~\ref{app:sw07_model} for details.
...
The surrogate must be valid across the prior regions with posterior mass---not just near one mode---because the sampler can visit parameter configurations far from the training center during adaptation and mode exploration. I train on parameter draws from a Sobol sequence covering the bounded parameter space (Section~\ref{subsec:surrogate}), and FiLM conditioning (Appendix~\ref{app:resnet_film}) allows the network to adapt to each parameter configuration. The residual formulation also gives explicit control over approximation error: the error is directly measurable as the validation RRMSE and formally bounded in terms of posterior coverage distortion under the assumptions in Appendix~\ref{sec:formal_consistency}. At the realized non-binding validation RRMSE of 0.0012, the theoretical upper bound on coverage distortion is below one percentage point; the separate binding-region RRMSE is much larger and is treated as a lower-bound stress diagnostic rather than covered by this bound.

The procedure has two stages. \emph{Offline}: draw a parameter grid from prior support, solve the nonlinear system at each grid point, compute linear transitions at the same inputs, train the neural network on the residuals, and calibrate the regime-switching gate. \emph{Online}: for each MCMC proposal $\theta'$, recover shocks via an inversion filter, classify each period as calm or stressed based on the state's distance from steady state, evaluate the exact Kalman likelihood in calm periods and a penalized inversion objective in stressed periods, and accept or reject via HMC. Because the inversion step profiles over shocks without a reported Jacobian/Laplace determinant, I interpret the regime-switching values as comparable inversion log objectives rather than marginal likelihoods or Bayes factors. The full algorithm is in Appendix~\ref{app:hmc}.
...
\end{equation}
where $\tilde{R}_t$ is the unconstrained (``shadow'') Taylor rule rate from \eqref{eq:taylor_zlb_full} and $\kappa_s = 100$. At $\kappa_s = 100$, a shadow rate 200 basis points below the floor yields a softplus value $100^{-1}\log(1+e^{-2}) \approx 0.00127$ in gross-rate units, or about 12.7 basis points above the hard floor. The approximation is therefore a smooth lower-bound formulation rather than a numerically exact max operator. Its advantage is that the Jacobian exists everywhere and varies smoothly, avoiding the subdifferential complications that arise with $\max$. Figure~\ref{fig:zlb_binding_simulation} in Appendix~\ref{app:robustness_extended} illustrates the solver during a binding episode.
...
I recover shocks via an inversion filter rather than a particle filter. At each period, I solve for the shock vector that best fits the observed data under the linear model: $\hat{\eps}_t = \argmin_{\eps} \| (y_t^{\text{obs}} - Hg^{\text{ROM1}}(s_{t-1}, \eps, \theta))/\sigma_y \|^2 + \| \eps/\sigma_\eps \|^2$. This reduces the sampler's unknowns from $n_\theta + Tn_\eps$ to $n_\theta$ alone. The shocks are recovered under the linear model, but the stressed-period inversion objective uses the nonlinear surrogate prediction. The resulting object is a profiled Gaussian objective: it is useful for posterior comparison under a common inversion architecture, but it is not an exact marginal likelihood unless the omitted change-of-variables or Laplace determinant is added (Appendix~\ref{app:inversion}).
...
Structural parameters explain 11 percent of nonlinearity variation. The strongest correlates are government spending persistence $\rho_g$ ($+0.18$), risk-premium volatility $\sigma_b$ ($+0.17$), and Kimball curvature $\varepsilon_p$ ($-0.11$)---higher curvature flattens demand and makes the model \emph{more} linear (Appendix~\ref{app:additional_results}, Table~\ref{tab:param_nonlinearity}). Current-period shocks contribute negligibly ($R^2 = 0.0003$). Nonlinearity accumulates through persistent displacement, clustering in temporal bursts with autocorrelation 0.81 and mean duration 12.5 quarters; per-observable $R^2$ values remain below 12 percent (Appendix~\ref{app:additional_results}, Table~\ref{tab:per_obs_r2}).
...
\caption{Equation-block decomposition of the FOM--ROM1 gap across shock scales 0.1--1.5. Each point is the average block share across ten Mahalanobis-stratified posterior draws and all simulated periods at that scale. Shares are percentages of the total squared transition-function gap. Investment/capital (red) holds at 89--92 percent across the full shock-scale frontier; the consumption/Euler block (blue) accounts for the remaining 8--11 percent. All other blocks are below 2 percent at every scale.}
...
\item \textbf{Kimball curvature rises 2.7-fold ($\varepsilon_p$: 39 $\to$ 104).} Higher $\varepsilon_p$ reduces nonlinearity by flattening demand. The linear posterior concentrates on low $\varepsilon_p$ while the nonlinear posterior settles at higher values, directionally consistent with the theory but occurring in a weakly identified region (the nonlinear 90 percent credible interval [56.6, 149.6] covers much of the prior support). The sensitivity analysis in Section~\ref{subsec:csadjcost_sensitivity} supports the investment-dominance finding over $S''(1) \in [2, 10]$ in a bounded 25-draw design, with the investment share ranging from 89 to 95 percent.
\end{enumerate}

The 18-nat gap means some shifts could partly reflect surrogate error and the profiled inversion objective. Two independent linear chains converge to modes separated by 34 nats, so shifts should be read as mode-specific. A direct SEP comparison at posterior draws (Appendix~\ref{sec:identification}) shows that the ROM1--SEP prediction gap averages 1.04 RMSE per variable---well above the surrogate's training RMSE of 0.47---which supports the view that nonlinear solution differences are economically material at these parameter values.
...
\bottomrule
\end{tabular}
\tablenotes{\textit{Notes}: ``Kalman'' column reports posterior means from a linear NUTS chain (1,000 draws, AdvancedHMC.jl, zero divergences, ESS $\geq 501$) initialized with the same seed as the RS chain. These values differ from Table~\ref{tab:18param_posterior}, which reports a chain initialized from a different seed that converges to a different posterior mode (see convergence diagnostics in text). ``RS Surrogate'' reports posterior means from the regime-switching NUTS chain (1,000 draws, zero divergences, 2.4 hours on Apple M4). $\Delta$ is the difference in posterior means. RS 90\% CI is the 5th--95th percentile interval. Shock volatilities are quarterly innovation standard deviations in the model's transformed observable scale. Posterior-mean criterion values: Kalman log likelihood $-1{,}027$; regime-switching profiled objective $-1{,}045$. Sample period: 1959Q1--2004Q4 ($T=184$ quarters).}
\end{table}

\paragraph{Convergence diagnostics.} Neither chain produces divergent transitions; within-chain ESS exceeds 501 (linear) and 473 (regime-switching). Two independent linear chains find different posterior modes---$\rho_w$ settles at 0.39 versus 0.75, with a 34-nat log-likelihood gap---indicating ridged geometry in the wage/price markup block \citep{herbst2016sequential}. A second regime-switching chain initialized from a different point converges 12,700 nats below the primary chain, indicating severe multimodality. This is not unique to the surrogate: the 34-nat gap between linear chains shows that the SW07 posterior itself has ridged geometry, and the nonlinear correction amplifies the separation. A multi-start protocol with parallel tempering \citep{herbst2019tempered} would systematically explore competing modes; the results here should be read as conditional on the primary mode.
...
\bottomrule
\end{tabular}
\tablenotes{\textit{Notes}: Extended sample 1959Q1--2025Q1 ($T=265$). ``Kalman'' reports posterior means from linear NUTS-HMC (2,000 draws, AdvancedHMC.jl, zero divergences, ESS $\geq 668$). ``RS Surrogate'' pools 8{,}000 post-warmup draws from four warm-started NUTS-HMC chains (seeds 42, 5, 6, 7; 2{,}000 draws per chain) with soft gating and zero divergences across all chains; pooled ESS $\geq 90$ (Appendix~\ref{app:multi_chain_convergence}). $\Delta$ is the shift in posterior means. RS 90\% CI is the 5th--95th percentile of the pooled posterior. Shock volatilities are quarterly innovation standard deviations in the model's transformed observable scale. Posterior-mean criterion values: Kalman log likelihood $-1{,}817.59$; RS profiled objective $-1{,}781.68$ (gap $35.92$ nats). Per-chain surrogate posterior-mean objectives range from $-1{,}787.38$ to $-1{,}780.80$.}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/covid_shock_comparison.pdf}
\caption{Kalman-smoothed shocks (in $\sigma$ units), 2018--2025. Both series are evaluated through the same linear Kalman smoother; only the parameter vector differs. Blue dashed: linear posterior mean; red solid: surrogate posterior mean. Gray band: COVID window (2020Q1--2021Q2). At the linear posterior, risk-premium shocks peak at $56\sigma$. At the surrogate posterior, the lower $\sigma_b$ forces even larger normalized shocks under a linear filter, illustrating that the surrogate posterior parameters should be interpreted through the regime-switching nonlinear objective rather than through the Kalman smoother.}
...
\begin{table}[htbp]
\centering
\caption{Cross-evaluated log criteria at the linear and surrogate posterior means (extended sample, $T=265$).}
...
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: Fixed-parameter criterion evaluations on the extended sample. The no-NN row uses the same soft gate and inversion-filter objective as the surrogate row but zeros the neural residual correction. A full posterior ablation requires rerunning HMC with \texttt{--disable-nn-correction=true}.}
\end{table}

The extended-sample results are directionally consistent with the decomposition predictions. The theory predicts that the linear model's estimated shock processes may be inflated relative to the nonlinear model when the missing investment-channel amplification is quantitatively important. The baseline estimation on 1959--2004 is consistent with this qualitatively for some margins, though $\sigma_b$ rises rather than falls in the baseline; extending through 2025 provides a more demanding test where the direction of the $\sigma_b$ shift aligns with the prediction. The COVID window generates observations that the linear posterior mean rationalizes only with extreme shock realizations. The nonlinear model achieves a higher profiled objective with smaller estimated shocks, the largest reduction in $\sigma_b$ ($-49$ percent)---the shock that feeds most directly into the investment Euler equation.
...
Surrogate accuracy is strong in the non-binding validation design but weaker in the lower-bound stress region. Across most parameter regions the held-out RRMSE is below 0.002, while the far-tail 95th percentile is 0.00231 (Appendix~\ref{app:robustness_extended}, Table~\ref{tab:surrogate_regional_accuracy}). The combined surrogate (63,939 samples including 20,240 ZLB-binding episodes at shock scale 0.4) achieves RRMSE of 0.12 on binding-period test data---an 89 percent improvement over the version trained without ZLB episodes, but much larger than the non-binding validation error. Earlier Gal\'i seed and prior sensitivity tables are omitted until the direct Gal\'i validation comparison exists.

During ZLB-binding episodes, ROM1 predicts deeply negative rates; the surrogate adds back the approximate lower-bound correction and tracks the SEP benchmark qualitatively across the reported observables (Appendix~\ref{app:robustness_extended}, Figures~\ref{fig:zlb_correction_robs}--\ref{fig:zlb_correction_4panel}).
...
The baseline surrogate is trained at shock scale 0.1, where the ZLB rarely binds; the supplementary dataset at scale 0.4 extends coverage to binding episodes. The same surrogate can be evaluated on different sample periods without retraining because the neural network maps from parameter-state-shock space rather than time-series indices, but this does not eliminate out-of-distribution risk. The theoretical coverage-distortion bound from Corollary~\ref{cor:coverage} evaluates to below one percentage point at the non-binding validation RRMSE of 0.0012 (Appendix~\ref{sec:formal_consistency}); it should not be applied to the binding-region RRMSE of 0.12. A direct SEP comparison at twenty posterior draws shows that the ROM1--SEP prediction gap (average RMSE 1.04 per variable, exceeding $3\sigma_y$ for labor and the interest rate) is large relative to the non-binding surrogate training error.

To bridge the gap to the 18-parameter estimation, I provide two lines of current evidence: (i) the formal consistency theory shows coverage distortion scales as $\sqrt{T} \cdot \text{RRMSE}^2$ with spectral prefactor $\sqrt{\kappa}/\sqrt{\lambda_{\min}(\mathcal{I}(\theta_0))}$, bounding the theoretical distortion below one percentage point at the non-binding validation RRMSE; (ii) the twenty-draw SEP comparison provides an empirical full-scale solution check. A Monte Carlo coverage study at the full 18-parameter scale (Appendix~\ref{app:mc_coverage}) validates only the linear Kalman + NUTS pipeline: it achieves average coverage of 79.8\% across 14 replications, with the Clopper--Pearson 95\% confidence interval containing the nominal 90\% rate for 16 of 18 parameters. It is not evidence on surrogate posterior coverage. The direct Gal\'i SEP-HMC comparison is listed separately in Appendix~\ref{sec:validation_design}.

\subsection{Policy Implications}

Within the HLT specification studied here, the posterior comparison suggests that first-order estimation can overstate exogenous shock volatility when the missing investment-channel amplification is quantitatively important. The direct SEP comparison shows that the nonlinear correction exceeds $3\sigma_y$ for labor and the interest rate at selected posterior parameter values, suggesting that investment-channel nonlinearities deserve more attention in policy counterfactuals. Formal welfare analysis and cross-model validation are beyond this paper's scope. For model-building priorities, the results favor checking the investment block before treating the ELB or Kimball pricing as the dominant source of nonlinearity.


\section{Conclusion}
\label{sec:conclusion}

The main finding of this paper is that first-order perturbation misses the dominant source of nonlinearity in the Smets--Wouters model. The level investment adjustment cost satisfies $S(1) = S_x(1) = 0$, making the level resource-cost term invisible to linearization, while nonlinear products involving the marginal cost $S_x$ remain quantitatively important away from trend growth. This channel accounts for 69 percent of the gap between nonlinear and linear solutions; the price Phillips curve accounts for 0.2 percent. These are deterministic measurements, independent of MCMC or surrogate quality. In the bounded $S''(1)$ sensitivity exercise, investment dominance ranges from 89 to 95 percent, while the price Phillips share remains at 0.0 percent (Section~\ref{subsec:csadjcost_sensitivity}).

The posterior shifts are consistent with the theory, but the empirical claim is conditional. Extending the sample through 2025Q1, the high-likelihood warm-started surrogate mode achieves a better regime-switching profiled objective for COVID-era observations with smaller estimated shock volatilities, a pattern consistent with endogenous amplification rather than reliance on implausible shock draws. All seven shock volatilities fall (risk premium by 49 percent), persistence parameters rise ($\rho_b$: 0.73 $\to$ 0.90), and the posterior-mean objective gap is 35.92 nats. Within this model, the posterior geometry is consistent with the decomposition: the data strongly identify parameters entering the investment channel (pooled ESS for $\rho_b$ is $\approx 4{,}400$), while Kimball curvature is nearly flat (pooled ESS $= 90$).

The method matters for the substantive finding. The investment-dominance result, the identification asymmetry between $\rho_b$ and $\varepsilon_p$, the 35.92-nat profiled-objective gap, and the $56\sigma$ COVID shocks under the linear model require nonlinear Bayesian estimation at this scale. Relative to existing nonlinear estimation methods \citep{kase2025estimating,naubert2025differentiable}, the surrogate adds an auditable target: residual RRMSE is measured directly on $g^{\text{SEP}} - g^{\text{ROM1}}$. It also adds scalability, because the same trained surrogate serves multiple estimation samples, parameter configurations, and decomposition exercises without retraining. These properties expose the caveats: the posterior comparison remains conditional on one HLT specification, one country, the profiled inversion objective, the warm-started mode, the gate design, and COVID out-of-distribution behavior.

Appendix~\ref{app:mc_coverage} reports a Monte Carlo coverage study at the full 18-parameter scale: 14 replications of the linear estimator, each drawing true parameters from the prior and checking whether 90\% credible intervals contain the true values. Average coverage of 79.8\% is below the nominal 90\%, though the Clopper--Pearson 95\% confidence intervals contain the nominal rate for 16 of 18 parameters, and the shortfall is consistent with the short chains (300 post-warmup draws) used per replication. Multi-chain $\hat{R}$ diagnostics from 4 independent chains (Appendix~\ref{app:multi_chain_convergence}) show acceptable convergence for 15 of 18 linear parameters; the three non-converging parameters ($\rho_\pi$, $\sigma_\pi$, $\iota_p$) belong to the price Phillips curve block that contributes only 0.2\% of the nonlinear correction. For the surrogate estimator, warm-starting three additional chains from the best chain's posterior mean (seeds 5, 6, 7) resolves the initialization failure that caused all 18 parameters to show $\hat{R} > 1.1$ in the original four-chain diagnostic: the four warm-started chains now converge with max $\hat{R} = 1.05$ and pooled minimum ESS of 90 (Appendix~\ref{app:multi_chain_convergence}).

Three extensions follow naturally. First, extending the invisibility analysis to other models with investment adjustment costs \citep[e.g.,][]{christiano2005nominal,justiniano2010investment} would test whether investment dominance is a general property of medium-scale models. Second, pseudo-out-of-sample exercises in Appendix~\ref{app:oos_forecast} sharpen the gate-design caveat. Estimating both models on 1959Q1--2019Q4 and scoring 2020Q1--2025Q1, the switching surrogate reduces aggregate RMSE by 16 percent during the COVID window, with gains of 29 percent on output growth, 24 percent on consumption growth, and 26 percent on investment growth. But a quiet-sample pilot that estimates through 1994Q4 and evaluates 1995Q1--2007Q4 fails the calm-period criterion under the original gate: the switching surrogate's aggregate RMSE is 2.40 times ROM1, and the original hard gate is active throughout the holdout. A fixed-posterior q95 gate diagnostic reduces the aggregate ratio to 1.04 and the hard-gate share to 1.9 percent. Re-estimating the surrogate chain under the q95 padded gate gives a 1.09 aggregate ratio, improves the early quiet window to 0.70, and worsens the remaining window to 1.19. Gate calibration is therefore no longer an unlocalized failure, but it remains a central implementation risk. Third, Appendix~\ref{app:sep_sensitivity} reports a bounded 40-period SEP solver-sensitivity pilot: all tolerance/quadrature cells converge and the observation RMSE against the tight reference is below $10^{-2}$. The full 265-period sensitivity rerun remains a scaling task, so the headline decomposition still rests primarily on the deterministic shock-scale study.
...
This appendix presents the complete equilibrium system for the Gal\'i (2015) New Keynesian model with occasionally binding zero lower bound. It serves as a negative-control specification and as the source model for the hard-ELB validation package in Appendix~\ref{sec:validation_design}; the full three-parameter direct-SEP HMC comparison remains unreported. The 18-parameter analysis in Section~\ref{subsec:nonlinearity_sources} uses the richer \citet{smets2007shocks} specification, detailed in Appendix~\ref{app:sw07_model} below.
...
Table~\ref{tab:computational_cost} reports the runtime profile of the existing HLT three-parameter validation workflow. This is a pipeline-cost benchmark, not the Gal\'i direct-HMC validation described in Appendix~\ref{sec:validation_design}.
...
\]
The surrogate would need non-binding RRMSE exceeding about $0.0013$ before this conservative theoretical bound reached one percentage point at this sample size, so the realized value of $0.0012$ is close to the threshold rather than an order of magnitude below it. The bound applies worst-case RRMSE uniformly across all periods and all state dimensions. It does not cover the separate binding-region RRMSE of 0.12, and the 18-parameter Monte Carlo coverage study in Appendix~\ref{app:mc_coverage} validates only the linear Kalman + NUTS pipeline. The Gal\'i hard-ELB package supplies a passing two-parameter posterior diagnostic for the ROM1-residual/inversion/HMC pipeline, while the full three-parameter direct-SEP HMC comparison remains an unreported scaling target.
\end{remark}
...
The reduced HLT bridge varies four parameters, $(\rho_b,\rho_{qs},\sigma_b,\sigma_{qs})$, corresponding in the code to \texttt{crhob}, \texttt{crhoqs}, \texttt{z\_eb}, and \texttt{z\_eqs}. The support is trimmed to the region where direct SEP solves reliably: after support mapping, the maintained grid uses $z_{eb} \in [1.20,1.85]$ rather than the high-stress $z_{eb}=2.50$ edge. On the resulting 4-by-4-by-4-by-4 grid, all 256 direct-SEP cells solve, with SEP residual min/median/max equal to $2.384\times 10^{-12}$, $1.517\times 10^{-9}$, and $9.745\times 10^{-6}$. I then train an observation-space ROM1-residual surrogate on the supported grid and compare three posterior surfaces---direct SEP, ROM1, and ROM1 plus the learned residual---under the same Gaussian observation scale and the same held-out truth point from the validation split.

The HLT bridge passes the pre-specified finite-support comparison. Prediction RMSE relative to direct SEP falls from 0.1427 under ROM1 to 0.00564 under the surrogate, and log-posterior-surface RMSE falls from 86.48 to 0.36. Surrogate 90 percent intervals overlap direct SEP intervals for all four bridge parameters. The bridge is therefore the main direct evidence that the residual surrogate is learning the intended nonlinear correction in the medium-scale application. It remains a one-period, known-feature grid comparison, not a full multi-period inversion-filter or HMC certificate.

The more ambitious validation target is a direct SEP-HMC versus surrogate-HMC comparison on the Gal\'i synthetic model in Section~\ref{subsec:gali_model}. The DGP should use the same SEP solution and priors for both samplers, with identical data, seeds, and chain lengths. The comparison is acceptable if posterior means are within Monte Carlo error, 90 percent intervals materially overlap, and no systematic coverage distortion appears. The current package verifies this logic in a smaller hard-ELB OBC exercise; it does not yet complete the full three-shock SEP-HMC target.

I do not report the full requested Gal\'i three-parameter direct-SEP HMC comparison here. Existing Gal\'i diagnostics show that the SEP engine solves the model at a cached true parameter value, but the full three-parameter direct-SEP HMC run remains too computationally expensive for this draft. I therefore treat that exercise as a design target rather than as posterior-validating evidence. The replication package now includes narrower actual-floor diagnostics using the maintained Gal\'i OBC model. First, an adverse $\eps_z$ shock block generates a six-period floor episode with falling output and inflation, and a local known-shock grid over $\sigma_z$ confirms that a residual surrogate can reproduce the direct OBC posterior on that path. Second, a one-parameter inversion-grid check recovers shocks with the linear ROM1 inversion filter and evaluates the common recovered shocks under direct OBC and ROM1 plus the residual surrogate. That run has zero solver warnings, finite support at all 18 grid points, direct and surrogate posterior means of $0.050053$, and true-value coverage in both 90 percent intervals. Third, a matched one-parameter HMC smoke on the same inversion objective runs direct and surrogate NUTS with identical seeds, 30 post-warmup draws, zero numerical errors, overlapping intervals, true-value coverage, and a posterior-mean difference of 0.21 combined MCSE units. Fourth, a two-parameter matched-HMC check estimates $(\sigma_z,\sigma_a)$ on a 24-period actual-floor path with post-ELB identification blocks. In the extended four-chain run, the grid/training stage has zero solver warnings, actual-floor binding in 9 of 24 periods, finite support at all 36 grid points, and holdout residual RMSE approximately zero. The matched-HMC posterior comparison uses 4,000 post-warmup draws per objective; direct and surrogate 90 percent intervals overlap for both parameters, both true values are covered, and posterior-mean differences are $-0.45$ and $-0.46$ combined MCSE units for $\sigma_z$ and $\sigma_a$. The direct sampler records six numerical-error proposals out of 4,000 post-warmup draws, while the surrogate sampler records zero; the negligible-issue diagnostic and the overall validation flag both pass. Fifth, $\sigma_\nu$ probes are retained as expected identification-failure diagnostics: direct and surrogate objectives agree, but both push the parameter toward the local grid boundary, so the failure is not evidence of surrogate approximation bias. These are transition-map, inversion-layer, and sampler checks, not substitutes for the requested three-parameter direct-SEP HMC exercise.

I also maintain a bounded HLT direct-SEP/MH versus surrogate/MH harness targeting the SW07-HLT legacy three-parameter price block $(\xi_p,\iota_p,\varepsilon_p)$ rather than the Gal\'i shock-volatility vector $(\sigma_A,\sigma_\mu,\sigma_R)$. This is a diagnostic harness, not a posterior validation pass: the direct SEP side returns a finite objective, while the historical surrogate bundle is not a current-runtime posterior comparison.

I therefore do not report the missing full three-parameter direct posterior comparison here. Reproducibility details for the available diagnostics belong in the replication package rather than in the manuscript.
...
HLT support map & 256 of 256 supported investment-block direct-SEP cells solve; SEP residual max $9.745\times 10^{-6}$ \\
HLT posterior-grid bridge & Passes one-period held-out comparison: surrogate prediction RMSE falls from 0.1427 to 0.00564 and log-posterior-surface RMSE from 86.48 to 0.36 relative to direct SEP; all four surrogate intervals overlap direct SEP \\
Full Gal\'i three-parameter direct SEP-HMC & Not reported; remains a scaling target rather than a result in this draft \\
Full Gal\'i three-parameter surrogate-HMC comparison & Not reported on the same direct-SEP DGP, priors, seeds, and chain lengths \\
Full HLT inversion-filter/HMC bridge & Not reported; the current HLT bridge is a finite-support known-feature grid comparison \\
...
Table~\ref{tab:mode_sensitivity_chains} lists the cold-start and warm-start chain artifacts used for this mode-sensitivity accounting.

\begin{table}[htbp]
\centering
\caption{Mode Sensitivity Across Extended-Sample Chain Artifacts}
\label{tab:mode_sensitivity_chains}
\resizebox{\textwidth}{!}{\input{generated/table_mode_sensitivity.tex}}
\smallskip
...
\subsection{Omitted Validation Tables}

Earlier seed-sensitivity, prior-sensitivity, simplified 18-parameter validation, and sample-length recovery tables are omitted from this paper because the current replication package does not contain reproducible runs supporting those numerical claims. They should be restored only after the underlying runs are regenerated and documented. Appendix~\ref{sec:validation_design} tracks the passing hard-ELB Gal\'i package audit, the reduced HLT direct-SEP bridge, and the remaining full multi-period direct-SEP HMC scaling targets.

\subsection{Surrogate Architecture Comparison}

The architecture-comparison table previously reported posterior-mode errors from the omitted three-parameter recovery workflow. I omit it from this paper until the underlying run is regenerated. The current paper uses the architecture documented in the training materials and reports only the held-out accuracy and posterior consequences that have matching replication support.
