...
\hypersetup{
    ...
    pdftitle={Investment Adjustment Costs and the Nonlinear Posterior of Smets--Wouters}
}
...
\noindent
\footnotesize
First-order perturbation misses the dominant deterministic nonlinearity in the Smets--Wouters model studied here. Because the investment adjustment cost satisfies $S(1) = S'(1) = 0$, the first-order solution discards this channel at the approximation point. A stochastic-extended-path decomposition of the nonlinear-minus-linear solution gap assigns 69 percent of the gap to the investment and capital block and 0.2 percent to the price Phillips curve. Even in simulations with binding lower-bound episodes, investment remains the largest source of the nonlinear correction.

To estimate the nonlinear model on US data, I develop a neural network surrogate that learns the residual between nonlinear and linear transitions, coupled with a regime-switching filter that activates the nonlinear correction only in periods of large displacement from steady state. The residual formulation makes approximation error observable on validation data, and a formal consistency result gives conditions under which this error has limited posterior impact. Full Bayesian estimation via NUTS-HMC completes in 2.4 hours with zero divergent transitions in the baseline run.

The empirical results are model-specific rather than a generic proof of bias from linearization. On the 1959--2004 sample, nonlinear estimation is associated with higher risk-premium volatility and lower wage-markup persistence, consistent with the missing investment channel changing how shocks are allocated. On the 1959--2025 sample, a high-likelihood warm-started surrogate mode improves the posterior-mean log likelihood by 35.92 nats relative to the linear Kalman posterior mean, and the linear smoother requires risk-premium shocks of $56\sigma$ during COVID. The same comparison is conditional on one country, one model specification, a single posterior basin, the surrogate approximation, the gate design, and COVID out-of-distribution risk. A restored ARMA(1,1) markup specification improves the linear log likelihood by 34 nats on the baseline sample, comparable to the nonlinear gap, so the posterior shifts should be read as evidence relative to this nonlinear HLT benchmark rather than as a specification-invariant rejection of linear DSGE estimation. A quiet-sample forecast pilot fails under the original gate, while a no-HMC gate recalibration diagnostic reduces the aggregate RMSE ratio from 2.37 to 1.04; this substantially narrows, but does not close, the calm-period gate-design risk.
\end{abstract}
...
The standard motivation for nonlinear DSGE estimation is the zero lower bound on nominal interest rates. The effective lower bound (ELB) creates a kink that first-order perturbation cannot represent, and the episodes where it binds---2009--2015 in the United States, 2020--2022---are among the most policy-relevant in recent macroeconomic history. A large literature has developed nonlinear methods specifically to handle this constraint \citep{gust2012effects,boehl2022monetary,guerrieri2015occbin}. In the Smets--Wouters specification studied here, however, the dominant measured nonlinearity is elsewhere.

In an 18-parameter Smets--Wouters model with occasionally binding constraints, I solve the full nonlinear system at 22,080 state--shock--parameter combinations and compare each solution against the corresponding first-order perturbation output. The investment and capital block accounts for 69 percent of the total gap between nonlinear and linear dynamics; the price Phillips curve---where the Kimball aggregator enters and which has received substantial attention in the nonlinear DSGE literature \citep{gust2012effects,boehl2022monetary,fernandezvillaverde2021estimating}---accounts for 0.2 percent. Even during ELB-binding episodes, the investment channel dominates the next-largest block by a factor of eight (Figure~\ref{fig:decomposition_vs_scale}).

The reason is analytical. The investment adjustment cost $S(I_t/I_{t-1}) = \tfrac{S''(1)}{2}(\gamma I_t/I_{t-1} - \gamma)^2$ (where $S''(1) \equiv \phi$ denotes the curvature at steady state) satisfies $S(1) = 0$ (the cost vanishes at trend growth) and $S'(1) = 0$ (the marginal cost is zero at trend growth). First-order perturbation therefore sets both $S$ and $S'$ to zero---the entire adjustment cost is invisible to linearization. I formalize this as a general \textit{perturbation invisibility} property (Proposition~\ref{prop:invisibility}): any function satisfying $f(\bar{x}) = f'(\bar{x}) = 0$ at steady state is invisible to first-order methods.

Three diagnostics point away from the usual alternatives. First, the equation-block decomposition shows investment at 69 percent and pricing at 0.2 percent (Table~\ref{tab:equation_block}). Second, a Gal\'i (2015) model with the same ELB and Kimball aggregation but no investment shows near-zero nonlinearity at empirically relevant shock scales (Figure~\ref{fig:gali_obc_vs_no_obc})---same ELB, same Kimball structure, different investment block. Third, removing $S(\cdot)$ from the model causes the gap to \emph{explode} (the adjustment cost simultaneously creates the gap through invisibility and limits its magnitude through dampening), while linearizing the utilization cost $a(z)$ changes the gap by less than 1.5 percent.

If linearization discards an important investment-channel amplification, the linear posterior may compensate through the shock processes and persistence parameters that load on investment dynamics. On US data (1959Q1--2004Q4), switching from linear to nonlinear estimation is associated with a doubling of risk-premium volatility $\sigma_b$, a compression of wage markup persistence $\rho_w$ from 0.37 to 0.08, and a 2.7-fold rise in Kimball curvature $\varepsilon_p$ (Table~\ref{tab:18param_switching_posterior}). These shifts are consistent with the invisibility mechanism, but they are conditional on the HLT model, the chosen priors, and the surrogate likelihood.

Extending the sample through 2025Q1---to include the two modern ELB episodes where the standard argument for nonlinear estimation is strongest---is more revealing but also more fragile. The linear model attributes the COVID-19 collapse to risk-premium shocks exceeding $56\sigma$, price markup shocks of $32\sigma$, and monetary shocks of $23\sigma$ under its own smoother. The high-likelihood warm-started surrogate mode improves the posterior-mean log likelihood by 35.92 nats ($-1{,}781.68$ versus $-1{,}817.59$) and shifts the posterior toward lower shock volatilities and higher investment-related persistence. This comparison is conditional on a single posterior basin, the surrogate approximation, the gate design, and COVID out-of-distribution risk.

A specification caveat is equally important. The HLT baseline suppresses the ARMA(1,1) price- and wage-markup shock terms used in the original \citet{smets2007shocks} specification. This is the same block where \citet{farkas2020bayesian} document a flat posterior target in the full Smets--Wouters model. Restoring those MA terms on the baseline sample improves the linear log likelihood by 34 nats (Appendix~\ref{app:hlt_vs_ft}), almost the same scale as the 35.92-nat nonlinear gap in the extended-sample comparison. The nonlinear posterior shifts therefore should not be read as a specification-invariant measurement of linearization bias. They show that, relative to the HLT benchmark used to train the surrogate, the nonlinear likelihood favors a different posterior allocation of COVID and investment dynamics.

The policy implication is correspondingly narrow. For medium-scale New Keynesian models with investment adjustment costs and occasionally binding constraints, linear estimates may attribute too much variation to exogenous shocks when the missing investment channel is quantitatively important. For researchers choosing which nonlinearities to include in estimated models of this class, the deterministic decomposition favors prioritizing the investment block over the ELB and Kimball pricing. The posterior comparison strengthens that case within the current model but does not by itself establish global posterior dominance across alternative specifications.

To estimate the nonlinear model, I develop a neural network surrogate coupled with a regime-switching filter. The neural network learns the residual between the full nonlinear solution and the linear baseline---a target two orders of magnitude smaller than the full transition. A regime-switching gate classifies each sample period as calm or stressed based on the economy's distance from steady state, applying the nonlinear correction only where it matters and using the exact Kalman filter elsewhere. I sample the posterior via Hamiltonian Monte Carlo with the No-U-Turn Sampler. A bootstrap particle filter---the standard nonlinear alternative---collapses to a single effective particle in every period at all particle counts tested; at $N_p=5{,}000$, its log-likelihood magnitude is roughly four orders worse than the Kalman value (Table~\ref{tab:pf_benchmark}). The filter-free formulation sidesteps this degeneracy by treating shocks as explicit unknowns.

Two properties distinguish this approach from other recent surrogate methods \citep{kase2025estimating,naubert2025differentiable}. First, approximation error is directly measured: because the network learns the residual $g^{\text{SEP}} - g^{\text{ROM1}}$, validation RRMSE is an error on the nonlinear correction rather than on the full transition, and Appendix~\ref{sec:formal_consistency} states the assumptions under which this maps into posterior distortion. Second, the surrogate is trained once on parameter draws spanning the full prior support and reused across estimation samples without retraining. These properties make the decomposition exercise auditable, but they do not eliminate out-of-distribution risk in the COVID window. Online estimation with 1,000 NUTS draws completes in 2.4 hours on an Apple M4 with zero divergent transitions, after a 12-hour offline training stage.

The decomposition results in Sections~\ref{sec:invisibility}--\ref{sec:decomposition} are deterministic, computed from the nonlinear solver without MCMC. The posterior comparison in Section~\ref{sec:inference_consequences} is conditional on a single model, a single US sample, one high-likelihood warm-started surrogate mode, the surrogate approximation, and the gate design. The out-of-sample evidence is mixed: the switching surrogate improves COVID-window forecasts but fails a quiet-sample 1995Q1--2007Q4 pilot under the original gate, where aggregate RMSE is 2.37 times ROM1. A stricter q95 gate diagnostic using the same posterior draws reduces the ratio to 1.04 and hard-gate activation to 1.9 percent, showing that the failure is largely gate calibration rather than a settled rejection of the nonlinear transition. Because this diagnostic does not re-estimate the posterior and still leaves ROM1 slightly ahead, gate calibration remains a first-order pre-submission task.

The paper draws on three literatures. For the nonlinear solver, I use the stochastic extended path method \citep{fair1983solution,adjemian2025stochastic}, which handles occasionally binding constraints natively and scales to medium-scale models, unlike projection \citep{judd1992projection} or sparse grids \citep{krueger2004computing,winschel2010solving}; \citet{richter2017nonlinear} provides a survey of nonlinear solution methods with OBC. For the surrogate, I build on multi-fidelity methods from engineering \citep{peherstorfer2018survey} and the model-discrepancy framework of \citet{kennedy2001bayesian}. The residual formulation is related to \citet{koop2022bayesian} and \citet{naubert2025differentiable} but approximates the transition map rather than the likelihood; compared to \citet{kase2025estimating}, it learns a two-orders-of-magnitude smaller target and supplies smooth gradients for HMC. \citet{janssens2025efficient} achieve large speed gains via time-varying local linearization, but their first-order Taylor expansion cannot capture the $S' \times p^k \times \xi_{t+1}/\xi_t$ cross-terms that drive the investment-channel nonlinearity. For sampling, I extend the filter-free HMC framework of \citet{childers2022filter} and \citet{farkas2020bayesian} to the nonlinear setting. This sidesteps the particle-filter weight degeneracy documented in \citet{herbst2019tempered} and \citet{cubaborda2019likelihood}. Recent nonlinear estimation work---\citet{gust2012effects}, \citet{boehl2022monetary}, \citet{fernandezvillaverde2021estimating}, \citet{basu2017uncertainty}---faces either this degeneracy or the restriction to piecewise-linear approximations. The theoretical underpinnings of likelihood-based inference with computed nonlinear solutions are developed in \citet{fernandezvillaverde2006convergence}, whose convergence results motivate the formal consistency theory in Appendix~\ref{sec:formal_consistency}.
...
The Smets--Wouters model with the Holden--Lind\'e--Trabandt extension contains seven sources of nonlinearity. I rank them by the quadratic-to-linear (Q/L) ratio: the second-order Taylor term divided by the first-order term, evaluated at one standard deviation of the relevant variable. A higher ratio means a larger share of the function's effect comes from curvature that linearization discards.

The investment first-order condition dominates: Q/L = 18.7\% at $1\sigma$. Because $S'(1) = 0$, the entire $S'$ effect is nonlinear---there is no first-order term. Linearization does not undercount this nonlinearity; it misses it completely.
...
Standard DSGE estimation linearizes \eqref{eq:equilibrium} around steady state, evaluates the likelihood with a Kalman filter, and samples \eqref{eq:posterior} with random-walk Metropolis--Hastings. I keep the nonlinear structure at every stage. I solve \eqref{eq:equilibrium} globally using the stochastic extended path method on a design set $\{\theta^{(i)}\}_{i=1}^{N_\theta}$, train a neural network $\hat{g}(s_{t-1}, \eps_t, \theta; w)$ to approximate the transition map, and sample the augmented posterior $p(\theta, s_0, \eps_{1:T} \given y_{1:T}^{\text{obs}})$ with Hamiltonian Monte Carlo.

Instead of integrating out the shocks via a particle filter, I treat them as explicit unknowns:
\begin{equation*}
...
\end{equation*}
where $s_t = \hat{g}(s_{t-1}, \eps_t, \theta; w)$ is the surrogate-predicted state and $p(\eps_t) = \mathcal{N}(0, I_{n_\eps})$ is the shock prior. This filter-free formulation samples shocks jointly with the structural parameters, avoiding the particle-weight degeneracy that plagues medium-scale models.

\subsection{Illustrative Models}
\label{subsec:gali_model}

The Gal\'i (2015) New Keynesian model is used as a negative-control model because it has Calvo pricing, a Taylor rule, and an occasionally binding ZLB but no investment or capital block. The Taylor rule is $R_t = \max\{ 1, \, (R_{ss})^{1-\rho_R} (\Pi_t/\Pi_{ss})^{(1-\rho_R)\phi_\pi} (Y_t/Y_{ss})^{(1-\rho_R)\phi_y} R_{t-1}^{\rho_R} e^{\varepsilon_t^R} \}$, where the $\max\{1,\cdot\}$ operator enforces the ZLB. The direct SEP-HMC versus surrogate-HMC validation on this model is a pre-submission requirement rather than a result reported in this draft; Appendix~\ref{sec:validation_design} records the runnable protocol, the implemented HLT fallback harness, and the current direct-SEP runtime blocker. Full equations are in Appendix~\ref{app:model}.
...
The estimator combines three tools at different accuracy levels: first-order perturbation (ROM1, microseconds), a neural network surrogate (milliseconds), and the full nonlinear SEP solver (0.8 seconds). The neural network does not learn the full nonlinear transition. Instead, it learns the \textit{residual} between the nonlinear and linear solutions, $\Delta g = g_\theta^{\text{SEP}} - g_\theta^{\text{ROM1}}$. At inference, the surrogate prediction adds this correction to the linear baseline: $\hat{g}(s,\varepsilon,\theta) = g_\theta^{\text{ROM1}}(s,\varepsilon) + \hat{\Delta}(s,\varepsilon,\theta; w)$. Because the linear solution already captures 99 percent of the dynamics near steady state, the residual is two orders of magnitude smaller than the full transition, concentrating the network's capacity on the nonlinear correction \citep{peherstorfer2018survey}. When the correction is zero, the estimator reduces to the standard linear model.

The surrogate must be valid across the full prior support---not just near a posterior mode---because the sampler explores the entire parameter space during estimation. I train on parameter draws from a Sobol sequence covering the bounded parameter space (Section~\ref{subsec:surrogate}), and FiLM conditioning (Appendix~\ref{app:resnet_film}) allows the network to adapt to each parameter configuration. The residual formulation also gives explicit control over approximation error: the error is directly measurable as the validation RRMSE and formally bounded in terms of posterior coverage distortion (Appendix~\ref{sec:formal_consistency}). At the realized RRMSE of 0.0012, the theoretical upper bound on coverage distortion is below one percentage point.

The procedure has two stages. \emph{Offline}: draw a parameter grid from prior support, solve the nonlinear system at each grid point, compute linear transitions at the same inputs, train the neural network on the residuals, and calibrate the regime-switching gate. \emph{Online}: for each MCMC proposal $\theta'$, recover shocks via an inversion filter, classify each period as calm or stressed based on the state's distance from steady state, evaluate the likelihood under the linear model (calm periods) or the surrogate (stressed periods), and accept or reject via HMC. The full algorithm is in Appendix~\ref{app:hmc}.
...
A single SEP solve takes about 0.8 seconds. Embedding SEP directly in MCMC would require 30--60 evaluations per NUTS leapfrog step, making 1,000 draws infeasible (100--200 days). The surrogate pays the SEP cost once offline and evaluates in milliseconds during inference---a five-order-of-magnitude speedup that makes Bayesian estimation tractable.
...
A natural question is why SEP rather than a higher-order perturbation benchmark. The object of this paper is the empirical gap between the standard first-order likelihood used in Smets--Wouters estimation and the fully nonlinear model. SEP is the appropriate reference object for that comparison. It evaluates the nonlinear equilibrium equations directly, permits state-dependent impulse responses, and handles the smooth occasionally binding lower-bound formulation in \eqref{eq:softplus_zlb}. A second-order perturbation could be useful in a separate numerical accuracy study, but it is not the estimator used in the linear DSGE literature and it does not replace the ROM1-versus-SEP comparison. For a comprehensive treatment of perturbation limitations, see \citet{aruoba2006comparing,kimetal2008pruning,andreasen2018pruning,fernandez2016solution}.


\subsection{Dataset Generation and Surrogate Training}
\label{subsec:surrogate}

The SEP transition map is deterministic: the same inputs always produce the same outputs. The problem reduces to supervised learning. I select a design set $\{\theta^{(i)}\}_{i=1}^{N_\theta}$ using a Sobol sequence ($N_\theta \approx 120$ in the 18-parameter case), simulate 200 periods at each design point, and store the labeled pairs, yielding 22,080 usable samples.

I train the network to predict the \textit{residual} between SEP and ROM1:
\begin{equation*}
\Delta y_j = g_\theta^{\text{SEP}}(x_j) - g_\theta^{\text{ROM1}}(x_j).
\end{equation*}
At inference, the surrogate prediction is $\hat{y} = g^{\text{ROM1}}(x) + \hat{\Delta}(x; w)$. Because ROM1 is highly correlated with the full solution ($\rho > 0.99$ near steady state), the residual is 50--100$\times$ smaller than the full transition, concentrating the network's capacity on the nonlinear correction. The surrogate achieves 96--99 percent improvement over ROM1 per output dimension on held-out data, with RRMSE $\approx 0.0012$. I augment the training data with state trajectories recovered from observed data via the linear filter, reducing the maximum standardized distance from 6.2 to 1.5. Architecture, training, and hyperparameter details are in Appendices~\ref{app:surrogate} and~\ref{app:shock_scale_calibration}.

\subsection{Regime-Switching Gate and Inversion Filter}
\label{subsec:regime_switching}

Not every period needs the nonlinear correction. I classify each period as calm or stressed using the state's distance from steady state through the gate indicator $g_t = \mathbb{1}(\|s_t - \bar{s}(\theta)\| > \tau)$ (distinct from capital utilization $z_t$ in the SW07 notation). Calm periods use the linear transition $g_\theta^{\text{ROM1}}$; stressed periods use the surrogate $\hat{g}_\theta$:
\begin{equation*}
s_t = (1-g_t)\,g_\theta^{\text{ROM1}}(s_{t-1}, \eps_t) + g_t\,\hat{g}_\theta(s_{t-1}, \eps_t).
\end{equation*}
Because displacement from steady state explains 96 percent of the nonlinear correction (Section~\ref{subsec:nonlinearity_sources}), this simple threshold closely approximates the oracle ranking of periods.

I recover shocks via an inversion filter rather than a particle filter. At each period, I solve for the shock vector that best fits the observed data under the linear model: $\hat{\eps}_t = \argmin_{\eps} \| (y_t^{\text{obs}} - Hg^{\text{ROM1}}(s_{t-1}, \eps, \theta))/\sigma_y \|^2 + \| \eps/\sigma_\eps \|^2$. This reduces the sampler's unknowns from $n_\theta + Tn_\eps$ to $n_\theta$ alone. The shocks are recovered under the linear model, but the likelihood for stressed periods uses the nonlinear surrogate; this inconsistency is bounded by the ROM1--surrogate gap (RRMSE $< 0.002$; Appendix~\ref{app:inversion}).

\subsection{Filter-Free Hamiltonian Monte Carlo}
\label{subsec:hmc}

I sample the posterior using the No-U-Turn Sampler (NUTS), a variant of Hamiltonian Monte Carlo \citep{hoffman2014nuts}. In low-dimensional validation settings, shocks $\eps_{1:T}$ and the initial state $s_0$ can be sampled jointly with $\theta$; in the 18-parameter estimation, the inversion filter recovers shocks analytically, so NUTS samples $\theta$ alone and gradients are computed via finite differences. The linear baseline completes in 100 minutes; the regime-switching surrogate in 2.4 hours---both with zero divergent transitions. Details are in Appendix~\ref{app:hmc}.
...
Table~\ref{tab:equation_block} delivers the central result: the investment and capital block accounts for 68.9 percent of the total gap, while the price Phillips curve contributes just 0.2 percent. The mechanism involves three interacting nonlinear functions in the investment block (Appendix~\ref{app:sw07_model}): the exponential capital utilization cost $a(z_t)$, the quadratic investment adjustment cost $S(I_t/I_{t-1})$, and Tobin's $q$ dynamics. When a negative investment-specific shock depresses the shadow price of capital $p^k$, the adjustment cost for future recovery rises---a compounding $S' \times p^k \times \xi_{t+1}/\xi_t$ interaction that linearization replaces with a constant coefficient. The Gal\'i (2015) model, which lacks investment entirely, shows small gaps even at $5\sigma$, consistent with this channel being the dominant source.
...
Structural parameters explain 11 percent of nonlinearity variation. The strongest correlates are government spending persistence $\rho_g$ ($+0.18$), risk-premium volatility $\sigma_b$ ($+0.17$), and Kimball curvature $\varepsilon_p$ ($-0.11$)---higher curvature flattens demand and makes the model \emph{more} linear (Appendix~\ref{app:additional_results}, Table~\ref{tab:param_nonlinearity}). Current-period shocks contribute negligibly ($R^2 = 0.0003$). Nonlinearity accumulates through persistent displacement, clustering in temporal bursts with autocorrelation 0.81 and mean duration 12.5 quarters (Appendix~\ref{app:additional_results}, Table~\ref{tab:per_obs_r2}).

These patterns predict testable posterior shifts. Under nonlinear estimation, $\sigma_b$ should rise (endogenous amplification substitutes for exogenous risk-premium variation), $\rho_w$ should fall (nonlinear propagation substitutes for exogenous persistence), and $\varepsilon_p$ should rise (the investment channel reduces the pricing block's role). The 18-parameter results (Section~\ref{par:results_18param_switching}) are consistent with all three predictions: $\sigma_b$ doubles, $\rho_w$ falls 80 percent, $\varepsilon_p$ rises 2.7-fold.


\subsection{State-Dependent Impulse Responses}
\label{subsec:irf_comparison}

Under ROM1, impulse responses scale linearly and are symmetric in the sign of the shock. Under SEP, neither property holds. For investment-specific shocks at magnitudes from $1\sigma$ to $5\sigma$, the solutions coincide at small shocks but diverge substantially by $5\sigma$. Positive shocks are amplified, consistent with the quadratic adjustment cost $S$ compounding with the consumption-savings wedge; negative shocks are asymmetrically attenuated, consistent with the convex utilization cost $a(z)$ dampening downward adjustment. The risk premium shock shows the same pattern through the Tobin's $q$ channel; the price markup shock shows minimal divergence even at $5\sigma$ (Appendix~\ref{app:additional_results}). The previous six-panel investment-shock figure is omitted from this draft because one panel was empty in the generated PDF.
...
\caption{ROM1 one-step-ahead forecast errors for the real-data diagnostic sample. Top: investment growth; middle: output growth; bottom: inflation. Shaded areas: NBER recessions. Dashed red lines: $\pm 2\sigma$ bands.}
...
\tablenotes{\textit{Notes}: Crisis quarters defined as periods where the absolute investment growth innovation exceeds $2\sigma$ of its unconditional standard deviation (13 out of 184 quarters, concentrated in the 1974--75, 1980--82, and 2001 recessions). Normal quarters are all remaining periods. Variance share is each observable's contribution to total one-step-ahead forecast error variance across all 184 quarters. ROM1 inversion filter applied to US quarterly data (1959Q1--2004Q4, 184 quarters) at calibrated \citet{smets2007shocks} parameter values.}
\end{table}

The decomposition measures transition-function gaps (69 percent from investment); forecast errors measure one-step-ahead prediction failures (53 percent from investment). These are different objects---the numerical proximity is coincidental---but both point to the same channel. I therefore keep the benchmark comparison focused on the economically relevant choice in estimation practice: ROM1 versus the fully nonlinear SEP solution.


\subsection{The ELB as Alternative Hypothesis}
\label{subsec:elb_alternative}

If the ELB were the dominant nonlinearity, the investment-block share should decline when the constraint binds frequently. It does not. The investment block accounts for 89--92 percent of the total gap at every shock scale from 0.1 to 1.5 (Figure~\ref{fig:decomposition_vs_scale}). The composition is flat across the full range, including the stress region where the ZLB first binds ($\geq 1.2\sigma$).
...
During ZLB-binding episodes the investment share falls---to 90 percent at shock scale 1.2 and 37 percent at scale 1.5---but ZLB binding is extremely rare at posterior-estimated parameters (0.1--0.4 percent of periods), so the unconditional composition remains investment-dominated at 88--92 percent (Appendix~\ref{app:additional_results}, Figure~\ref{fig:gap_magnitude_vs_scale}). The 88--92 percent figure exceeds the training-set average of 69 percent because Sobol-sampled draws span parameter regions where the investment channel is weaker. Both exercises point the same way: investment dominates, and the ELB remains subordinate.


\subsection{Gal\'i Negative Control}
\label{subsec:gali_control}

The \citet{gali2015monetary} model shares the ELB and Kimball nonlinearities with the HLT model but lacks investment---a difference-in-differences design. If the ELB or Kimball pricing were the dominant nonlinearity, the Gal\'i model should show comparable gaps. It does not: the gap is an order of magnitude smaller at every shock scale (Appendix~\ref{app:additional_results}, Figure~\ref{fig:gali_block_decomposition}).
...
I construct three counterfactual models to verify the analytical hierarchy from Section~\ref{sec:invisibility}: one removes $S(\cdot)$ entirely, one replaces $a(z)$ with its first-order Taylor expansion, and one linearizes both.

Removing $S(\cdot)$ does not shrink the gap---it \emph{diverges}, growing 30--90 times larger across shock scales from 0.1 to 1.5. The adjustment cost simultaneously creates the gap (through invisibility) and limits its magnitude (by dampening investment volatility). Without that convex friction, investment jumps freely, amplifying every other nonlinearity through general equilibrium. Linearizing $a(z)$ alone changes the gap by less than 1.5 percent, confirming that capital utilization nonlinearity is quantitatively irrelevant. The residual gap when both are linearized---from Tobin's~$q$ interactions, Kimball aggregation, and non-separable marginal utility---accounts for the 8--11 percent assigned to the consumption/Euler block (Figure~\ref{fig:decomposition_vs_scale}).
...
The local validation harness currently verifies the HLT surrogate pipeline on synthetic data and contains a bounded direct-SEP/MCMC comparison script for the HLT legacy three-parameter block. A previous current-runtime failure in the direct SEP inversion filter was traced to finite-difference perturbations being smaller than the SEP solve tolerance; tightening the prediction tolerance inside the inversion Jacobian restores finite exact-determinant HLT direct-SEP smoke evaluations. A direct Gal\'i SEP-HMC versus surrogate-HMC posterior comparison nevertheless remains pending (Appendix~\ref{sec:validation_design}), and I do not use the Gal\'i recovery exercise as evidence in this draft. I now turn to the 18-parameter estimation on US data.
...
\begin{enumerate}
\item \textbf{Risk-premium volatility doubles ($\sigma_b$: 0.12 $\to$ 0.28).} The linear model cannot generate endogenous amplification from the invisible investment block. To match observed investment volatility, it must rely on the exogenous risk-premium shock. Restoring the nonlinear $S' \times p^k \times \xi_{t+1}/\xi_t$ channel is consistent with $\sigma_b$ moving back toward its structural value.

\item \textbf{Wage markup persistence collapses ($\rho_w$: 0.37 $\to$ 0.08).} The linear model's posterior $\rho_w$ is comparable in magnitude to the persistence of the nonlinear propagation channel (autocorrelation 0.81, mean burst length 12.5 quarters), a pattern consistent with the linear model relying on persistent wage markup shocks where the nonlinear model uses state-dependent amplification. Restoring the nonlinear channel is associated with a reduction in exogenous persistence, though the absence of the ARMA(1,1) moving-average terms (see Appendix~\ref{app:ft_comparison}) may also contribute to the low $\rho_w$ estimate. The baseline uncertainty from bimodality in $\rho_w$ (two linear chains find 0.39 vs.\ 0.75) means the magnitude should be interpreted cautiously.

\item \textbf{Kimball curvature rises 2.7-fold ($\varepsilon_p$: 39 $\to$ 104).} Higher $\varepsilon_p$ reduces nonlinearity by flattening demand. The linear posterior concentrates on low $\varepsilon_p$ while the nonlinear posterior settles at higher values, directionally consistent with the theory but occurring in a weakly identified region (the nonlinear 90 percent credible interval [56.6, 149.6] covers much of the prior support). The sensitivity analysis in Section~\ref{subsec:csadjcost_sensitivity} confirms that the investment-dominance finding is robust to $S''(1) \in [2, 10]$, with the investment share ranging from 89 to 95 percent.
\end{enumerate}

The 18-nat gap means some shifts could partly reflect surrogate error, though the RRMSE-based coverage scaling implies negligible distortion (Appendix~\ref{sec:formal_consistency}). Two independent linear chains converge to modes separated by 34 nats, so shifts should be read as mode-specific. A direct SEP comparison at posterior draws (Appendix~\ref{sec:identification}) confirms that the ROM1--SEP prediction gap averages 1.04 RMSE per variable---well above the surrogate's training RMSE of 0.47---indicating that the shifts reflect genuine nonlinear effects.
...
\bottomrule
\end{tabular}
\tablenotes{\textit{Notes}: ``Kalman'' column reports posterior means from a linear NUTS chain (1,000 draws, AdvancedHMC.jl, zero divergences, ESS $\geq 501$) initialized with the same seed as the RS chain. These values differ from Table~\ref{tab:18param_posterior}, which reports a chain initialized from a different seed that converges to a different posterior mode (see convergence diagnostics in text). ``RS Surrogate'' reports posterior means from the regime-switching NUTS chain (1,000 draws, zero divergences, 2.4 hours on Apple M4). $\Delta$ is the difference in posterior means. RS 90\% CI is the 5th--95th percentile interval. Log-likelihood at posterior mean: Kalman $-1{,}027$; Regime-switching $-1{,}045$. Sample period: 1959Q1--2004Q4 ($T=184$ quarters).}
\end{table}

\paragraph{Convergence diagnostics.} Neither chain produces divergent transitions; within-chain ESS exceeds 501 (linear) and 473 (regime-switching). Two independent linear chains find different posterior modes---$\rho_w$ settles at 0.39 versus 0.75, with a 34-nat log-likelihood gap---indicating ridged geometry in the wage/price markup block \citep{herbst2016sequential}. A second regime-switching chain initialized from a different point converges 12,700 nats below the primary chain, confirming multimodality. This is not unique to the surrogate: the 34-nat gap between linear chains shows that the SW07 posterior itself has ridged geometry, and the nonlinear correction amplifies the separation. A multi-start protocol with parallel tempering \citep{herbst2019tempered} would systematically explore competing modes; the results here should be read as conditional on the primary mode.

\paragraph{Scope limitations.} The baseline training data use shock scale $\sigma = 0.1$, at which the ZLB rarely binds. A supplementary dataset at $\sigma = 0.4$ adds 20,240 ZLB-binding samples, achieving RRMSE $= 0.12$ on binding episodes (Figures~\ref{fig:zlb_correction_robs}--\ref{fig:zlb_correction_4panel}). The OOD detection mechanism (Appendix~\ref{app:ood}) falls back to ROM1 for states outside the training envelope, but the surrogate's extrapolation behavior at intermediate out-of-distribution states remains a source of unquantified error. Section~\ref{subsec:extended_sample} extends both estimations through 2025Q1.
...
A bootstrap particle filter \citep{gordon1993novel} collapses to a single effective particle in every period at all particle counts tested ($N_p$ up to $5{,}000$), producing a log-likelihood magnitude roughly four orders worse than the exact Kalman value at $N_p=5{,}000$ (Table~\ref{tab:pf_benchmark}). In a 7-dimensional observation space, the number of particles required for reliable importance sampling grows exponentially \citep{snyder2008obstacles}. Advanced particle filters---tempered \citep{herbst2019tempered}, conditionally optimal \citep{aruoba2021piecewise}---alleviate degeneracy in smaller models but remain far below the Kalman benchmark in the artifacted comparison for this model. The filter-free formulation sidesteps this problem by treating shocks as explicit unknowns.

The offline cost of the surrogate is 12 hours (10 hours dataset generation, 2 hours training). The same surrogate serves multiple estimation runs without retraining, since the transition map $g_\theta(s, \varepsilon)$ does not depend on observed data. A 1,000-draw NUTS run achieves a $2{,}450\times$ speedup over direct SEP evaluation.
...
Table~\ref{tab:extended_sample_posterior} reports the results. Under the linear Kalman filter, extending through 2025Q1 produces large COVID-driven shifts: shock volatilities rise ($\sigma_a$: 0.50 to 0.72, $\sigma_{qs}$: 0.36 to 0.52), investment persistence falls ($\rho_{qs}$: 0.78 to 0.65), and monetary persistence doubles ($\rho_{ms}$: 0.18 to 0.38)---reflecting the Federal Reserve's aggressive COVID-era response. The Kalman smoother at the linear posterior mean recovers risk-premium shocks of $56\sigma$, price markup shocks of $32\sigma$, and monetary shocks of $23\sigma$ during 2020Q1--2021Q2 (Figure~\ref{fig:covid_shock_comparison}). These shock magnitudes are a diagnostic of model strain, not by themselves a proof that the nonlinear posterior is globally preferred.

The regime-switching surrogate improves the posterior-mean log likelihood by 35.92 nats ($-1{,}781.68$ versus $-1{,}817.59$; pooled posterior mean across four warm-started chains). This is a large likelihood difference, but it is conditional on the high-likelihood warm-started mode, the surrogate approximation, the gate design, and the COVID window's out-of-distribution status. The nonlinear posterior is consistent with absorbing COVID-era volatility through endogenous amplification rather than larger exogenous shocks. All seven shock volatilities fall: risk-premium volatility $\sigma_b$ drops 49 percent (0.179 to 0.092), investment volatility $\sigma_{qs}$ falls from 0.524 to 0.408, and TFP volatility $\sigma_a$ falls from 0.716 to 0.636. In exchange, persistence rises in the investment and wage blocks---$\rho_b$ from 0.731 to 0.899 and $\rho_w$ from 0.160 to 0.230.

The direction of the $\sigma_b$ shift reverses between the baseline and extended samples: $\sigma_b$ rises in the baseline (0.12 to 0.28) but falls here (0.18 to 0.09). The reversal reflects the different roles the risk-premium shock plays across sample periods. In the baseline (1959--2004), which excludes extreme episodes, the nonlinear model's endogenous amplification frees $\sigma_b$ to absorb investment-driven variation that the linear model attributes to other shocks. In the extended sample, the COVID episode generates observations so extreme that the linear model inflates $\sigma_b$ dramatically (from 0.12 to 0.18); the nonlinear channel then reduces $\sigma_b$ by absorbing COVID-era investment dynamics endogenously, while compensating with higher persistence ($\rho_b$: 0.73 to 0.90). The cold-started surrogate chains do not find this high-likelihood basin (Appendix~\ref{app:multi_chain_convergence}), so the headline comparison should be stated as conditional on the warm-started mode unless a broader mode search confirms global dominance.

The posterior geometry is consistent with the decomposition but should be read as supporting evidence. Kimball curvature $\varepsilon_p$ rises from 82 to 121 but achieves a pooled effective sample size of only 90 draws across four warm-started chains---the posterior is nearly flat, consistent with the data providing little information about the pricing nonlinearity. Risk-premium persistence $\rho_b$, which enters the investment Euler equation directly, achieves a pooled ESS of 4{,}434. The identification asymmetry---sharp identification on the investment-Euler parameters, near-flat posterior on the pricing nonlinearity---matches the equation-block decomposition. The surrogate's validation error (RRMSE $= 0.0012$; Appendix~\ref{sec:formal_consistency}) is small relative to the ROM1--SEP gap, but the COVID extrapolation and gate design remain first-order caveats.
...
\bottomrule
\end{tabular}
\tablenotes{\textit{Notes}: Extended sample 1959Q1--2025Q1 ($T=265$). ``Kalman'' reports posterior means from linear NUTS-HMC (2,000 draws, AdvancedHMC.jl, zero divergences, ESS $\geq 668$). ``RS Surrogate'' pools 8{,}000 post-warmup draws from four warm-started NUTS-HMC chains (seeds 42, 5, 6, 7; 2{,}000 draws per chain) with soft gating and zero divergences across all chains; pooled ESS $\geq 90$ (Appendix~\ref{app:multi_chain_convergence}). $\Delta$ is the shift in posterior means. RS 90\% CI is the 5th--95th percentile of the pooled posterior. Log-likelihood at posterior mean: Kalman $-1{,}817.59$; RS $-1{,}781.68$ (gap $35.92$ nats). Per-chain surrogate posterior-mean LL ranges from $-1{,}787.38$ to $-1{,}780.80$.}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/covid_shock_comparison.pdf}
\caption{Kalman-smoothed shocks (in $\sigma$ units), 2018--2025. Both series are evaluated through the same linear Kalman smoother; only the parameter vector differs. Blue dashed: linear posterior mean; red solid: surrogate posterior mean. Gray band: COVID window (2020Q1--2021Q2). At the linear posterior, risk-premium shocks peak at $56\sigma$. At the surrogate posterior, the lower $\sigma_b$ forces even larger normalized shocks under a linear filter, illustrating that the surrogate posterior parameters should be interpreted through the regime-switching nonlinear likelihood rather than through the Kalman smoother.}
\label{fig:covid_shock_comparison}
\end{figure}

\paragraph{Decomposing the Likelihood Gap.}
A natural objection is that the 35.92-nat improvement might reflect parameter relocation rather than the nonlinear channel: could the linear model close the gap if its shocks were re-tuned to the surrogate's preferred parameter values? Concretely, how much of $\Delta \mathrm{LL} = \mathrm{LL}_{\text{surr}}(\hat{\theta}_{\text{surr}}) - \mathrm{LL}_{\text{lin}}(\hat{\theta}_{\text{lin}})$ comes from the model change at fixed $\theta$, and how much from re-optimizing $\theta$ under the new model? I evaluate the four cells of the 2$\times$2 likelihood table at the two posterior means.

\begin{table}[htbp]
\centering
\caption{Cross-evaluated log-likelihoods at the linear and surrogate posterior means (extended sample, $T=265$).}
\label{tab:ll_decomposition}
\begin{tabular}{lcc}
\toprule
& $\hat{\theta}_{\text{lin}}$ & $\hat{\theta}_{\text{surr}}$ \\
\midrule
$\mathrm{LL}_{\text{lin}}$ (Kalman)            & $-1{,}817.59$ & $-1{,}852.63$ \\
$\mathrm{LL}_{\text{surr}}$ (regime-switching) & $-1{,}830.44$ & $-1{,}781.68$ \\
\bottomrule
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: $\hat{\theta}_{\text{lin}}$ is the linear NUTS-HMC posterior mean (extended sample, 2{,}000 draws); $\hat{\theta}_{\text{surr}}$ is the pooled posterior mean across four warm-started surrogate chains. Each cell is a single log-likelihood evaluation on the same data.}
\end{table}

The off-diagonal entries deliver the answer. At $\hat{\theta}_{\text{surr}}$, the linear Kalman likelihood falls by 35.04 nats relative to its own posterior mean ($-1{,}852.63$ versus $-1{,}817.59$): the linear model does not absorb the surrogate's parameters. At $\hat{\theta}_{\text{lin}}$, the surrogate likelihood is 12.85 nats below the linear posterior-mean likelihood---swapping models without re-tuning loses ground because each parameter vector is fitted to its own likelihood function. Symmetrically averaging the two path decompositions of the gap (model-change-then-parameters and parameters-then-model-change) attributes 29.05 nats (80.9 percent) to the model change and 6.86 nats (19.1 percent) to parameter relocation. This supports the nonlinear channel as the main source of the likelihood gain within the high-likelihood surrogate mode.

The remaining design concern is whether the gain comes from the gate/inversion architecture itself rather than from the learned nonlinear residual. I therefore evaluate a fixed-parameter ablation that keeps the same gate, inversion filter, observation scaling, and soft likelihood mixing, but sets the NN residual correction to zero. Table~\ref{tab:linear_gate_ablation} reports this no-NN likelihood at the same two posterior means. The gate/inversion architecture alone lowers the likelihood at both parameter vectors; the NN residual correction is what produces the positive gain. Symmetrically averaging across the two fixed parameter vectors gives a $-18.22$-nat gate/inversion contribution and a $+47.28$-nat NN-residual contribution. This fixed-parameter ablation does not replace a dedicated no-NN HMC chain, but it rules out the simplest concern that the 35.92-nat result is mechanically generated by the gate or inversion filter.
...
\midrule
$\mathrm{LL}_{\text{lin}}$ (Kalman) & $-1{,}817.59$ & $-1{,}852.63$ \\
$\mathrm{LL}_{\text{gate}}$ (gate+inversion, no NN) & $-1{,}845.10$ & $-1{,}861.57$ \\
$\mathrm{LL}_{\text{surr}}$ (gate+inversion+NN) & $-1{,}830.44$ & $-1{,}781.68$ \\
\midrule
...
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: Fixed-parameter likelihood evaluations on the extended sample. The no-NN row uses the same soft gate and inversion-filter likelihood as the surrogate row but zeros the neural residual correction. Artifact: \path{.local_artifacts/ll_decomposition/LINEAR_GATE_ABLATION_SUMMARY.md}. A full posterior ablation requires rerunning HMC with \texttt{--disable-nn-correction=true}.}
\end{table}

The extended-sample results are directionally consistent with the decomposition predictions. The theory predicts that the linear model's estimated shock processes may be inflated relative to the nonlinear model when the missing investment-channel amplification is quantitatively important. The baseline estimation on 1959--2004 is consistent with this qualitatively for some margins, though $\sigma_b$ rises rather than falls in the baseline; extending through 2025 provides a more demanding test where the direction of the $\sigma_b$ shift aligns with the prediction. The COVID window generates observations that the linear posterior mean rationalizes only with extreme shock realizations. The nonlinear model achieves a higher regime-switching likelihood with smaller estimated shocks, the largest reduction in $\sigma_b$ ($-49$ percent)---the shock that feeds most directly into the investment Euler equation.

The decomposition identifies investment as the dominant nonlinearity source (69 percent); the estimation independently finds that risk-premium dynamics are where linear and nonlinear posteriors diverge most. These two results---one deterministic, one Bayesian---are consistent with the same underlying channel, approached from completely different angles.


\section{Discussion and Robustness}
\label{sec:robustness}

Initial-state treatment is not used as a headline validation result in this draft. Any posterior-recovery claim for $s_0$ should be reported only after the pending Gal\'i direct SEP-HMC versus surrogate-HMC artifact is generated.

Surrogate accuracy remains within RRMSE $< 0.002$ across the full parameter support (Appendix~\ref{app:robustness_extended}, Table~\ref{tab:surrogate_regional_accuracy}). The combined surrogate (63,939 samples including 20,240 ZLB-binding episodes at shock scale 0.4) achieves RRMSE of 0.12 on binding-period test data---an 89 percent improvement over the version trained without ZLB episodes. Earlier Gal\'i seed and prior sensitivity tables are omitted until the direct Gal\'i validation artifact exists.
...
Table~\ref{tab:csadjcost_sensitivity} reports the results. Investment dominance is robust: the investment/capital block accounts for 89--95 percent of the gap across the full range, with lower curvature producing \emph{higher} investment shares ($S''(1) = 2$: 95.1\%) because the investment Euler equation becomes more nonlinear when the adjustment cost is less convex. The price Phillips curve contributes 0.0 percent at every calibration. The absolute gap magnitude declines monotonically with $S''(1)$ (from 5.6 to 2.8 model units), consistent with higher curvature suppressing investment volatility and thus reducing the scope for nonlinear amplification. All 25 theta draws converge at every calibration point.

The stability of the relative shares confirms that investment dominance is a structural property of the invisibility mechanism ($S(1) = S'(1) = 0$) rather than an artifact of the baseline calibration. The curvature $S''(1)$ controls the \emph{magnitude} of the nonlinear gap but not its \emph{composition}---other equation blocks do not scale similarly with $S''(1)$ because their nonlinear terms (Kimball demand, capacity utilization) operate through different channels.

\input{csadjcost_sensitivity_table.tex}

\subsection{Known Limitations}
\label{subsec:known_limitations}

Two independent linear chains find different posterior modes (34-nat gap), indicating ridged geometry in the wage/price markup block \citep{herbst2016sequential}. The decomposition results are deterministic and unaffected; the posterior comparisons in Section~\ref{sec:inference_consequences} are conditional on a single mode.

The baseline surrogate is trained at shock scale 0.1, where the ZLB rarely binds; the supplementary dataset at scale 0.4 extends coverage to binding episodes. The same surrogate applies to different sample periods without retraining, since the neural network maps from parameter-state-shock space rather than time-series indices. The theoretical coverage-distortion bound from Corollary~\ref{cor:coverage} evaluates to below one percentage point at the realized RRMSE of 0.0012 (Appendix~\ref{sec:formal_consistency}), and a direct SEP comparison at twenty posterior draws confirms that the ROM1--SEP prediction gap (average RMSE 1.04 per variable, exceeding $3\sigma_y$ for labor and the interest rate) is large relative to surrogate error.

To bridge the gap to the 18-parameter estimation, I provide two lines of evidence available in the current artifact set: (i) the formal consistency theory shows coverage distortion scales as $\sqrt{T} \cdot \text{RRMSE}^2$ with spectral prefactor $\sqrt{\kappa}/\sqrt{\lambda_{\min}(\mathcal{I}(\theta_0))}$, bounding the theoretical distortion below one percentage point at the realized RRMSE; (ii) the twenty-draw SEP comparison provides an empirical check at full scale. A Monte Carlo coverage study at the full 18-parameter scale (Appendix~\ref{app:mc_coverage}) achieves average coverage of 79.8\% across 14 replications, with the Clopper--Pearson 95\% confidence interval containing the nominal 90\% rate for 16 of 18 parameters. The shortfall is consistent with the short chains (300 post-warmup draws) used per replication; the two parameters outside the bound ($\rho_b$, $\iota_p$) are near the boundary of the identified region. The planned direct Gal\'i SEP-HMC comparison is listed separately in Appendix~\ref{sec:validation_design}.

\subsection{Policy Implications}

If linear models systematically overestimate exogenous shock volatility and underestimate endogenous amplification---as the results suggest for medium-scale New Keynesian models with investment adjustment costs---then in models of this class the economy is driven more by endogenous propagation than standard estimates suggest. The direct SEP posterior comparison confirms that the nonlinear correction exceeds $3\sigma_y$ for labor and the interest rate at posterior parameter values, suggesting that revisiting optimal policy toward dampening investment volatility is warranted---a question requiring formal welfare analysis beyond this paper's scope. For model selection, the results favor prioritizing the investment block over the ELB and Kimball pricing---the opposite of current practice.


\section{Conclusion}
\label{sec:conclusion}

The main finding of this paper is that first-order perturbation misses the dominant source of nonlinearity in the Smets--Wouters model. The investment adjustment cost satisfies $S(1) = S'(1) = 0$, making it invisible to linearization. This channel accounts for 69 percent of the gap between nonlinear and linear solutions; the price Phillips curve accounts for 0.2 percent. These are deterministic measurements, independent of MCMC or surrogate quality. The result is robust to the calibration of $S''(1)$ across the range $[2, 10]$: investment dominance ranges from 89 to 95 percent, while the price Phillips share remains at 0.0 percent (Section~\ref{subsec:csadjcost_sensitivity}).

The posterior shifts are consistent with the theory, but the empirical claim is conditional. Extending the sample through 2025Q1, the high-likelihood warm-started surrogate mode achieves a better regime-switching likelihood for COVID-era observations with smaller estimated shock volatilities, a pattern consistent with endogenous amplification rather than reliance on implausible shock draws. All seven shock volatilities fall (risk premium by 49 percent), persistence parameters rise ($\rho_b$: 0.73 $\to$ 0.90), and the posterior-mean log-likelihood gap is 35.92 nats. Within this model, the posterior geometry is consistent with the decomposition: the data strongly identify parameters entering the investment channel (pooled ESS for $\rho_b$ is $\approx 4{,}400$), while Kimball curvature is nearly flat (pooled ESS $= 90$).

The methodological contribution is inseparable from the substantive finding. The investment-dominance result, the identification asymmetry between $\rho_b$ and $\varepsilon_p$, the 35.92-nat likelihood gap, and the $56\sigma$ COVID shocks under the linear model require nonlinear Bayesian estimation at this scale. What the surrogate adds relative to existing nonlinear estimation methods \citep{kase2025estimating,naubert2025differentiable} is an auditable approximation target---the residual RRMSE is measured directly on $g^{\text{SEP}} - g^{\text{ROM1}}$---and scalability: the same trained surrogate serves multiple estimation samples, parameter configurations, and decomposition exercises without retraining. The same properties expose the caveats: the posterior comparison remains conditional on one HLT specification, one country, the warm-started mode, the gate design, and COVID out-of-distribution behavior.

Appendix~\ref{app:mc_coverage} reports a Monte Carlo coverage study at the full 18-parameter scale: 14 replications of the linear estimator, each drawing true parameters from the prior and checking whether 90\% credible intervals contain the true values. Average coverage of 79.8\% is below the nominal 90\%, though the Clopper--Pearson 95\% confidence intervals contain the nominal rate for 16 of 18 parameters, and the shortfall is consistent with the short chains (300 post-warmup draws) used per replication. Multi-chain $\hat{R}$ diagnostics from 4 independent chains (Appendix~\ref{app:multi_chain_convergence}) confirm convergence for 15 of 18 linear parameters; the three non-converging parameters ($\rho_\pi$, $\sigma_\pi$, $\iota_p$) belong to the price Phillips curve block that contributes only 0.2\% of the nonlinear correction. For the surrogate estimator, warm-starting three additional chains from the best chain's posterior mean (seeds 5, 6, 7) resolves the initialization failure that caused all 18 parameters to show $\hat{R} > 1.1$ in the original four-chain diagnostic: the four warm-started chains now converge with max $\hat{R} = 1.05$ and pooled minimum ESS of 90 (Appendix~\ref{app:multi_chain_convergence}).

Three extensions follow naturally. First, extending the invisibility analysis to other models with investment adjustment costs \citep[e.g.,][]{christiano2005nominal,justiniano2010investment} would test whether investment dominance is a general property of medium-scale models. Second, pseudo-out-of-sample forecasts in Appendix~\ref{app:oos_forecast} sharpen the gate-design caveat. Estimating both models on 1959Q1--2019Q4 and scoring 2020Q1--2025Q1, the switching surrogate reduces aggregate RMSE by 16 percent during the COVID window, with gains of 29 percent on output growth, 24 percent on consumption growth, and 26 percent on investment growth. But a quiet-sample pilot that estimates through 1994Q4 and forecasts 1995Q1--2007Q4 fails the calm-period criterion under the original gate: the switching surrogate's aggregate RMSE is 2.37 times ROM1, with hard-gate activation in 26.9 percent of holdout quarters. A fixed-posterior q95 gate diagnostic reduces the aggregate ratio to 1.04 and the hard-gate share to 1.9 percent, so the failure appears substantially gate-driven. Gate calibration is therefore no longer an unlocalized failure, but it remains a central implementation risk until the posterior is re-estimated under the revised gate and calm-period predictive performance is no worse than ROM1. Third, Appendix~\ref{app:sep_sensitivity} now reports a bounded 40-period SEP solver-sensitivity pilot: all tolerance/quadrature cells converge and the observation RMSE against the tight reference is below $10^{-2}$. The full 265-period sensitivity rerun remains a pre-submission scaling task, so the headline decomposition still rests primarily on the deterministic shock-scale study.
...
This appendix presents the complete equilibrium system for the Gal\'i (2015) New Keynesian model with occasionally binding zero lower bound. In this draft it serves as a negative-control specification and as the planned target for the direct SEP-HMC versus surrogate-HMC validation; numerical recovery results from that exercise are not reported until the matching artifact exists. The 18-parameter analysis in Section~\ref{subsec:nonlinearity_sources} uses the richer \citet{smets2007shocks} specification, detailed in Appendix~\ref{app:sw07_model} below.
...
\end{equation}
At $\kappa_s = 100$, the approximation error is bounded: for a notional rate 200 basis points below the floor ($\tilde{R}_t - 1 = -0.02$), the softplus output is $\kappa_s^{-1}\log(1 + e^{-2}) \approx 0.01\,\text{bp}$---well below measurement precision. The key advantage for the Newton solver is that the softplus is $C^\infty$: the Jacobian exists everywhere and varies smoothly, avoiding the subdifferential complications of a $\max(\cdot, 0)$ kink. This ensures that the LM--Newton iteration converges quadratically near the solution even when the ZLB binds.
...
Warm starts reduce Newton iterations from about 20 with a cold start to about 3--5 in the grid runs. This reduction implies a 4--6$\times$ speedup across the grid. Critically, I accept a warm start only when the previous solve converged (convergence flag equals zero); if the previous solve failed or exceeded the acceptance tolerance, I fall back to a cold start from the first-order perturbation steady state. This check prevents the solver from inheriting a divergent trajectory that would contaminate subsequent solves along the parameter grid---a subtle bug that I discovered caused systematic convergence failures in early dataset-generation runs before the flag check was added.
...
\paragraph{Loss function.} Mean squared error on standardized outputs:
\begin{equation}
...
The Sobol parameter draws cover the prior support broadly, but the states visited during SEP simulation may not overlap with the states that the estimation filter encounters when processing actual US data. To close this gap, I augment the training set with state trajectories recovered from observed data. Specifically, I run the Kalman smoother on the 7-observable US dataset at the calibrated Smets--Wouters parameters to obtain a smoothed state sequence $\{\hat{s}_t^{\text{KF}}\}_{t=1}^T$ and associated shocks $\{\hat{\varepsilon}_t^{\text{KF}}\}_{t=1}^T$. These $(\hat{s}_{t-1}^{\text{KF}}, \hat{\varepsilon}_t^{\text{KF}}, \theta_{\text{calib}})$ tuples are solved through the SEP to produce FOM transitions, then appended to the training set. This reduces the maximum standardized input distance (across all dimensions) from 6.2 to 1.5, ensuring the training data cover the estimation-relevant region of state space. I also optionally add IRF-based augmentation at $\pm 0.5\sigma$ and $\pm 2.0\sigma$ per shock, which improves coverage of large-shock events like the 2008Q4 financial crisis.


\subsection{Output Format and Checkpointing}

The dataset is serialized as a Julia dictionary containing: input matrix $X \in \R^{d_{\text{in}} \times N}$ (standardized state, shock, and parameter inputs), FOM output matrix $Y^{\text{FOM}} \in \R^{d_{\text{out}} \times N}$, ROM1 output matrix $Y^{\text{ROM1}}$, and metadata (parameter bounds, shock scale, acceptance tolerance, random seed, solver options). Checkpoints are written every 10 parameter draws, so that a crash after processing 80 of 120 draws does not require restarting from scratch. Full reproducibility is guaranteed: the RNG seed, Sobol sequence state, and all solver settings are saved alongside the data.
...
\subsection{Per-Period Log-Likelihood}

Given the recovered shocks $\hat{\varepsilon}_t$ and predicted observables $\hat{y}_t$, the per-period log-likelihood is:
\begin{equation}
...
\end{equation}
where $\mathcal{S}$ indexes the structural shocks and the constant is $c = (n_y + |\mathcal{S}|) \log(2\pi) + \sum_j \log \sigma_{y,j}^2 + \sum_{k \in \mathcal{S}} \log \sigma_{\varepsilon,k}^2$. The total log-likelihood sums over all periods: $\ell(\theta) = \sum_{t=1}^T \ell_t$. Under the inversion filter, when the recovered shocks perfectly match the observations (zero observation residual), the log-likelihood reduces to the shock prior penalty plus constants---recovering the standard Gaussian likelihood of the structural shocks. In practice, the NN correction leaves small residuals ($\|r_{\text{obs}}\|_2 \approx 10^{-3}$), so the observation fit term contributes negligibly.


\subsection{Batch NN Correction}
\label{app:batch_nn}

In Phase~2, I exploit the fact that all $T$ periods are available simultaneously to batch the NN evaluations. I construct the input matrix $X_{\text{nn}} = [s_{t-1};\, \varepsilon_t;\, \theta] \in \R^{d_{\text{in}} \times T}$ and evaluate $\hat{\Delta} = \text{MLP}(X_{\text{nn}}) \in \R^{(d_{\text{obs}} + d_{\text{state}}) \times T}$ in a single BLAS-3 matrix multiplication per layer, which is 50--70\% faster than $T$ sequential vector evaluations due to cache efficiency and SIMD utilization. Only the first $d_{\text{obs}}$ rows of $\hat{\Delta}$ correct the observables; the state dimensions are discarded (obs-only mode). I optionally clamp the correction: $|\hat{\Delta}_j| \leq 3 \times \text{RMSE}_j^{\text{val}}$, where $\text{RMSE}_j^{\text{val}}$ is the per-dimension validation RMSE. This prevents outlier NN predictions from dominating the likelihood at individual periods, a failure mode I observed when the network extrapolated to unseen state--parameter combinations during early HMC warmup.

When the regime-switching gate is active, I use a hybrid strategy: gate periods ($g_t = 1$) receive per-period NN evaluation (since these periods need both observable and state corrections), while non-gate periods are batch-corrected via a single BLAS-3 call over the $T - \sum g_t$ non-gate indices.
...
\subsection{Switched Likelihood}

Under hard gating, the total log-likelihood combines Kalman and inversion-filter contributions:
\begin{equation}
\ell(\theta) = \sum_{t:\, g_t = 1} \ell_t^{\text{surr}}(\theta) + \sum_{t:\, g_t = 0} \ell_t^{\text{Kalman}}(\theta),
\end{equation}
where $\ell_t^{\text{surr}}$ is equation~\eqref{eq:per_period_ll} and $\ell_t^{\text{Kalman}}$ is the standard Kalman filter log-likelihood at period $t$. Under soft gating, the per-period likelihood is the log-sum-exp mixture:
\begin{equation}
\ell_t^{\text{mix}} = \text{logsumexp}\!\bigl(\log p_t + \ell_t^{\text{surr}},\; \log(1 - p_t) + \ell_t^{\text{Kalman}}\bigr),
\end{equation}
and the total is $\ell(\theta) = \sum_t \ell_t^{\text{mix}}$. The logsumexp formulation avoids underflow when one component has much higher likelihood than the other.
...
\end{equation}
where $\ell(\theta)$ is the switched log-likelihood (Section~\ref{app:gate}) and $\log p(\theta)$ is the prior.
...
\item \textbf{Data augmentation}: Append Kalman-smoother trajectories from observed US data to the training set (Section~\ref{app:data_augmentation}).
...
Table~\ref{tab:computational_cost} reports the runtime profile of the existing HLT three-parameter validation workflow. This is a pipeline-cost benchmark, not the pending Gal\'i direct-HMC validation.
...
All code and data are available at \url{https://github.com/matyasfarkas/SurrogateNN_Estimation.jl}.
...
The contrast between Tables~\ref{tab:pf_benchmark} and~\ref{tab:copf_benchmark} is informative. The bootstrap filter is catastrophically degenerate (LL $= -21 \times 10^6$, ESS $= 1$), the COPF is three orders of magnitude better (LL $= -16{,}000$, ESS $= 2$--$4$), but neither approaches the exact Kalman value of $-1{,}818$. Even with the theoretically optimal proposal, particle filtering in a 7-observable DSGE requires particle counts far exceeding $10^4$ to eliminate weight degeneracy---consistent with the theoretical $N_p = O(\exp(n_y))$ scaling in \citet{snyder2008obstacles}. For the nonlinear model, where neither the Kalman update nor the optimal proposal is available in closed form, particle filtering is infeasible at any realistic computational budget. The inversion filter sidesteps this entirely by treating shocks as explicit unknowns and using the surrogate for deterministic evaluation rather than stochastic proposal construction.
...
A natural concern is whether the 69-percent investment share in the equation-block decomposition (Section~\ref{subsec:nonlinearity_sources}) depends on the SEP solver's acceptance tolerance $\tau_{\text{acc}}$---the residual threshold that determines which SEP solutions count as ``converged'' during dataset generation and inference-time evaluation. I reran the fixed sensitivity script on three posterior draws stratified by Mahalanobis distance, using a 40-period window from the extended payload. The grid crosses $\tau_{\text{acc}} \in \{0.01, 0.1, 0.35\}$ with $K \in \{3,5\}$ quadrature nodes and compares every cell to the tight reference $(\tau_{\text{acc}},K)=(0.01,5)$.

\input{../../.local_artifacts/sep_sensitivity/sep_sensitivity_table.tex}

\paragraph{Interpretation.} This bounded rerun closes the earlier provenance gap that the sensitivity script had not been executed after the cache-initialization fix. All cells converge over the 40-period window, and changing $\tau_{\text{acc}}$ within a fixed $K$ leaves the SEP predictions unchanged at the reported precision. Moving from $K=5$ to $K=3$ changes observation predictions by RMSE $6.16\times 10^{-3}$, below the $10^{-2}$ screening threshold used in the artifact. Because a full 265-period run with $K=5$ was prohibitively slow in this pass, I treat this as a bounded solver-stability pilot rather than a substitute for the full pre-submission rerun. The main text therefore continues to rely on the deterministic shock-scale decomposition, with this table as supporting provenance only.
...
The ranking confirms three substantive points. First, SEP is the most accurate solution method by a wide margin: the median normalized Euler residual is $4.2 \times 10^{-13}$, essentially machine precision, indicating that the SEP solver satisfies the model's first-order conditions to numerical zero on a typical equation in a typical period. ROM1 sits at $4.7 \times 10^{-4}$---a 0.05\% relative error---which is well within the 1\% threshold conventionally cited as ``acceptable'' for first-order perturbation in the global-solution literature \citep{aruoba2006comparing}. Second, the surrogate's gate-on correction yields a typical normalized residual of $1.7 \times 10^{-3}$, larger than ROM1's by about half a log decade in median. The neural network is trained to reduce the prediction RMSE between the FOM (truth) and ROM1 (linear baseline), delivering accurate \emph{forecasts} of observables, but it does not enforce the model's forward-looking consistency conditions; the surrogate functions as a likelihood-approximation tool---a learned correction to the linear filter's prediction step---rather than as a global solution method. Third, the gate-everywhere stress test produces residuals five orders of magnitude above the gated configuration, confirming that the gate's role is to keep the NN inside its training distribution.

The quantitatively meaningful comparison for likelihood-based inference is the prediction RMSE of the surrogate (validation RRMSE $= 0.0012$) and the realized log-likelihood improvement (35.92 nats over the linear model on the extended sample). The Euler-error ranking does not contradict these results because the two metrics measure different objects: Euler errors quantify how well a trajectory satisfies the model's own equations, while the likelihood quantifies how well a method's predictions match observed data. The 69-percent investment-dominance decomposition (Section~\ref{subsec:nonlinearity_sources}) is computed from SEP solutions---the most accurate method on the Euler-error metric---so the decomposition does not inherit the surrogate's larger Euler error. The bounded SEP-sensitivity pilot in Appendix~\ref{app:sep_sensitivity} supports tolerance stability over a 40-period window, while the full 265-period sensitivity rerun remains a pre-submission scaling task.
...
\begin{assumption}[Mean-square and score control]
\label{ass:surrogate_smoothness}
Let $e_t(\theta) = H(s_t^{\text{FOM}}(\theta)-\hat{s}_t(\theta))$ and $y_t = Hs_t^{\text{FOM}}(\theta_0)+\eta_t$. Uniformly over $\Theta$, $\E\|e_t(\theta)\|^2 = O(\delta_T^2)$ and the approximation error is orthogonal to the measurement innovation, $\E[\eta_t'\Sigmay^{-1}e_t(\theta)] = 0$. The score contribution of the surrogate error is smooth enough that $\sup_{\theta\in\Theta}\|\nabla_\theta[\hat{\ell}(\theta)-\ell(\theta)]\| = O_p(T\delta_T^2)$.
\end{assumption}
...
Proposition~\ref{prop:ll_bias_bound} below separates the deterministic and mean-square statements. Pathwise, the exact identity gives a first-order bound in the sup-norm approximation error. For the posterior consistency claim I use the expected, orthogonality-based bound in Assumption~\ref{ass:surrogate_smoothness}: the per-period surrogate-induced bias is $O(\delta_T^2)$ uniformly in $\theta$. Therefore, for a constant $C$ independent of $t$ and $\theta$,
\begin{equation}
\sup_{\theta \in \Theta}
...
\end{equation}
Under the rate condition $\delta_T = o(T^{-1/2})$, the right-hand side is $o_p(1)$, so the surrogate log-likelihood converges uniformly to the FOM log-likelihood in probability. A purely deterministic sup-norm theorem would instead require the stronger pathwise rate $\delta_T=o(T^{-1})$.
...
\label{cor:coverage}
Let $C_\alpha^T \subset \Theta$ be a $(1-\alpha)$-credible set under the surrogate posterior: $\hat{\pi}_T(C_\alpha^T \given y_{1:T}) = 1-\alpha$. Let $B_T = T \cdot C \cdot \delta_T^2$ denote the mean-square log-likelihood perturbation from \eqref{eq:uniform_ll_approx}, and write $\kappa = \lambda_{\max}(\mathcal{I}(\theta_0)) / \lambda_{\min}(\mathcal{I}(\theta_0))$ for the spectral condition number of the Fisher information. Under the conditions of Theorem~\ref{thm:posterior_consistency}, the frequentist coverage of $C_\alpha^T$ satisfies
\begin{equation}
\bigl|\,P_{\theta_0}\!\left(\theta_0 \in C_\alpha^T\right) - (1 - \alpha)\,\bigr|
  \;\leq\; \frac{B_T}{\sqrt{T}} \cdot
    \frac{\sqrt{\lambda_{\max}(\mathcal{I}(\theta_0))}}{\lambda_{\min}(\mathcal{I}(\theta_0))}
  \;+\; o(T^{-1/2}).
\label{eq:coverage_bound}
\end{equation}
Expressed in terms of the surrogate RRMSE through $B_T = O(T \cdot \text{RRMSE}^2)$, the distortion scales as $\sqrt{T} \cdot \text{RRMSE}^2 \cdot \sqrt{\kappa}/\sqrt{\lambda_{\min}(\mathcal{I}(\theta_0))}$. The bound requires $\kappa$ to be bounded; in the application, the Fisher information is well-conditioned at the posterior mode (condition number $< 50$ from the inverse Hessian), so this assumption is satisfied.
\end{corollary}
...
\]
The right-hand side is bounded in norm by $\|\nabla[\hat{\ell} - \ell]\|_\infty$, which is $O_p(T\delta_T^2)=O_p(B_T)$ by Assumption~\ref{ass:surrogate_smoothness}. A second-order Taylor expansion of $\ell$ around $\tilde{\theta}_T$ and the identity $-\nabla^2\ell(\tilde{\theta}_T)/T \to \mathcal{I}(\theta_0)$ yield
\begin{equation}
\|\hat{\theta}_T - \tilde{\theta}_T\|
  \;\leq\; \frac{\|\nabla[\hat{\ell} - \ell](\hat{\theta}_T)\|}
    {\lambda_{\min}(-\nabla^2\ell(\tilde{\theta}_T))}
  \;=\; O\!\left(\frac{B_T}{T\,\lambda_{\min}(\mathcal{I}(\theta_0))}\right).
\label{eq:mode_shift}
\end{equation}
...
\[
\text{TV}(\hat{\pi}_T, \pi_T)
  \;\leq\; \tfrac{1}{\sqrt{2\pi}}\,\sqrt{T\lambda_{\max}}\,\cdot\,\frac{B_T}{T\lambda_{\min}}
  \;=\; \tfrac{1}{\sqrt{2\pi}}\,\frac{B_T}{\sqrt{T}}\cdot\frac{\sqrt{\lambda_{\max}}}{\lambda_{\min}}.
\]
...
The coverage distortion in \eqref{eq:coverage_bound} scales as $\sqrt{T} \cdot \text{RRMSE}^2$ with a spectral prefactor $\sqrt{\kappa}/\sqrt{\lambda_{\min}(\mathcal{I}(\theta_0))}$. In the extended-sample application, $T = 265$, $\text{RRMSE} = 0.0012$, and the constant $C$ in $B_T = T \cdot C \cdot \text{RRMSE}^2$ satisfies $C \lesssim \|H\|_{\text{op}}^2 / (2\lambda_{\min}(\Sigmay)) = 50$ from Proposition~\ref{prop:ll_bias_bound}. This gives $B_T \leq 265 \cdot 50 \cdot (0.0012)^2 = 0.019$ nats. With spectral condition number $\kappa \lesssim 50$ and $\lambda_{\min}(\mathcal{I}(\theta_0)) \approx 1$ at the posterior mode, the bound evaluates to
\[
\text{Coverage distortion}
  \;\leq\; \frac{0.019}{\sqrt{265}} \cdot \frac{\sqrt{50}}{1}
  \;\approx\; 0.0082 \;=\; 0.82 \text{ percentage points}.
\]
The surrogate would need RRMSE exceeding $0.013$ before the theoretical bound reached one percentage point at this sample size. The bound is conservative: it applies worst-case RRMSE uniformly across all periods and all state dimensions, whereas the surrogate's error concentrates in a few investment-block variables during stressed periods and is negligible elsewhere. The 18-parameter Monte Carlo coverage study (Appendix~\ref{app:mc_coverage}) indicates that realized coverage distortion is below the worst-case theoretical bound, though the planned direct Gal\'i SEP-HMC comparison remains a required validation before submission.
\end{remark}
...
Theorem~\ref{thm:posterior_consistency} requires $\delta_T = o(T^{-1/2})$, a condition that cannot be verified directly since the FOM solution at the posterior mode is unavailable during online estimation. Three diagnostics provide evidence. First, the held-out RRMSE (Table~\ref{tab:surrogate_regional_accuracy}) bounds $\delta_T$ at parameter values not used in training. Second, the realized log-likelihood bias bound from Proposition~\ref{prop:ll_bias_bound} evaluates to $B_T \leq 0.02$ nats, small relative to the likelihood scale, and the coverage-distortion bound from Corollary~\ref{cor:coverage} is below one percentage point. Third, a direct FOM posterior comparison---computing SEP solutions at twenty subsampled posterior draws---confirms that the ROM1--SEP prediction gap averages 1.04 RMSE per variable, with all twenty SEP simulations converging and the largest gaps concentrated in labor (3.07) and the interest rate (0.86), consistent with the equation-block decomposition identifying investment dynamics as the dominant nonlinear channel.


\section{Validation Design}
\label{sec:validation_design}

The pre-submission validation target is a direct SEP-HMC versus surrogate-HMC comparison on the Gal\'i (2015) synthetic model in Section~\ref{subsec:gali_model}. The DGP should use the same SEP solution and priors for both samplers, with identical data, seeds, and chain lengths. The comparison is acceptable if posterior means are within Monte Carlo error, 90 percent intervals materially overlap, and no systematic coverage distortion appears.

The current local artifacts do not yet contain that Gal\'i direct-HMC comparison. The 2026-05-04 audit found an archived Gal\'i validation package under \path{archive/development/SurrogateNN/}, including a trained three-observable Gal\'i surrogate, cached Gal\'i synthetic data, and a two-parameter surrogate-HMC run. The archived Gal\'i SEP engine itself solves at the cached true parameter in a small smoke run (flag 0, residual $9.74\times 10^{-6}$), but this is not the requested three-parameter direct SEP-HMC versus surrogate-HMC posterior comparison. After tightening the inversion finite-difference prediction tolerance, the current MacroModelling exact-determinant inversion likelihood also passes a one-period Gal\'i three-observable synthetic smoke, so the earlier rank-zero diagnostic is no longer the active blocker. The remaining blocker is the missing matched Gal\'i direct-HMC and surrogate-HMC posterior artifact.

I added a bounded HLT direct-SEP/MH versus surrogate/MH harness, \texttt{scripts/hlt\_direct\_sep\_surrogate\_posterior\_validation.jl}, targeting the SW07-HLT legacy three-parameter price block $(\xi_p,\iota_p,\varepsilon_p)$ rather than the Gal\'i shock-volatility vector $(\sigma_A,\sigma_\mu,\sigma_R)$. The resulting provenance artifact, \path{.local_artifacts/hlt_direct_sep_surrogate_validation/HLT_DIRECT_SEP_SURROGATE_VALIDATION_SUMMARY.md}, is not a posterior validation pass: it is a one-draw smoke/provenance harness. In the current run the direct SEP side returns a finite likelihood with the exact determinant and records \texttt{status=ok}; the serialized HLT3 surrogate bundle still cannot be deserialized by the current runtime, so the surrogate side falls back to the existing surrogate-HMC chain payload.

I therefore treat the Gal\'i validation and the full direct posterior comparison as pending and do not report numerical posterior recovery results from them in this draft. The Gal\'i provenance note is \path{.local_artifacts/gali_direct_hmc/GALI_DIRECT_HMC_STATUS.md}; the HLT direct-SEP smoke is tracked in \path{.local_artifacts/hlt_direct_sep_surrogate_validation/HLT_DIRECT_SEP_SURROGATE_VALIDATION_SUMMARY.md}.
...
\begin{table}[htbp]
\centering
\caption{Direct posterior-validation status before submission}
\label{tab:param_recovery}
\begin{tabular}{ll}
\toprule
Item & Status \\
\midrule
Common Gal\'i SEP DGP & Archived synthetic data found; direct posterior DGP still pending \\
Direct SEP likelihood HMC & Pending; exact-determinant one-period Gal\'i inversion smoke is finite \\
Surrogate likelihood HMC & Archived two-parameter run found; requested three-parameter run pending \\
Common priors, seeds, and chain lengths & Pending \\
Posterior mean and interval comparison & Pending \\
Coverage and distortion diagnostic & Pending \\
HLT direct-SEP/MH fallback harness & Implemented; exact-determinant direct SEP smoke now finite \\
Historical HLT direct-FOM smoke & Available; status \texttt{ok}, LL $=-339.98$ \\
\bottomrule
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: The current repository has archived Gal\'i surrogate assets, HLT three-parameter validation assets, and historical direct SEP smoke artifacts, but not the Gal\'i direct SEP-HMC versus surrogate-HMC artifact required for a submission-grade posterior validation. The HLT direct-SEP/MH harness writes \path{.local_artifacts/hlt_direct_sep_surrogate_validation/HLT_DIRECT_SEP_SURROGATE_VALIDATION_SUMMARY.md}; the current direct SEP side is finite under the exact determinant after tightening the inversion prediction tolerance. The Gal\'i status note is \path{.local_artifacts/gali_direct_hmc/GALI_DIRECT_HMC_STATUS.md}.}
\end{table}
...
An initial four-chain diagnostic with cold-start chains (seeds 42, 2, 3, 4) produced $\hat{R} > 1.1$ for all 18 parameters. Chains 2--4, initialized from calibrated baseline values, failed to find the nonlinear posterior mode (log-likelihood $\approx -14{,}535$ vs.\ $-1{,}787$ for the best chain)---reflecting the surrogate posterior's narrower basin of attraction and the difficulty of starting from a cold initialization. The nonlinear likelihood surface is more concentrated than the linear one (the 35.92-nat log-likelihood improvement documented in Section~\ref{subsec:extended_sample} implies a sharper peak), creating an initialization challenge that the standard NUTS warmup phase cannot always overcome in 500 adaptation steps.

The fix is straightforward: warm-starting new chains from the best chain's posterior mean. Three additional chains (seeds 5, 6, 7) were initialized from the seed-42 posterior mean and run for 2{,}000 post-warmup draws each ($\approx 65$ hours per chain on Apple M4). Table~\ref{tab:rhat_surrogate} reports the diagnostics pooled across the four warm-started chains (seeds 42, 5, 6, 7). Convergence now holds broadly: maximum $\hat{R} = 1.049$ (below the 1.1 threshold for all 18 parameters), 13 of 18 parameters satisfy the stricter $\hat{R} < 1.01$ criterion, and the remaining five---$\rho_{qs}$, $\rho_{ms}$, $\sigma_{qs}$, $\iota_p$, $\varepsilon_p$---show only mild warnings with $\hat{R} \in [1.02, 1.05]$. Minimum pooled ESS is 90 (for Kimball curvature $\varepsilon_p$), consistent with the weak identification of the pricing nonlinearity; ESS exceeds 4{,}000 for the risk-premium and TFP shock blocks. The per-chain posterior-mean log-likelihood agrees to within 7 nats across the four chains ($-1{,}787$ to $-1{,}780.8$), and no chain records a divergent transition.

\begin{table}[htbp]
\centering
\caption{Mode Sensitivity Across Extended-Sample Chain Artifacts}
\label{tab:mode_sensitivity_chains}
\resizebox{\textwidth}{!}{\input{../../.local_artifacts/mode_sensitivity/table_mode_sensitivity.tex}}
\smallskip
\tablenotes{\textit{Notes}: Existing chain artifacts only; generated by \texttt{scripts/mode\_sensitivity\_report.jl}. Cold-started surrogate chains initialized from calibrated values remain in a low-likelihood basin ($\approx -14{,}535$). The high-likelihood surrogate comparison in the main text uses the warm-started basin and should be interpreted conditionally unless a broader mode search confirms global dominance.}
\end{table}
...
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: Linear NUTS-HMC (AdvancedHMC.jl) on US data, 1959Q1--2004Q4 ($T=184$). 90\% CI is the 5th--95th percentile of the posterior. This chain converges to a different mode than the one used for comparison in Table~\ref{tab:18param_switching_posterior}; see convergence diagnostics in Section~\ref{sec:inference_consequences}.}
\end{table}

\begin{table}[htbp]
\centering
\caption{Runtime profile of the HLT three-parameter validation workflow (seconds)}
...
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: All times are wall-clock seconds on an Apple M4. Median, min, and max are computed across four replications of the existing SW07-HLT legacy three-parameter validation harness. The table does not report the pending Gal\'i direct SEP-HMC versus surrogate-HMC comparison.}
\end{table}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/gap_magnitude_vs_shock_scale.pdf}
\caption{FOM--ROM1 gap magnitude and lower-bound binding diagnostics across the plotted shock-scale grid. Investment/capital remains the largest contributor over the displayed range; at the most extreme scale (1.5$\sigma$), the binding-period share is based on only 3 binding periods.}
...
\caption{FOM--ROM1 gap decomposition in the Gal\'i (2015) model, which shares the ELB and Kimball structure with the HLT model but lacks investment. The plotted gap is much smaller than in the HLT model at every shock scale, consistent with the investment channel being the main difference between the models.}
...
One natural question about the COVID-era results in Section~\ref{subsec:extended_sample} is whether the nonlinear model's better in-sample fit translates into better out-of-sample predictive performance---particularly over the COVID window that dominates the in-sample log-likelihood gap. This appendix reports a pseudo-out-of-sample exercise: I re-estimate both models on the pre-COVID sample 1959Q1--2019Q4 ($T_{\text{pre}} = 244$ quarters) and compute one-step-ahead forecast RMSE for each observable over the holdout window 2020Q1--2025Q1 ($T_{\text{fore}} = 21$ quarters).

\paragraph{Design.} I truncate the extended-sample data payload to 1959Q1--2019Q4 and re-run (i) the linear Kalman-NUTS chain and (ii) the surrogate regime-switching chain on the restricted sample, each warm-started from the corresponding full-sample posterior mean to reduce warmup cost. I use 500 post-warmup NUTS draws for the linear chain and 300 for the surrogate (reduced from the paper's main-text 2{,}000 because warm-starting largely eliminates the adaptation-period requirement; pooled ESS values in the linear chain on 500 draws exceed 120 for all but three parameters). From each pre-COVID chain I then evaluate the 21-period holdout window via one-step-ahead forecasts: for each posterior draw $\theta^{(j)}$, I run the Kalman smoother (linear) or inversion filter with surrogate correction (regime-switching) up to $T_{\text{pre}}$ to obtain the final filtered state $\hat{s}_{T_{\text{pre}}}$, then apply the model's state transition under observed shocks to produce $\hat{y}_{t+1|t}$ for $t \in \{T_{\text{pre}}, \ldots, T_{\text{pre}} + T_{\text{fore}} - 1\}$. Forecast RMSE is computed per observable as $\text{RMSE}_j = \sqrt{\frac{1}{T_{\text{fore}}}\sum_{t} (y_{t+1,j} - \hat{y}_{t+1|t,j})^2}$.

\paragraph{Results.} Table~\ref{tab:oos_forecast} reports the one-step-ahead RMSE per observable over three windows: the full OOS period 2020Q1--2025Q1 (21 quarters), the COVID window 2020Q1--2021Q2 (6 quarters), and the post-COVID recovery 2021Q3--2025Q1 (15 quarters). The pattern is informative. During the COVID window the switching surrogate outperforms ROM1 on the four quantity aggregates---output growth (29 percent RMSE reduction), consumption (24 percent), investment (26 percent), and wage growth (8 percent)---and on the aggregate RMSE by 16 percent. These are the observables in which the investment-channel nonlinearity matters most, and the ranking is consistent with the in-sample decomposition in Section~\ref{subsec:nonlinearity_sources}. Over the post-COVID window, the ranking flips: ROM1 delivers smaller RMSE across most observables, reflecting that the regime-switching gate continues to fire through the recovery even as the underlying dynamics return to the linear regime. On the full OOS sample the aggregate RMSE ratio is $1.14$---the switching model's COVID gains are partly offset by its post-COVID over-activation, yielding a narrow overall ROM1 advantage.

\begin{table}[htbp]
\centering
\caption{Out-of-sample one-step-ahead forecast RMSE: 2020Q1--2025Q1}
...
labour       & 1.53 & 5.97 & 3.90 & 2.68 & 6.36 & 2.38 & 0.65 & 5.81 & 9.00 \\
...
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: One-step-ahead forecast RMSE at the pre-COVID posterior mean. ``ROM1'' uses the Kalman filter with the linear posterior mean from a 500-draw pre-COVID NUTS chain (min ESS $= 141$). ``Switch'' uses the inversion filter with the regime-switching surrogate at the surrogate posterior mean from a 300-draw pre-COVID NUTS chain (min ESS $= 80$), with soft gating calibrated on the pre-COVID sample. Ratio below 1 indicates the switching model produces smaller forecast errors. Estimation sample: 1959Q1--2019Q4 ($T_{\text{pre}} = 244$); forecast sample: 2020Q1--2025Q1 ($T_{\text{fore}} = 21$). Aggregate RMSE is the root of the mean squared error pooled across all 7 observables, normalizing each observable by its in-sample standard deviation to make the aggregation scale-invariant.}
\end{table}

\paragraph{Interpretation.} The COVID-window result---a 16 percent reduction in aggregate forecast RMSE, driven by 24--29 percent reductions on output, consumption, and investment---is the intended test. The nonlinear model is designed to activate during stressed episodes; 2020Q1--2021Q2 is the canonical stressed episode for post-war US data, and the switching model delivers measurable predictive gains over the linear alternative. The largest single-observable improvement, on investment growth during COVID (RMSE $13.29 \to 9.85$), is consistent with the equation-block decomposition assigning 69 percent of the nonlinearity to the investment block.

Two caveats apply. First, the post-COVID under-performance reflects a gating issue rather than a model-fundamental disadvantage: the gate threshold calibrated on the pre-COVID sample treats 2021Q3--2025Q1 as more stressed than the economy actually is, firing the NN correction in quarters when ROM1 would fit better. Re-calibrating the gate with a post-COVID tail adjustment, or moving to the soft-gate mixture formulation (Section~\ref{app:gate}) across the full holdout, is a natural robustness check. Second, the labour and interest-rate observables show large ratios in both windows; these observables enter the inversion filter's shock-recovery step and are therefore sensitive to the filter architecture, not just to the underlying model solution. I report the disaggregated numbers for transparency; the aggregate and block-level results are the quantitatively meaningful objects.

\paragraph{Quiet-sample diagnostic.} To test whether the same gate/surrogate design damages calm-period performance, I run a bounded quiet-sample pilot: estimate both models through 1994Q4 ($T_{\text{pre}} = 144$), then score one-step-ahead forecasts over 1995Q1--2007Q4 (52 quarters). The chains are shorter than the main posterior runs (200 linear draws and 150 surrogate draws, both warm-started from the full-sample posterior means, with zero post-warmup divergences), so this is a failure-mode diagnostic rather than a final forecasting comparison. Under the original gate, the acceptance criterion is not met. The switching surrogate's aggregate RMSE is 2.37 times ROM1 over the full quiet holdout, despite improving output and consumption growth. The degradation is concentrated in labour, inflation, and the policy rate, and the hard gate still activates in 26.9 percent of quiet-holdout quarters (soft-gate mean probability 8.3 percent).

I then run a fixed-posterior gate-recalibration diagnostic using the same quiet-sample chains. This diagnostic changes only the stored gate policy used by the evaluator; it does not re-estimate the surrogate posterior and should therefore be read as a localization exercise rather than a final forecasting result. Table~\ref{tab:oos_gate_recalibration} shows that the original quiet failure is largely a gate-stickiness problem. Removing the default padding alone lowers the aggregate ratio from 2.37 to 1.18 and reduces hard-gate activation from 26.9 percent to 1.9 percent. A stricter q95 threshold with default padding performs best, lowering the aggregate ratio to 1.04, the remaining-window ratio to 1.03, and the hard-gate share to 1.9 percent. This substantially narrows the calm-period failure but does not close it: ROM1 still wins slightly, and a production robustness run should re-estimate the surrogate posterior under the revised gate.

\begin{table}[htbp]
\centering
\caption{Quiet-sample one-step-ahead forecast RMSE: 1995Q1--2007Q4}
...
Observable & ROM1 & Switch & Ratio & ROM1 & Switch & Ratio & ROM1 & Switch & Ratio \\
\midrule
$\Delta y$   & 1.265 & 0.932 & 0.74 & 1.335 & 0.451 & 0.34 & 1.233 & 1.079 & 0.87 \\
$\Delta c$   & 1.866 & 1.099 & 0.59 & 1.793 & 0.459 & 0.26 & 1.897 & 1.285 & 0.68 \\
$\Delta i$   & 1.505 & 2.670 & 1.77 & 1.125 & 1.054 & 0.94 & 1.645 & 3.131 & 1.90 \\
labour       & 0.476 & 5.752 & 12.07 & 0.585 & 2.569 & 4.39 & 0.420 & 6.697 & 15.96 \\
$\pi$        & 0.166 & 1.139 & 6.85 & 0.155 & 0.283 & 1.82 & 0.171 & 1.356 & 7.93 \\
$\Delta w$   & 0.888 & 0.907 & 1.02 & 0.464 & 0.605 & 1.31 & 1.021 & 1.013 & 0.99 \\
$R$          & 0.239 & 1.784 & 7.45 & 0.248 & 0.282 & 1.14 & 0.236 & 2.136 & 9.06 \\
\midrule
Aggregate    & 1.098 & 2.607 & \textbf{2.37} & 0.993 & 1.112 & 1.12 & 1.142 & 3.045 & 2.67 \\
\bottomrule
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: One-step-ahead forecast RMSE at the quiet-sample posterior means. ROM1 uses the Kalman filter with a 200-draw linear NUTS chain. Switch uses the inversion filter with the regime-switching surrogate at the posterior mean from a 150-draw surrogate NUTS chain. Both chains are warm-started from the full-sample posterior means and have zero post-warmup divergences. Estimation sample: 1959Q1--1994Q4 ($T_{\text{pre}} = 144$); forecast sample: 1995Q1--2007Q4. The early quiet window is 1995Q1--1998Q4. Hard-gate shares are 26.9 percent over the full holdout, 0.0 percent in the early quiet window, and 38.9 percent in the remaining window; soft-gate mean probabilities are 8.3, 7.9, and 8.5 percent. Raw artifacts: \texttt{.local\_artifacts/oos\_forecast/OOS\_SUMMARY\_quiet\_1994.md} and \texttt{oos\_innovations\_quiet\_1994.jls}.}
\end{table}
...
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: Distance measured as Mahalanobis norm from prior mean. All regions satisfy RRMSE $< 0.002$ acceptance threshold.}
\end{table}

\subsection{Pending Validation Tables}

The earlier seed-sensitivity, prior-sensitivity, simplified 18-parameter validation, and sample-length recovery tables are deliberately omitted from this draft because I do not find matching local artifacts tying those numerical claims to a reproducible run. They should be restored only after the underlying runs are regenerated and documented under \path{.local_artifacts/}. The required direct Gal\'i SEP-HMC versus surrogate-HMC comparison is tracked in Appendix~\ref{sec:validation_design}.

\subsection{Surrogate Architecture Comparison}

The architecture-comparison table previously reported posterior-mode errors from the omitted three-parameter recovery workflow. I therefore remove it from the submission draft until the underlying artifact is regenerated. The current paper uses the architecture documented in the training artifacts and reports only the held-out accuracy and likelihood consequences that have matching local provenance.
...
The 34-nat log-likelihood improvement from restoring the MA terms confirms that the ARMA(1,1) markup structure captures genuine data variation and changes the posterior geometry. For the nonlinear estimation in the main text, I retain the HLT default ($\mu_p = \mu_w = 0$) to maintain consistency with the surrogate training data, noting that future work should explore estimating the MA coefficients jointly with the structural parameters.
