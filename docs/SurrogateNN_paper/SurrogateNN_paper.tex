...
A stricter dynamic HLT bridge is kept as a diagnostic rather than as evidence in this draft. In that exercise I roll a four-period observation panel directly from SEP at a held-out truth, recover shocks with the same baseline-ROM1 inversion filter used by the empirical HMC code, and evaluate direct SEP and surrogate objectives under a common measurement-error likelihood. All ten direct-SEP anchors are finite, the local MAP agrees, and the surrogate 90 percent intervals overlap the direct-SEP intervals for all four bridge parameters. The surface-shape criterion does not pass: after removing mean objective offsets, the surrogate--direct RMSE is 0.156 while ROM1--direct RMSE is 0.103. I therefore leave full dynamic HLT inversion validation as a scaling target.
...
Four-period HLT inversion-bridge diagnostic & Direct SEP is finite at 10 of 10 anchors, local MAP and 90 percent intervals agree, but centered surface RMSE is 0.156 for the surrogate versus 0.103 for ROM1; not used as a passing validation claim \\
