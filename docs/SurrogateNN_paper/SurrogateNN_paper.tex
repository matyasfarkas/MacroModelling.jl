...
\noindent
\footnotesize
This paper asks what first-order Smets--Wouters misses. The answer, in the
specification studied here, is not mainly the effective lower bound or Kimball
...
capital, and 0.2 percent to the price Phillips curve. The reason is simple:
the level investment adjustment cost has zero level and zero marginal cost at
...
four-parameter HLT bridge, all 10{,}000 supported direct-SEP cells solve; the
surrogate lowers prediction RMSE from 0.1436 to 0.00075 and log-posterior
surface RMSE from 88.07 to 0.053. The empirical posterior results are
conditional, not universal. In the 1959--2025 sample, a high-likelihood
warm-started surrogate mode improves the posterior-mean profiled objective by
35.92 nats relative to the linear Kalman posterior mean, and the linear smoother
requires risk-premium shocks of $56\sigma$ during COVID. But restoring the
ARMA(1,1) markup terms improves the linear likelihood by 34 nats on the
baseline sample, and calm-period forecasting remains gate-sensitive. The result
is evidence for an important investment-channel nonlinearity in this HLT
benchmark, not a specification-invariant rejection of linear DSGE estimation.
\end{abstract}
...
paper asks a different question: in a Smets--Wouters model, where is the
largest nonlinear error from using first order?
...
model with the same ELB and Kimball pricing but no investment block has small
gaps at relevant shock scales, which is the cleanest negative-control result
(Figure~\ref{fig:gali_obc_vs_no_obc}).

To estimate the nonlinear model, I use a surrogate that learns only what first
order misses:
\[
...
10{,}000 supported direct-SEP cells solve; the surrogate lowers prediction RMSE
from 0.1436 to 0.00075 and log-posterior surface RMSE from 88.07 to 0.053. The
online technical appendix documents the computation, output files, and
validation criteria.

The posterior results are useful but conditional. In the 1959--2004 sample,
nonlinear estimation is associated with higher risk-premium volatility and
lower wage-markup persistence. In the 1959--2025 sample, a high-likelihood
warm-started surrogate mode improves the posterior-mean profiled objective by
35.92 nats, and the linear smoother requires risk-premium shocks of
\(56\sigma\) during COVID. This comparison is not a Bayes factor. It is
conditional on one U.S. sample, one HLT specification, one posterior basin, the
surrogate approximation, the profiled inversion objective, the gate, and COVID
...
baseline sample, almost the same size as the nonlinear objective gap in the
extended sample. I therefore read the posterior shifts as evidence relative to
the HLT benchmark, not as a specification-invariant rejection of linear DSGE
estimation.

The main message is narrow. For this class of models, the first nonlinear
object to audit is the investment block, not the ELB or Kimball pricing. The
deterministic decomposition is the strongest evidence. The posterior
comparison supports the same mechanism, but it remains model- and
implementation-specific. Calm-period forecasting confirms the caution: the
original gate fails on 1995Q1--2007Q4, and a q95 padded re-estimation improves
the aggregate RMSE ratio to 1.09 but does not beat ROM1 over the full quiet
holdout.
...
rules: the first-order perturbation \(g_\theta^{\mathrm{ROM1}}\) and the
direct nonlinear SEP solution \(g_\theta^{\mathrm{SEP}}\).
...
samples them directly in small validation models or profiles them out through
the inversion filter in the 18-parameter application:
\begin{equation*}
...
\end{equation*}
where \(s_t=\hat g_\theta(s_{t-1},\eps_t)\). This avoids the
particle-weight degeneracy documented below and keeps the likelihood comparison
operational in a medium-scale model.

\subsection{Illustrative Models}
\label{subsec:gali_model}

The Gal\'i (2015) model is a negative control. It has Calvo pricing, a Taylor
rule, and an occasionally binding lower bound, but no investment or capital
block:
\[
R_t = \max\{1, (R_{ss})^{1-\rho_R}(\Pi_t/\Pi_{ss})^{(1-\rho_R)\phi_\pi}(Y_t/Y_{ss})^{(1-\rho_R)\phi_y}R_{t-1}^{\rho_R}e^{\varepsilon_t^R}\},
\]
Appendix~\ref{sec:validation_design} reports the maintained hard-ELB validation
package for this model. The full equations are in Appendix~\ref{app:model}.

The main application uses the SW07-HLT model: habit formation, capital
...
\((\xi_p,\iota_p,\varepsilon_p,\xi_w)\). Appendix~\ref{app:sw07_model}
...
NUTS-HMC. Since the inversion step profiles over shocks without a reported
Jacobian or Laplace determinant, I call the resulting object a profiled log
objective rather than a marginal likelihood.
...
Appendix~\ref{app:sep} and in the online technical appendix.
...
Appendices~\ref{app:surrogate} and~\ref{app:shock_scale_calibration}.
...
criterion is a profiled Gaussian objective, not an exact likelihood unless the
missing determinant term is added (Appendix~\ref{app:inversion}).
...
divergent transitions. Details are in Appendix~\ref{app:hmc}.
...
the profiled objective by 35.92 nats and avoids interpreting COVID as a sequence
of extreme linear shocks. But this is conditional on the HLT specification, one
country, one posterior basin, the inversion objective, the surrogate, and the
gate. The ARMA markup comparison and the quiet-sample forecast results show why
...
compare a direct nonlinear benchmark with a feasible Bayesian estimator. The
next step is clear: extend the direct validation beyond the reported HLT bridge
and test the investment result in other medium-scale models with adjustment
costs.
