...
\]
The online appendix reports the maintained hard-ELB validation package and the
full equations for this model.
...
\((\xi_p,\iota_p,\varepsilon_p,\xi_w)\). The online appendix summarizes the
full system.
...
missing determinant term is added (online technical appendix).
...
State distance alone explains 96 percent of the nonlinear correction ($R^2 = 0.960$): the gap is a function of where the economy sits, not which parameters generated the displacement. ROM1 errors grow by two orders of magnitude from the nearest to the farthest state-distance quintile; the surrogate stays flat, achieving 99 percent improvement in the farthest quintile (online appendix).
...
Structural parameters explain 11 percent of nonlinearity variation. The strongest correlates are government spending persistence $\rho_g$ ($+0.18$), risk-premium volatility $\sigma_b$ ($+0.17$), and Kimball curvature $\varepsilon_p$ ($-0.11$)---higher curvature flattens demand and makes the model \emph{more} linear. Current-period shocks contribute negligibly ($R^2 = 0.0003$). Nonlinearity accumulates through persistent displacement, clustering in temporal bursts with autocorrelation 0.81 and mean duration 12.5 quarters; per-observable $R^2$ values remain below 12 percent (online appendix).
...
In rare ZLB-binding episodes the investment share falls---to 90 percent at shock scale 1.2 and 37 percent at scale 1.5. At posterior-estimated parameters, however, binding occurs in only 0.1--0.4 percent of periods, so the unconditional composition remains investment-dominated at 88--92 percent (online appendix). That figure exceeds the training-set average of 69 percent because Sobol-sampled draws span parameter regions where the investment channel is weaker. Both exercises point the same way: investment dominates, and the ELB remains subordinate.


\subsection{Gal\'i Negative Control}
\label{subsec:gali_control}

The \citet{gali2015monetary} model shares the ELB and Kimball nonlinearities with the HLT model but lacks investment---a difference-in-differences design. If the ELB or Kimball pricing were the dominant nonlinearity, the Gal\'i model should show comparable gaps. It does not: the gap is an order of magnitude smaller at every shock scale (online appendix).
...
I estimate the full 18-parameter SW07-HLT model on U.S. data (1959Q1--2004Q4, 184 quarters, 7 observables). The linear Kalman baseline uses 1,000 NUTS draws, has zero divergences, and takes 100 minutes; the online appendix reports the full baseline table.

\paragraph{Regime-switching surrogate estimation.}
\label{par:results_18param_switching}

The regime-switching gate classifies 10 percent of periods as nonlinear---concentrating around the Volcker disinflation, the early-1990s recession, and the dot-com bust---and completes in 2.4 hours with zero divergent transitions. The 18-nat profiled-objective gap ($-1{,}045$ vs.\ $-1{,}027$) is small relative to the objective scale itself.
...
\paragraph{Scope limitations.} The baseline training data use shock scale $\sigma = 0.1$, at which the ZLB rarely binds. A supplementary dataset at $\sigma = 0.4$ adds 20,240 ZLB-binding samples, achieving RRMSE $= 0.12$ on binding episodes. The OOD detection mechanism falls back to ROM1 for states outside the training envelope, but the surrogate's extrapolation behavior at intermediate out-of-distribution states remains a source of unquantified error. Section~\ref{subsec:extended_sample} extends both estimations through 2025Q1.
...
Surrogate accuracy is strong in the non-binding validation design but weaker in the lower-bound stress region. Across most parameter regions the held-out RRMSE is below 0.002, while the far-tail 95th percentile is 0.00231. The combined surrogate (63,939 samples including 20,240 ZLB-binding episodes at shock scale 0.4) achieves RRMSE of 0.12 on binding-period test data---an 89 percent improvement over the version trained without ZLB episodes, but much larger than the non-binding validation error. Gal\'i seed and prior sensitivity tables are excluded because the present validation evidence comes from the hard-ELB package summarized in the online appendix.

During ZLB-binding episodes, ROM1 predicts deeply negative rates; the surrogate adds back the approximate lower-bound correction and tracks the SEP benchmark qualitatively across the reported observables (online appendix).
...
The baseline surrogate is trained at shock scale 0.1, where the ZLB rarely binds; the supplementary dataset at scale 0.4 extends coverage to binding episodes. The same surrogate can be evaluated on different sample periods without retraining because the neural network maps from parameter-state-shock space rather than time-series indices, but this does not eliminate out-of-distribution risk. The online technical appendix's coverage-distortion diagnostic evaluates to below one percentage point at the non-binding validation RRMSE of 0.0012; it should not be applied to the binding-region RRMSE of 0.12. A direct SEP comparison at twenty posterior draws shows that the ROM1--SEP prediction gap (average RMSE 1.04 per variable, exceeding $3\sigma_y$ for labor and the interest rate) is large relative to the non-binding surrogate training error.

To bridge the gap to the 18-parameter estimation, I provide two lines of current evidence: (i) the formal consistency theory shows coverage distortion scales as $\sqrt{T} \cdot \text{RRMSE}^2$ with spectral prefactor $\sqrt{\kappa}/\sqrt{\lambda_{\min}(\mathcal{I}(\theta_0))}$ under the score-control assumption, giving a below-one-percentage-point diagnostic at the non-binding validation RRMSE; (ii) the twenty-draw SEP comparison provides an empirical full-scale solution check. A Monte Carlo coverage study at the full 18-parameter scale validates only the linear Kalman + NUTS pipeline: it achieves average coverage of 79.8\% across 14 replications, with the Clopper--Pearson 95\% confidence interval containing the nominal 90\% rate for 16 of 18 parameters. It is not evidence on surrogate posterior coverage. The direct Gal\'i SEP-HMC comparison is listed separately in the online appendix.
