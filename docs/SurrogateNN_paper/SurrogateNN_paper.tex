...
Structural parameters explain 11 percent of nonlinearity variation in separate diagnostic regressions. The strongest correlates are government spending persistence $\rho_g$ ($+0.18$), risk-premium volatility $\sigma_b$ ($+0.17$), and Kimball curvature $\varepsilon_p$ ($-0.11$)---higher curvature flattens demand and makes the model \emph{more} linear. Current-period shocks contribute negligibly ($R^2 = 0.0003$). Nonlinearity accumulates through persistent displacement, clustering in temporal bursts with autocorrelation 0.81 and mean duration 12.5 quarters; per-observable diagnostic $R^2$ values remain below 12 percent. A fixed-calibration stress check at the \citet{harding2022missingdeflation} high-curvature neighborhood, mapping their reported $\varphi_p=1.20$, $\xi_p=0.667$, and $\varepsilon_p=77.3$ values plus the matching reported core and shock-process parameters into the maintained code, gives an investment/capital share of 76.5 percent and a price Phillips share of 2.0 percent. A posterior-region local ablation gives the complementary check: across 40 representative posterior draws and shock scales 0.1, 0.25, and 0.5, all SEP paths converge; Kimball curvature is the largest local sensitivity in 5, 0, and 0 percent of draws, while investment-adjustment and utilization curvature are largest in the remaining 95, 100, and 100 percent. Thus nonlinear Phillips-curve effects can be generated at selected high-curvature calibrations, but the estimated posterior region studied here puts the largest local nonlinear residual in the real investment block.
