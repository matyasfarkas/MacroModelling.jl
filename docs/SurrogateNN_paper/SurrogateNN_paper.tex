...
\]
where the $\max\{1,\cdot\}$ operator enforces the ZLB. The direct SEP-HMC versus surrogate-HMC validation on this model is a planned validation exercise rather than a result reported here; Appendix~\ref{sec:validation_design} records its design and current status. Full equations are in Appendix~\ref{app:model}.
...
The local validation harness exercises the HLT surrogate pipeline on synthetic data and includes a bounded direct-SEP/MCMC comparison script for the HLT legacy three-parameter block. The repository currently has finite direct-SEP inversion checks, but it does not contain the matched Gal\'i SEP-HMC versus surrogate-HMC posterior comparison described in Appendix~\ref{sec:validation_design}. I therefore do not use the Gal\'i recovery exercise as evidence here. I now turn to the 18-parameter estimation on U.S. data.
...
This appendix presents the complete equilibrium system for the Gal\'i (2015) New Keynesian model with occasionally binding zero lower bound. It serves as a negative-control specification and as the planned target for the direct SEP-HMC versus surrogate-HMC validation; numerical recovery results from that exercise are not reported in this paper. The 18-parameter analysis in Section~\ref{subsec:nonlinearity_sources} uses the richer \citet{smets2007shocks} specification, detailed in Appendix~\ref{app:sw07_model} below.
...
\]
The surrogate would need non-binding RRMSE exceeding about $0.0013$ before this conservative theoretical bound reached one percentage point at this sample size, so the realized value of $0.0012$ is close to the threshold rather than an order of magnitude below it. The bound applies worst-case RRMSE uniformly across all periods and all state dimensions. It does not cover the separate binding-region RRMSE of 0.12, and the 18-parameter Monte Carlo coverage study in Appendix~\ref{app:mc_coverage} validates only the linear Kalman + NUTS pipeline. The direct Gal\'i SEP-HMC comparison remains the required posterior-validation exercise for the surrogate.
\end{remark}
...
Theorem~\ref{thm:posterior_consistency} requires $\delta_T = o(T^{-1/2})$, a condition that cannot be checked directly since the FOM solution at the posterior mode is unavailable during online estimation. Three diagnostics provide partial evidence. First, the held-out RRMSE (Table~\ref{tab:surrogate_regional_accuracy}) measures error at parameter values not used in training, while also revealing a far-tail cell above 0.002 and a much larger ZLB-binding stress error. Second, the realized log-likelihood bias bound from Proposition~\ref{prop:ll_bias_bound} evaluates to $B_T \leq 0.02$ nats at the non-binding validation RRMSE, and the coverage-distortion bound from Corollary~\ref{cor:coverage} is below one percentage point at that same value. Third, direct SEP evaluations at twenty subsampled posterior draws show that the ROM1--SEP prediction gap averages 1.04 RMSE per variable, with all twenty SEP simulations converging and the largest gaps concentrated in labor (3.07) and the interest rate (0.86), consistent with the equation-block decomposition identifying investment dynamics as the dominant nonlinear channel.


\section{Validation Design}
\label{sec:validation_design}

The validation target is a direct SEP-HMC versus surrogate-HMC comparison on the Gal\'i (2015) synthetic model in Section~\ref{subsec:gali_model}. The DGP should use the same SEP solution and priors for both samplers, with identical data, seeds, and chain lengths. The comparison is acceptable if posterior means are within Monte Carlo error, 90 percent intervals materially overlap, and no systematic coverage distortion appears.

I do not report that matched Gal\'i direct-HMC comparison here. Existing Gal\'i diagnostics show that the SEP engine solves the model at a cached true parameter value and that a smaller surrogate-HMC validation path exists, but they do not constitute the requested three-parameter direct SEP-HMC versus surrogate-HMC posterior comparison. I therefore treat the Gal\'i exercise as a design target rather than as posterior-validating evidence.
...
I therefore do not report numerical posterior recovery results from the Gal\'i validation or the full direct posterior comparison here. Reproducibility details for the available diagnostics belong in the replication package rather than in the manuscript.

\section{Validation Status}
\label{app:validation_results}

Table~\ref{tab:param_recovery} summarizes the current validation status. It separates finite direct-SEP diagnostics from the matched Gal\'i direct SEP-HMC versus surrogate-HMC comparison that remains necessary for submission-grade posterior validation.

\begin{table}[htbp]
\centering
\caption{Direct posterior-validation status}
...
Item & Status \\
\midrule
Common Gal\'i SEP DGP & Existing synthetic data found; direct posterior DGP not reported \\
Direct SEP likelihood HMC & Not reported for the matched three-parameter comparison \\
Surrogate likelihood HMC & Existing two-parameter run found; requested three-parameter run not reported \\
Common priors, seeds, and chain lengths & Not reported \\
Posterior mean and interval comparison & Not reported \\
Coverage and distortion diagnostic & Not reported \\
...
\end{tabular}
\smallskip
\tablenotes{\textit{Notes}: The repository has Gal\'i surrogate assets, HLT three-parameter validation assets, and direct SEP diagnostic checks, but not the Gal\'i direct SEP-HMC versus surrogate-HMC run required for submission-grade posterior validation.}
\end{table}
