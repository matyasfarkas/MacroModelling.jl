...
\begin{flushleft}
\footnotesize
\textbf{Notes}: Illustration of the dataset generation pipeline. For each parameter value $\theta^{(i)}$ in a Sobol-sequence grid, we simulate the model at randomly sampled state-shock pairs $(s_{-1}, \varepsilon)$. Each SEP solution is paired with the ROM1 prediction at the same input to produce a residual target $\Delta s_t = s_t^{\rm SEP} - s_t^{\rm ROM1}$. The complete dataset captures the parameter-conditional nonlinear correction across the prior support.
\end{flushleft}
\end{figure}
...
\node[below=1mm of output, font=\footnotesize] {$\Delta s_t$ (ROM1 residual)};
...
\begin{flushleft}
\footnotesize
\textbf{Notes}: Schematic of the feedforward multilayer perceptron (MLP) used to approximate the SEP-minus-ROM1 residual. The network takes as input the previous state $s_{-1}$, current shock $\varepsilon$, and structural parameters $\theta$. Two hidden layers provide nonlinear feature extraction. The output layer produces the residual correction $\Delta s_t$ that is added back to the ROM1 prediction. All inputs and residual targets are standardized to zero mean and unit variance during training.
\end{flushleft}
\end{figure}
